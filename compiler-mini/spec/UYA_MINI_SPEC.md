...
- **状态**：⚠️ 此特性已在规范中定义，但编译器实现状态为"未实现"（P1 优先级，参见 `TODO_uya_mini_extension.md`）
...
- **字符串字面量**：`"..."` ⚠️ 计划支持，当前未实现
  ...
  - **状态**：此特性已在规范中定义，但编译器实现状态为"未实现"（P1 优先级，参见 `TODO_uya_mini_extension.md`）
  - 示例：
    ```uya
    extern fn strcmp(s1: *byte, s2: *byte) i32;
    extern fn fopen(filename: *byte, mode: *byte) *FILE;
...
- **最后更新**：2026-01-13
- **目的**：Uya Mini 编译器自举实现
- **状态说明**：规范文档定义了完整的语法和语义规则。部分特性（如字符串字面量）标记为"计划支持"，当前编译器实现状态请参考 `TODO_uya_mini_extension.md`
