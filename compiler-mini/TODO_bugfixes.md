...
6. ✅ **关键发现（2026-01-11，继续调试）**：
   - 通过添加调试输出，发现问题的根本原因：**解析阶段失败**
   - `logical_expr.uya`（逻辑与）有 0 个声明，说明解析器在解析时失败并提前停止
   - `logical_expr3.uya`（逻辑或）有 1 个声明，可以成功解析
   - `boolean_logic.uya` 只有 2 个声明（is_positive, is_even），缺少 main 函数，说明 main 函数的解析失败
   - 编译器返回 0（成功），但解析失败，说明错误没有被正确报告
   - **结论**：问题不在代码生成阶段，而是在解析阶段。包含逻辑与表达式的函数在解析时失败，但错误没有被正确报告。
7. ✅ **深度调查发现（2026-01-11，继续调试）**：
...
8. ✅ **详细测试结果模式（2026-01-11，深入调试）**：
   - **可以成功解析的情况**：
     ...
     - ✅ `simple_function.uya` 中的相等性表达式（`if result == 30`，其中 result 是通过函数调用初始化的）
   - **解析失败的情况**：
   ...
   - **关键观察**：
     ...
     - 但 `control_flow.uya` 中的 `if x == 10 && count == 5` 可以成功，说明问题更复杂
9. 🔄 **下一步调查方向**：
   - 深入检查 `parser_parse_and_expr` 和 `parser_parse_or_expr` 函数的实现差异
   - 检查表达式解析的调用链（`parser_parse_expression` → `parser_parse_assign_expr` → `parser_parse_or_expr` → `parser_parse_and_expr` → `parser_parse_eq_expr` → `parser_parse_rel_expr`）
   - 特别关注两个标识符比较时的解析路径
   - 修复解析器的错误处理机制，确保解析失败时正确报告错误
   - 修复导致两个标识符比较解析失败的问题
10. 🔄 修复逻辑与运算符的解析问题
