...
**状态**：⏳ 进行中（2026-01-27）- 已完成：基础类型、算术、比较、布尔逻辑、控制流、数组访问、赋值、逻辑表达式、嵌套控制、多参数、嵌套函数调用测试（全部通过）；部分完成：简单结构体测试（struct_test, struct_assignment, nested_struct通过）；待完成：结构体参数、结构体比较、嵌套结构体访问、外部函数、完整测试集
