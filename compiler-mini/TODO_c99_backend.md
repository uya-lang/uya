...
**状态**：⏳ 进行中（2026-01-27）- 已完成：基础类型、算术、比较、布尔逻辑、控制流、数组访问、赋值、逻辑表达式、嵌套控制、多参数、嵌套函数调用、结构体测试、结构体赋值、嵌套结构体、结构体比较、嵌套结构体访问、外部函数测试、枚举测试、字符串常量转义（全部通过），`run_programs.sh` C99 选项支持；测试通过率：111/131 (84.7%)；核心功能测试全部通过；待完成：修复剩余20个失败的测试用例；已知问题：部分边缘测试用例失败（复杂表达式、某些特殊场景等，需要进一步修复）

**最新修复**（2026-01-27）：
- ✅ 修复指针字段访问问题：函数参数中的指针类型字段访问现在正确使用 `->` 而不是 `.`
- ✅ 修复 sizeof/alignof 中的类型名称问题：结构体类型现在正确添加 `struct` 前缀（通过从程序节点查找结构体声明）
