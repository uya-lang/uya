...
- [x] 实现类型映射函数 `c99_type_to_c()`：
  ...
  - [x] 指针类型映射（`&T` → `T*`，`*T` → `T*`） ✅
  - [ ] 数组类型映射（`[T: N]` → `T[N]`） ⚠️ 部分完成（大小表达式未评估，目前使用占位符大小1）
...
**输出**：部分的类型映射功能 ⚠️ 基础类型、指针、结构体支持已完成；数组大小和枚举支持待完成

**状态**：⚠️ 部分完成（2026-01-26）- 已完成：基础类型映射、指针类型、结构体定义；待完成：数组大小评估、枚举类型、嵌套结构体处理、字段对齐
...
**输出**：部分的函数生成功能 ⚠️ 支持普通函数定义；全局变量和外部函数声明待实现

**状态**：⚠️ 部分完成（2026-01-26）- 已完成：普通函数定义、参数列表、函数体；待完成：外部函数声明、全局变量（const/var）、初始化表达式处理

**下一步**：
- 实现 `c99_gen_global_var()` - 生成全局变量和常量定义
- 处理全局变量初始化表达式（支持编译时常量）
...
**状态**：⚠️ 部分完成（2026-01-26）- 主生成流程框架已完成，文件头生成、结构体定义、函数定义已支持；待完成：枚举定义、全局变量、函数前向声明、字符串常量输出、依赖关系处理

**下一步**：
- 在 `c99_codegen_generate()` 中添加枚举定义生成（遍历所有枚举声明）
- 添加全局变量定义生成（在函数定义之前输出）
- 添加函数前向声明生成（解决相互递归调用问题）
- 在适当位置调用 `emit_string_constants()` 输出字符串常量
...
- [ ] 创建测试用例：
  ...
  - 外部函数测试
- [ ] 编译测试：
  - 使用 `gcc --std=c99` 编译生成的 C99 代码
  - 运行生成的可执行文件，验证行为
- [ ] 对比测试：
  - 相同 Uya Mini 代码的 LLVM 后端和 C99 后端输出对比
  - 确保语义一致
- [ ] 集成测试：
  - 编译现有的测试程序（`tests/programs/`）
  - 确保所有测试程序都能正确编译和运行
...
**状态**：⏳ 待开始 - 需要等待核心功能（表达式、语句、全局变量）完成后才能开始测试
...
| 任务 3：类型系统 | ⚠️ 部分完成 | 70% | P0 |
| 任务 4：表达式生成 | ⚠️ 部分完成 | 30% | P0 |
| 任务 5：语句生成 | ⚠️ 部分完成 | 40% | P0 |
| 任务 6：函数和全局变量 | ⚠️ 部分完成 | 50% | P0 |
| 任务 7：主程序集成 | ✅ 已完成 | 100% | P0 |
| 任务 8：代码生成器入口 | ⚠️ 部分完成 | 60% | P0 |
| 任务 9：测试 | ⏳ 待开始 | 0% | P1 |
...
- ⚠️ 表达式和语句支持不完整，大量 AST 节点类型待实现
- ⚠️ 全局变量、枚举、字符串常量处理待完成

**下一步优先级**：
1. **高优先级**：完成任务 4（表达式生成）和任务 5（语句生成）- 这是核心功能
2. **中优先级**：完成任务 3（数组大小评估）和任务 6（全局变量）
3. **低优先级**：任务 9（测试）和任务 10（文档）- 可在核心功能完成后进行
