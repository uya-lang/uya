...
**状态**：⏳ 进行中（2026-01-27）- 已完成：基础类型、算术、比较、布尔逻辑、控制流、数组访问、赋值、逻辑表达式、嵌套控制、多参数、嵌套函数调用、结构体测试、结构体赋值、嵌套结构体、结构体比较、嵌套结构体访问、外部函数测试（全部通过），`run_programs.sh` C99 选项支持；待完成：完整测试集验证（部分完成）；已知问题：null 关键字已处理为 NULL
...
- [ ] **保证 `tests/run_programs.sh` 所有测试用例通过**（进行中，核心测试已通过）
