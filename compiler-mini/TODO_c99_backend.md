...
- [x] 实现结构体定义生成：
  ...
  - [ ] 处理字段对齐（当前使用默认对齐）
- [x] 实现枚举类型映射：
  - [x] `enum Name` → `enum Name` ✅（在c99_type_to_c中实现）
  - [x] 生成枚举常量定义 ✅（gen_enum_definition函数）

**输出**：完整的类型映射功能 ✅ 基础类型、指针、数组、结构体、枚举支持已完成

**状态**：✅ 已完成（2026-01-27）- 已完成：基础类型映射、指针类型、数组类型（含编译时常量表达式评估）、结构体定义、枚举类型映射和定义生成；待完成：嵌套结构体处理（可选）、字段对齐（可选）

**说明**：
- ✅ 枚举类型映射已完成（`enum Name` → `enum Name`）
- ✅ 枚举定义生成已完成（`gen_enum_definition`函数）
- ⚠️ 枚举值访问（如`Color.RED`）需要在表达式生成中处理（见任务4）
...
- ✅ 枚举类型映射和常量定义生成已完成
- 测试嵌套结构体定义（可选）
- 考虑添加 `#pragma pack` 或 `_Alignas` 处理字段对齐（可选）
...
- [x] 处理运算符优先级和括号（当前使用保守的括号策略，功能正确但可能生成多余括号）✅
- [x] 处理指针解引用（`*expr`）和取地址（`&expr`）✅（通过一元表达式实现）
- [x] 处理结构体字段访问（自动解引用）✅（指针类型自动使用`->`，非指针类型使用`.`）
- [x] 处理枚举值访问（`Color.RED` → `RED`）✅

**输出**：完整的表达式生成功能 ✅ 支持所有主要表达式类型，包括枚举值访问和结构体指针自动解引用

**状态**：✅ 已完成（2026-01-27）- 已完成所有主要表达式节点类型；枚举值访问和结构体字段自动解引用已实现

**下一步**：
- ✅ 括号使用策略已确定（保守策略，确保正确性）
- ✅ 结构体字段访问的自动解引用已完成（指针->字段自动转为`->`）
- ✅ 枚举值访问已实现（`Color.RED` → `RED`）
- ✅ 数组字面量和结构体字面量的类型推断已基本完成
...
- [x] 处理变量声明（`AST_VAR_DECL`）：
  - [x] 全局变量声明 ✅（在gen_global_var中处理）
  - [x] 局部变量声明 ✅
  - [x] 函数参数 ✅（在函数参数列表中处理）
  - [x] 结构体字段 ✅（在结构体定义中处理）
- [x] 处理作用域和变量生命周期 ✅（使用local_variables表跟踪变量，从后往前查找支持最新变量，满足C99代码生成需求）
- [x] 处理循环控制（break/continue）✅（基础实现）
- [x] 改进 for 循环实现 ✅（数组元素类型推断已实现，多维数组支持已完成）

**输出**：基本完整的语句生成功能 ✅ 支持所有主要语句类型

**状态**：✅ 已完成（2026-01-27）- 已完成所有主要语句节点类型；作用域管理已实现（使用local_variables表，满足C99代码生成需求）

**下一步**：
- ✅ 全局变量声明生成已完成（gen_global_var函数）
- ✅ for循环数组元素类型推断已完成（get_array_element_type函数）
- ✅ 多维数组在for循环中的支持已完成（2026-01-27修复）
- ✅ 作用域管理已完成（使用local_variables表跟踪变量，从后往前查找）
- 实现循环栈（loop_stack）支持带标签的 break/continue（可选功能，当前基础实现已满足需求）
...
- [x] 实现 `c99_gen_function()` 函数：
  - [x] 生成函数声明（外部函数）✅（在gen_function_prototype中实现）
  - [x] 生成函数定义（普通函数） ✅
  - [x] 处理参数列表 ✅
  - [x] 处理函数体 ✅
- [x] 实现 `c99_gen_global_var()` 函数：
  ...
  - [x] 生成全局变量（`var`） ✅
  - [x] 处理初始化表达式 ✅
- [x] 处理外部函数声明（`extern fn`）：
  - [x] 生成相应的 C 函数声明（添加extern关键字）✅
  - [x] 处理 FFI 指针类型参数（*T和&T都映射为T*）✅

**输出**：基本完整的函数和全局变量生成功能 ✅ 支持普通函数定义、外部函数声明和全局变量（const/var）

**状态**：✅ 已完成（2026-01-27）- 已完成：函数声明（包括外部函数）、函数定义、参数列表、函数体、全局变量（const/var）定义、初始化表达式处理、外部函数声明生成；所有功能已完整实现

**下一步**：
- ✅ 外部函数声明生成已完成（在gen_function_prototype中为extern函数添加extern关键字）
- ✅ FFI指针类型参数处理已完成（*T和&T在C99中都映射为T*）
...
- [x] 处理依赖关系：
  - [x] 结构体之间的依赖 ✅（通过前向声明解决）
  - [x] 函数之间的调用关系 ✅（通过函数前向声明解决）
- [x] 优化输出格式：
  - [x] 合理的缩进和换行 ✅
  - [x] 有意义的注释 ✅

**输出**：完整的 C99 代码生成器 ✅ 支持结构体、枚举、全局变量、函数定义、函数前向声明、结构体前向声明和字符串常量

**状态**：✅ 已完成（2026-01-27）- 主生成流程完全实现，文件头生成、结构体定义、枚举定义、全局变量定义、函数定义、函数前向声明、结构体前向声明、字符串常量输出已全部支持；依赖关系处理已完成

**说明**：
- ✅ 所有功能已完整实现
- ✅ 依赖关系分析已完成（通过前向声明解决结构体和函数依赖）
- ✅ 输出格式已优化（缩进、换行、注释）
...
- [x] 创建测试用例：
  - [x] 基础类型测试 ✅
  - [x] 结构体测试 ✅（结构体定义、访问、比较、嵌套结构体测试全部通过）
  - [x] 控制流测试 ✅
  - [x] 函数调用测试 ✅
  - [x] 外部函数测试 ✅（extern_function.uya测试通过）
- [x] 编译测试：
  - [x] 使用 `gcc --std=c99` 编译生成的 C99 代码 ✅
  - [x] 运行生成的可执行文件，验证行为 ✅
- [x] 对比测试：
  - [x] 相同 Uya Mini 代码的 LLVM 后端和 C99 后端输出对比 ✅（通过测试验证）
  - [x] 确保语义一致 ✅（核心测试用例验证通过）
- [x] 集成测试：
  - [x] 编译现有的测试程序（`tests/programs/`）✅ - **脚本已支持 `--c99` 选项，可进行测试**
  - [x] 确保所有测试程序都能正确编译和运行 ✅（全部通过，100%通过率）
  - [x] **保证 `tests/run_programs.sh` 核心测试用例通过**✅（所有测试用例全部通过，100%通过率）
  - [x] 验证多文件编译测试用例在 C99 后端下正常工作 ✅（多文件编译支持已实现，测试通过）

**输出**：通过所有测试的 C99 后端

**状态**：✅ 已完成（2026-01-27）- 已完成：基础类型、算术、比较、布尔逻辑、控制流、数组访问、赋值、逻辑表达式、嵌套控制、多参数、嵌套函数调用、结构体测试、结构体赋值、嵌套结构体、结构体比较、嵌套结构体访问、外部函数测试、枚举测试、字符串常量转义、`run_programs.sh` C99 选项支持、sizeof/alignof 结构体类型、多维数组 for 循环（全部通过）；测试通过率：100%（所有测试用例全部通过）

**最新修复**（2026-01-27）：
- ✅ 修复指针字段访问问题：函数参数中的指针类型字段访问现在正确使用 `->` 而不是 `.`
- ✅ 修复 sizeof/alignof 中的结构体类型名称问题：结构体类型现在正确生成 `sizeof(struct Point)` 而不是 `sizeof(Point)`
  - 问题根源：解析器将 `sizeof(Point)` 解析为表达式而不是类型，导致 `is_type = 0`
  - 解决方案：在 `c99_gen_expr` 的 `AST_SIZEOF` 处理中，即使 `is_type = 0`，也检查标识符是否是结构体名称
  - 使用 `is_struct_in_table` 和备用查找（从程序节点）来识别结构体类型
  - ✅ 测试验证通过：`test_sizeof_test.uya` 测试用例全部通过
- ✅ 修复函数参数中数组类型的语法：`int32_t[3] arr` 现在正确转换为 `int32_t arr[3]`
- ✅ 修复数组赋值问题：数组赋值现在使用 `memcpy` 而不是直接赋值
- ✅ 修复 sizeof 数组指针语法问题：`sizeof(uint8_t[10] *)` 现在正确转换为 `sizeof(uint8_t(*)[10])`
- ✅ 修复多维数组在 for 循环中的代码生成问题（2026-01-27）：
  - 问题：对于 `[[i32: 2]: 3]` 这样的二维数组，在 `for arr2d |row|` 循环中，`row` 的类型应该是 `[i32: 2]`（一维数组），但代码生成时错误地将其生成为 `i32`（标量类型）
  - 解决方案：
    1. 修改 `get_array_element_type` 函数：正确处理多维数组类型字符串，对于 `int32_t[3][2]`，返回 `int32_t[2]`（去掉最外层维度）
    2. 修改 for 循环代码生成：对于数组类型的值迭代，将类型字符串从 `int32_t[2]` 转换为 C 数组声明格式 `int32_t row[2]`，并使用 `memcpy` 复制数组内容
  - 生成的代码示例：
    ```c
    // 修复前（错误）：
    int32_t row = arr2d[_i];  // 类型不匹配
    
    // 修复后（正确）：
    int32_t row[2];
    memcpy(row, arr2d[_i], sizeof(row));  // 正确复制数组内容
    ```
  - ✅ 测试验证通过：`test_multidimensional_array.uya` 测试用例全部通过

**说明**：
- `tests/run_programs.sh` 是项目的标准测试脚本，包含 87+ 个测试用例
- C99 后端必须通过所有现有测试，确保与 LLVM 后端语义一致
- 需要特别关注多文件编译、结构体、数组、指针等复杂测试用例
- ✅ **最终状态**（2026-01-27）：所有测试用例全部通过，C99 后端已完全验证，与 LLVM 后端语义一致
...
3. ✅ 嵌套结构体和结构体指针的处理 - 已解决（测试验证通过）
4. ✅ null关键字的处理 - 已解决（替换为NULL）
5. ✅ 外部函数声明支持 - 已解决（添加extern关键字）
6. ✅ 完整测试集验证 - 已完成（所有测试用例全部通过）
...
- [x] 更新 `README.md`：
  ...
  - [x] 添加命令行选项文档 ✅
- [x] 更新 `Makefile`：
  - [x] 添加 `c99-backend` 目标（单独编译 C99 后端示例）✅
  - [x] 添加 `test-c99` 目标（运行 C99 后端测试）✅
- [x] 更新 `BOOTSTRAP_PLAN.md`：
  - [x] 记录 C99 后端实现过程 ✅
  - [x] 更新编译器架构图 ✅
- [x] 创建示例：
  - [x] `examples/c99_output/` - 展示生成的 C99 代码 ✅
  - [x] `examples/compile_with_c99.sh` - 编译脚本 ✅

**输出**：完整的文档和集成 ✅

**状态**：✅ 已完成（2026-01-27）- 所有文档和集成任务已完成：
- ✅ Makefile 已添加 `c99-backend` 和 `test-c99` 目标
- ✅ BOOTSTRAP_PLAN.md 已更新，记录 C99 后端实现过程和架构更新
- ✅ 示例目录和文件已创建（`examples/c99_output/` 包含示例程序和生成的 C99 代码）
- ✅ 编译脚本已创建（`examples/compile_with_c99.sh`）
- ✅ README.md 已更新，添加 C99 后端使用说明
...
| 任务 3：类型系统 | ✅ 已完成 | 100% | P0 |
| 任务 4：表达式生成 | ✅ 已完成 | 100% | P0 |
| 任务 5：语句生成 | ✅ 已完成 | 100% | P0 |
| 任务 6：函数和全局变量 | ✅ 已完成 | 100% | P0 |
| 任务 7：主程序集成 | ✅ 已完成 | 100% | P0 |
| 任务 8：代码生成器入口 | ✅ 已完成 | 100% | P0 |
| 任务 9：测试 | ✅ 已完成 | 100% | P1 |
| 任务 10：文档和集成 | ✅ 已完成 | 100% | P1 |
...
- ✅ 枚举定义支持已完成
- ✅ 函数前向声明和字符串常量输出支持已完成
- ✅ 外部函数声明生成已完成（添加extern关键字）
- ✅ for循环数组元素类型推断已完成
- ✅ 结构体前向声明和依赖关系处理已完成
- ✅ **所有测试用例全部通过（100%通过率）**✅

**下一步优先级**：
1. **低优先级**：优化运算符优先级和括号使用（任务4，可选优化）
2. **已完成**：所有核心任务已完成，C99 后端已完全实现并集成 ✅
...
- [x] C99 后端架构设计完成
- [x] 基础框架实现完成
- [x] C99 后端可以正确编译大部分 Uya Mini 测试程序（核心功能已验证）
- [x] 生成的 C99 代码可以通过 `gcc --std=c99` 编译
- [x] 生成的可执行文件行为与 LLVM 后端一致（已验证多个测试用例）
- [x] 支持 `-o xxx.c` 选项输出 C99 代码
- [x] 支持 `--exec` 选项自动编译和链接
- [x] 支持 `--c99` 选项显式选择 C99 后端
- [x] 代码符合项目编码规范（已通过编译检查）
- [x] 核心功能文档完整（TODO文档已更新）
- [x] **保证 `tests/run_programs.sh` 核心测试用例通过**✅（全部通过，100%通过率）
