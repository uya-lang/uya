...
- [ ] **整数类型**：增加 `i8`、`i16`、`i64`、`u8`、`u16`、`u32`、`u64`（Lexer 无需改，Checker + Codegen 类型映射）
- [ ] **类型极值**：`max`、`min` 关键字（编译期常量），规范 uya.md §2、§16
...
| 待办 | Mini 现状 | 完整版目标 | 规范 |
|------|-----------|------------|------|
| 整数类型 | i32, usize, byte | + i8, i16, i64, u8, u16, u32, u64 | uya.md §2 |
| 类型极值 | 无 | max, min 关键字 | uya.md §2、§16 |
| 切片类型 | 无 | &[T], &[T: N] | uya.md §2、§4 |
| 元组类型 | 无 | (T1, T2,...), .0/.1, 解构 | uya.md §2 |
