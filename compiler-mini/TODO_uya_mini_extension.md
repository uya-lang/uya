...
**状态**：未实现（编译器实现）  
**规范状态**：✅ 已在 `spec/UYA_MINI_SPEC.md` 中定义（包括指针解引用运算符 `*expr`）  
**优先级**：P0（必需）

**实现状态说明**（基于源码分析）：
- ❌ `TypeKind` 枚举中没有 `TYPE_POINTER`
- ❌ AST 节点类型中没有指针类型节点
- ❌ 解析器没有实现指针类型解析（`&Type` 和 `*Type`）
- ❌ 类型检查器没有指针类型检查
- ❌ 代码生成器没有指针代码生成
- ❌ 取地址运算符 `&expr` 未实现（一元表达式只支持 `!` 和 `-`）
- ❌ 解引用运算符 `*expr` 未实现（一元表达式只支持 `!` 和 `-`）
...
**规范文档更新**（2026-01-11）：
- ✅ 已在 `spec/UYA_MINI_SPEC.md` 中添加指针解引用运算符 `*` 的语法定义
- ✅ 运算符列表、BNF 语法、运算符优先级均已更新
- ✅ 表达式说明中补充了指针解引用的详细说明
...
**状态**：未实现（编译器实现）  
**规范状态**：✅ 已在 `spec/UYA_MINI_SPEC.md` 中定义  
**优先级**：P0（必需）

**实现状态说明**（基于源码分析）：
- ❌ `TypeKind` 枚举中没有 `TYPE_ARRAY`
- ❌ AST 节点类型中没有数组相关节点
- ❌ 解析器没有实现数组类型解析
- ❌ 类型检查器没有数组类型检查
- ❌ 代码生成器没有数组代码生成
...
**状态**：未实现（编译器实现）  
**规范状态**：✅ 已在 `spec/UYA_MINI_SPEC.md` 中定义（包括 BNF 语法定义）  
**优先级**：P0（必需）

**规范文档更新**（2026-01-11）：
- ✅ 已在 `spec/UYA_MINI_SPEC.md` 中添加 `sizeof_expr` 的 BNF 语法定义
- ✅ 在表达式语法中正确集成 `sizeof` 语法

**实现状态说明**（基于源码分析）：
- ❌ AST 节点类型中没有 `sizeof` 相关节点
- ❌ 解析器没有实现 `sizeof` 解析
- ❌ 类型检查器没有 `sizeof` 类型检查
- ❌ 代码生成器没有 `sizeof` 代码生成
- 注意：源码中的 `sizeof` 是 C 语言的 `sizeof`，不是 Uya 语言的内置函数
...
- ✅ 添加了字符串字面量的词法规则定义（3.4 节）
- ✅ 在表达式语法中添加了字符串字面量（`STRING`）
- ✅ 添加了详细的说明和示例

**规范说明**（参考完整 Uya）：
- 字符串字面量类型：`*byte`（FFI 指针类型）
- 字符串字面量存储：编译器在只读数据段中存储（null 终止）
...
- `spec/UYA_MINI_SPEC.md` - Uya Mini 语言规范（✅ 已更新：2026-01-11 修复指针解引用和 sizeof 语法定义）
...
- **2026-01-11**：修复了 `spec/UYA_MINI_SPEC.md` 中的以下问题：
  - ✅ 添加了指针解引用运算符 `*expr` 的完整语法定义
  - ✅ 添加了 `sizeof` 的 BNF 语法定义（`sizeof_expr`）
  - ✅ 修复了章节编号问题（4.6 → 4.5）
  - ✅ 合并了重复的"说明"部分
  - ✅ 完善了指针解引用和类型推断的说明

**当前实现状态总结**（基于源码分析，2026-01-11）：

**已实现特性**（编译器代码中已实现）：
- ✅ 基础类型：`i32`、`bool`、`void`、`struct Name`
- ✅ 结构体定义、实例化和字段访问
- ✅ `extern` 函数声明（基础类型参数）
- ✅ 控制流：`if`、`while`、`return`
- ✅ 表达式：算术、逻辑、比较运算
...
**规范已定义但未实现特性**：
- ❌ 指针类型 `&T` 和 `*T`（规范已定义，编译器未实现）
- ❌ 数组类型 `[T: N]`（规范已定义，编译器未实现）
- ❌ `sizeof` 内置函数（规范已定义，编译器未实现）
- ❌ `for` 循环（规范已定义，编译器未实现）
- ❌ `break` 和 `continue` 语句（规范已定义，编译器未实现）
- ❌ 取地址运算符 `&expr`（规范已定义，编译器未实现）
- ❌ 解引用运算符 `*expr`（规范已定义，编译器未实现）
- ❌ `null` 字面量（规范已定义，编译器未实现）
- ❌ `byte` 类型（规范已定义，TypeKind 枚举中未包含）

**实现状态检查方法**：
- 检查 `src/checker.h` 中的 `TypeKind` 枚举
- 检查 `src/ast.h` 中的 `ASTNodeType` 枚举
- 检查 `src/lexer.h` 中的 `TokenType` 枚举
- 检查测试程序目录 `tests/programs/` 中的测试用例

**规范文档与实现状态的关系**：

- **规范状态**：指语法规范是否在 `spec/UYA_MINI_SPEC.md` 中定义
- **实现状态**：指编译器代码是否实现了该特性
- 规范可能已定义，但实现仍为"未实现"状态
