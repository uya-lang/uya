...
**修复任务**：
- [x] 在 `checker.h` 的 `TypeKind` 枚举中添加 `TYPE_USIZE`
- [x] 在 `checker.c` 的 `type_from_ast()` 函数中添加 `usize` 类型识别
- [x] 在 `codegen.c` 的 `codegen_get_base_type()` 函数中添加 `TYPE_USIZE` 支持
- [x] 在 `codegen.c` 的 `get_llvm_type_from_ast()` 函数中添加 `usize` 类型映射（根据目标平台）
- [x] 修改算术运算符类型检查规则（`checker_check_binary_expr()`），支持 `i32` 和 `usize` 混合运算
- [x] 修改算术运算符代码生成（`codegen_gen_expr()`），支持混合类型运算
- [x] 添加 `i32 ↔ usize` 类型转换支持（`as` 关键字）
- [ ] 添加 `usize` 类型相关测试用例
...
**状态**：✅ 已完成（2026-01-13，规范文档已更新）

---

### 3. ✅ 算术运算符类型规则不符合规范（P0 - 高优先级）【已完成】
...
**修复任务**：
- [x] 修改 `checker_check_binary_expr()` 函数，支持 `i32` 和 `usize` 混合运算
- [x] 实现结果类型推断规则：如果至少有一个是 `usize`，结果为 `usize`
- [x] 修改代码生成器，支持混合类型算术运算（需要类型提升）
- [ ] 添加混合类型运算测试用例
...
### 4. ✅ 指针大小硬编码（P1 - 中优先级）【已完成】

**问题描述**：
- 规范要求：指针大小应平台相关（32位平台=4字节，64位平台=8字节）
- **当前实现**：在 `codegen.c` 的 `sizeof` 实现中硬编码为 8 字节（第 1955-1964 行）
  ```c
  case LLVMPointerTypeKind:
      ...
      break;
  ```
- 同样的问题也出现在数组元素大小计算中（第 1976-1984 行）

**修复任务**：
- [x] 使用 LLVM TargetData API 获取指针大小（而不是硬编码）
- [x] 修改 `codegen.c` 中的 `sizeof` 实现，使用 `LLVMPointerSize()` 或类似 API
- [x] 修改数组元素大小计算中的指针大小处理
- [x] 移除硬编码 fallback，改为返回错误（因为 DataLayout 在代码生成前已设置）

**修复说明**：
- DataLayout 在 `codegen_generate()` 的第零步就已经设置（第3321-3376行）
- 在 `sizeof` 实现中，如果无法获取 TargetData，现在返回 NULL 而不是使用硬编码值
- 这与其他地方的错误处理方式一致（如结构体类型数组的处理）
- `codegen_get_base_type()` 中的 `usize` 类型处理保留了 fallback（默认64位），因为可能在边缘情况下有用
...
- [x] 验证结构体大小和对齐计算
- [x] 创建测试用例验证各种结构体布局场景
- [ ] 验证填充字节是否为 0（规范明确要求，需要更底层的测试）

**验证说明**：
- 已创建 `test_struct_layout.uya` 测试用例，验证：
  - 基本对齐（无填充）
  - 需要填充的结构体
  - 嵌套结构体布局
  - 数组字段布局
  ...
  - 平台差异（指针和 usize）
  - 空结构体
- 测试通过字段访问和 `sizeof`/`alignof` 来间接验证布局正确性
- 填充字节内容验证需要更底层的测试（检查生成的 LLVM IR 或二进制代码），当前测试通过字段访问间接验证布局正确性

**参考规范章节**：
- 2.3 结构体内存布局详细规则（第 120-277 行）

**状态**：✅ 已完成（2026-01-13，已创建测试用例）
...
- 2026-01-13：完成问题 1（usize 类型缺失）和问题 3（算术运算符类型规则）的实现
- 2026-01-13：完成问题 2（枚举类型支持不一致）- 规范文档已更新
- 2026-01-13：完成问题 4（指针大小硬编码）- 移除硬编码 fallback，改为返回错误
- 2026-01-13：完成问题 5（结构体内存布局验证）- 创建全面的测试用例 `test_struct_layout.uya`
