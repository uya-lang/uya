...
1. **翻译 Checker 模块**
   - 将 `src/checker.c/h` 翻译为 `uya-src/checker.uya`
   - 类型检查器依赖 Parser 生成的 AST
   - 代码量大（约1678行），逻辑复杂

**参考文档**：
- `BOOTSTRAP_PLAN.md` - 详细的自举实现计划
- `spec/UYA_MINI_SPEC.md` - Uya Mini 语言规范
- `TODO_phase5.md` - 阶段5的详细任务列表
- `src/checker.c/h` - C99 版本的 Checker 实现
- `uya-src/parser.uya` - 语法分析器模块（已完成）
- `uya-src/ast.uya` - AST 数据结构（已完成）
...
| 阶段4 | Parser | ✅ 已完成 | 2026-01-13 | 语法分析器（2741行） |
| 阶段5 | Checker | ⏳ 待开始 | - | 类型检查器（约1678行） |
