...
- **会话4（进行中）**：实现完整表达式解析（`parser_parse_expression()`，包括运算符、函数调用、结构体字面量和字段访问）
  - 🚧 待实现：扩展 `parser_parse_primary_expr()` - 添加函数调用、结构体字面量、字段访问解析
  - 🚧 待实现：实现一元表达式解析（`parser_parse_unary_expr()`）- 支持 `!` 和 `-` 运算符
  - 🚧 待实现：实现二元表达式解析（按优先级：mul_expr, add_expr, rel_expr, eq_expr, and_expr, or_expr）
  - 🚧 待实现：实现赋值表达式解析（`assign_expr`）
  - 🚧 待实现：添加测试用例并运行测试
