...
- **Uya Mini 规范扩展**（阶段10前置工作）- 实现支持自举所需的语言特性 🔄 进行中
  ...
  - 已完成的工作（2026-01-11 续）：
    ...
    - ✅ 步骤3：扩展类型检查器支持指针和数组类型检查（部分完成，2026-01-11 续）
      - ✅ 扩展 `type_from_ast()` 函数支持从 `AST_TYPE_POINTER` 和 `AST_TYPE_ARRAY` 节点创建 Type 结构
      - ✅ 支持递归处理嵌套类型（指针指向结构体、数组元素类型等）
      - ✅ 数组大小验证（必须是编译期常量数字字面量）
      - ✅ 所有现有测试通过
      - ⏳ `checker_infer_type()` 函数扩展（待步骤4完成后实现，因为需要取地址和解引用运算符支持）
  - 待实现特性（P0 优先级）：
    - ⏳ 步骤3：扩展类型检查器支持指针和数组类型检查
    - ⏳ 步骤4：实现取地址和解引用运算符（`&expr` 和 `*expr`）
    - ⏳ 步骤5：扩展代码生成器支持指针和数组类型代码生成
    - ⏳ 步骤6：实现 `sizeof` 内置函数
