...
- ✅ 创建 Uya 测试程序（15个测试用例）
  ...
  - ✅ `scope_shadowing.uya` - 变量遮蔽测试（不同作用域的同名变量）
...
**依赖关系**：所有模块已完成（Arena、AST、Lexer、Parser、Checker、CodeGen、Main）
...
- 扩展阶段9：添加更多测试用例（6个新测试用例）
- 创建 `scope_shadowing.uya` - 变量遮蔽测试（不同作用域的同名变量）
