...
**当前阶段**：阶段4 - 词法分析器（Lexer）

**下一步任务**：
1. 创建 `src/lexer.h` - 词法分析器头文件
2. 创建 `src/lexer.c` - 词法分析器实现
3. 实现 Token 类型定义（使用 Arena 分配）
4. 实现词法分析测试（TDD）

**依赖关系**：依赖 Arena 分配器（已完成）

**参考文档**：
- `spec/UYA_MINI_SPEC.md` - 语言规范（词法规则部分）
- `TODO.md` - 任务索引
- `TODO_phase4.md` - 阶段4详细任务列表
