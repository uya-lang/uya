...
1. **翻译 Parser 模块**
   - 将 `src/parser.c/h` 翻译为 `uya-src/parser.uya`
   - 语法分析器是 Checker 和 CodeGen 的基础依赖
   - 代码量最大（约2556行），逻辑复杂

**参考文档**：
- `BOOTSTRAP_PLAN.md` - 详细的自举实现计划
- `spec/UYA_MINI_SPEC.md` - Uya Mini 语言规范
- `TODO_phase4.md` - 阶段4的详细任务列表
- `src/parser.c/h` - C99 版本的 Parser 实现
- `uya-src/lexer.uya` - 词法分析器模块（已完成）
...
| 阶段3 | Lexer | ✅ 已完成 | 2026-01-13 | 词法分析器 |
