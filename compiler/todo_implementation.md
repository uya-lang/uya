...
- [x] 代码生成器：
  ...
  - [ ] 元组字面量初始化表达式的类型名称修复（当前使用临时名称 `tuple_N`，需要从变量声明类型中获取正确的类型名称）
- [x] 测试用例：
...
- ⚠️ 元组字面量类型名称：元组字面量在代码生成时仍使用临时名称 `tuple_N`（如 `tuple_2`），而不是正确的类型名称（如 `tuple_i32_bool`）。这导致生成的C代码无法编译，因为初始化表达式使用了不存在的类型名称。需要从变量声明的类型信息中获取正确的类型名称并用于元组字面量初始化表达式。
