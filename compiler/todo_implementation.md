...
- 元组类型实现进展：已完成 AST 节点定义、语法解析、IR 生成（字面量生成已完善为完整的结构体初始化）、代码生成和部分类型检查，测试用例已编写（`test_tuple_feature.uya` 和 `test_tuple_comprehensive.uya`）。已知问题：需要为每个元组类型生成唯一的结构体定义（当前使用临时名称 `tuple_N`），生成的C代码无法编译（缺少结构体定义）。已添加函数框架但需要完整实现。
...
- [x] IR 生成器：
  ...
  - [ ] 元组类型的结构体定义生成（`IR_STRUCT_DECL`）- 已添加函数框架（`generate_tuple_type_name` 和 `ensure_tuple_struct_decl`），需要完整实现
- [x] 代码生成器：
  - [x] 元组类型的 C 代码生成（使用结构体表示）
  - [x] 元组字段访问的代码生成（生成 `._0`, `._1` 形式的字段访问）
  - [ ] 元组结构体定义的代码生成（当前生成的C代码无法编译，缺少结构体定义，如 `tuple_2`、`tuple_3` 等）
- [x] 测试用例：
...
- ⚠️ 结构体定义生成：当前使用临时名称 `tuple_N`（N 为元素数量），需要为每个不同的元组类型生成唯一的结构体定义。生成的C代码无法编译，因为使用了 `tuple_2`、`tuple_3` 等结构体名称但缺少对应的结构体定义。需要实现：1) 从AST_TYPE_TUPLE生成唯一类型名称，2) 生成IR_STRUCT_DECL指令，3) 在代码生成时生成C结构体定义。这是一个较大的架构改动。
...
- ✅ 当前状态：
  ...
  - 代码生成部分仍有问题：match 表达式作为语句时，生成的代码中仍显示 "Unknown instruction type: 25"（IR_CONSTANT）
- ⚠️ 问题分析：
  - 逻辑上，`IR_CONSTANT (25)` 不应进入 `codegen_generate_inst` 的 default case
  - 但实际执行仍然进入了 `codegen_generate_inst` 的 default case
  - 可能是条件判断未生效，或存在其他代码路径
- 📝 建议的后续工作：
  - 添加调试输出，查看 `body_inst` 的实际类型值和是否为 NULL
  - 检查是否有其他代码路径也在处理这种情况
  - 或者采用更简单的策略：对于 match 表达式作为语句的情况，使用不同的代码生成路径
...
- [ ] 已知问题：
  - [ ] match 表达式作为语句时，代码生成仍有问题：IR_CONSTANT (25) 类型仍然进入了 codegen_generate_inst 的 default case（详见上面的"当前进展总结"部分）
- [ ] 测试用例：
  - [ ] 各种模式类型的测试
  - [ ] 模式匹配完整性检查
  - [ ] 验证生成的代码能正确编译和运行
...
16. ⚠️ 元组类型（AST 节点定义、语法解析、IR 生成已完善、类型检查部分实现、测试用例已编写，存在结构体定义生成问题需要修复）
17. ⚠️ match 表达式（AST 节点定义、语法解析和 IR 生成已实现，代码生成部分实现但存在已知问题：作为语句时 IR_CONSTANT 类型仍进入 default case，需要进一步调试）
...
5. **错误类型比较** ✅（已完成）
6. **枚举类型** ✅（已完成）
7. **元组类型** ⚠️（部分完成：IR 生成已完善，类型检查部分实现，测试用例已编写，存在结构体定义生成问题需要修复）
