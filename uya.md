...
- 关键字保留：
  ...
  export use
  ```
- **内置函数**：所有内置函数均以 `@` 开头，无需导入。包括：`@size_of(T)`、`@align_of(T)`、`@len(a)`（数组长度）、`@max`、`@min`（整数类型极值，类型由上下文推断）、`@file`（当前源文件名）、`@line`（当前源代码行号）。
...
| `@file` | `*byte` | 当前源文件名（编译期常量） |
| `@line` | `i32` | 当前源代码行号（编译期常量） |
...
5. **`@file`**
   - **功能**：`@file` 返回当前源文件的路径（字符串）
   - **位置**：编译器内置函数（以 `@` 开头），无需导入
   - **返回类型**：`*byte`（C 字符串指针，null 终止）
   - **使用场景**：调试、日志记录、错误报告
   - **示例**：
     ```uya
     extern fn printf(fmt: *byte, ...) i32;
     
     fn log_message(msg: *byte) void {
         printf("<%s:%d> %s\n" as *byte, @file, @line, msg);
     }
     ```
   - **常量表达式**：在编译期直接替换为字符串字面量
   - **零运行时保证**：编译器在代码生成时直接输出文件名字符串，无运行时开销
...
6. **`@line`**
   ...
   - **示例**：
     ...
     fn assert(condition: bool, msg: *byte) void {
         if !condition {
             printf("断言失败于 %s:%d: %s\n" as *byte, @file, @line, msg);
         }
     }
     ```
   - **常量表达式**：在编译期直接替换为整数字面量
   - **零运行时保证**：编译器在代码生成时直接输出行号，无运行时开销
   - **注意**：同一行的多个 `@line` 返回相同的行号
