...
- **参数列表即元组（在函数体内可当元组访问）**
  - **整份参数列表**在函数体内除按名称访问外，还可作为**一个元组**访问，而不仅是可变参数部分。例如：`fn f(x: i32, y: i32)` 内部可通过内置名（如 `@params`）得到类型为 `(i32, i32)` 的值，使用 `@params.0`、`@params.1` 或解构 `const (a, b) = @params` 等元组操作。
  - 参数的类型序列在类型论上等价于一个元组类型；命名参数与「按位置访问的元组」两种视图并存，便于转发、泛型、反射式用法，并与可变参数（见下）统一为同一套元组模型。

- **可变参数（类型安全 + C 互操作）**
  - 可变参数在类型上建模为**元组**：每个调用点的可变部分对应一个具体的元组类型，由编译器推断，实现调用端类型检查。
  - 声明侧可用对元组类型的泛型表达可变参数（如 `fn f[T: Tuple](fmt: *byte, args: T)`），或沿用 `...` 语法、在语义上绑定到元组类型。
  - **与 C 单一 ABI**：生成的可变参数函数在 ABI 层使用 C variadic 约定；函数入口用 `va_start` 等将 `...` 读入并组为元组供函数体使用；Uya 调用 Uya 时按元组传参、生成时拆为 C variadic 传参。C 可直接调用 Uya 导出的可变参数函数，无需额外包装。
  - 可选：对 printf 风格 API，由格式串推断可变参数元组类型，实现格式串与参数类型的一致性检查。
