...
- **参数列表即元组**（规范 0.34）：
  - 整份参数列表在函数体内除按名称访问外，还可作为元组访问（如通过 `@params` 得到 `(T1, T2, ...)`，使用 `.0`/`.1` 或解构）。
  - 参数的类型序列与元组类型等价；命名访问与按位置元组访问两种视图并存，与可变参数统一为同一套元组模型。
- **可变参数（类型安全 + C 互操作）**：
  - 可变参数在类型上建模为元组；与 C 单一 ABI，C 可直接调用 Uya 导出的可变参数函数；可选由格式串推断可变参数元组类型。
- **字符串插值与 printf 结合**：
