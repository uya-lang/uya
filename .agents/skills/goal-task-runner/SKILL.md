...
归档时保留原始任务文本、验证命令、失败原因和缩进层级。若只移动嵌套叶子或子树，归档中要保留最近标题和必要的父级任务路径作为普通文本上下文，避免孤立缩进。若归档文件不存在，创建最小头部，写明来源 todo、整理日期和说明。优先把任务追加到归档文件中匹配的章节；找不到合适章节时追加到通用的 `## 归档任务` 章节。移动后主 todo 中不要残留同一任务的 `[x]` / `[f]` 复本；如需保留历史定位，只留下简短的非 checkbox 索引或说明。
