...
- [f] `MIR-C99-FULL-SUPPORT-GENERIC-COREBODY-LOWERING`: 将 full-language 主路径从
  ...
  - 当前状态（2026-06-28）：`MIR-C99-FULL-SUPPORT-GENERIC-COREBODY-CLI-ENTRY`
    与 main-language varargs capability baseline 已完成；父项仍未完成，因为
    `src/main.uya --mir-c99` 当前 fail-closed 到
    `usize_from_ptr_requires_target_capability`，尚未写出完整 `main.c`。
  ...
  - 后续重开条件：补齐或显式分桶 `usize_from_ptr` target capability 后，重跑
...
- [f] `MIR-C99-FULL-SUPPORT-ASYNC-REAL-CLI-BASELINE-ROUTE`: 确认按本轮硬约束必须使用的固定 `../uya/bin/uya build --mir-c99` 已进入真实 MIR-C99 CLI 路由，再继续 async real-CLI shard 切换。
  ...
  - 当前状态（2026-06-28）：HelloWorld 与基础 async case 均已进入真实 `[MIR-C99]`
    路由，不再落回 legacy C99；但基础 async case 当前 fail-closed 到
    `AST_CATCH_EXPR / catch_return_not_lowered`，尚未写出可 host-C parity 的 MIR-C99 C 输出，因此
    async real-CLI 父项仍不能标记完成。
...
- [f] `MIR-C99-FULL-SUPPORT-CLI-SUITE-DISTINCT-OUTPUTS`: 让 `src/main.uya` 输出进入
  ...
  - 当前状态（2026-06-28）：build-entry recovery、HelloWorld real route 与 varargs capability
    baseline 已收敛；重跑 `bash tests/verify_mir_c99_cli_distinct_outputs.sh` 仍失败，手动探针确认
    `src/main.uya` 已进入 `[MIR-C99]`，但 fail-closed 到 `usize_from_ptr` target capability，
    尚未写出 distinct `main.c`。
  - 阻塞命令：
    - `bash tests/verify_mir_c99_cli_distinct_outputs.sh`
    - `UYA_ROOT="$PWD/lib/" ../uya/bin/uya build --mir-c99 src/main.uya -o <tmp>/main.c`
  - 关键错误：
    - `mir_c99_capability_diagnostic: kind=AST_USIZE_FROM_PTR reason=usize_from_ptr_requires_target_capability file=.../lib/std/runtime/runtime.uya line=60`
  - 后续重开条件：
...
- [f] `MIR-C99-FULL-SUPPORT-CLI-SUITE-SRC-MAIN-DISTINCT-OUTPUTS`: 让 `src/main.uya`
  ...
  - 当前状态（2026-06-28）：fixed/current-source build entry、HelloWorld real route 与
    varargs capability baseline 已恢复；`src/main.uya` 现在进入 `[MIR-C99]` 后 fail-closed 到
    `usize_from_ptr_requires_target_capability`，不再卡在 current-source helper/bootstrap
    构建、legacy C99 路由或 extern varargs 声明 bucket，但仍没有生成 distinct MIR-C99
    `main.c`。
  - 阻塞命令：
    - `bash tests/verify_mir_c99_cli_distinct_outputs.sh`：失败；脚本未打印日志。
    - `UYA_ROOT="$PWD/lib/" ../uya/bin/uya build --mir-c99 src/main.uya -o <tmp>/main.c`：
      退出码 1，日志包含 `[MIR-C99]` 和 `usize_from_ptr` capability diagnostic，未生成输出 C。
  - 关键错误：
    - `mir_c99_capability_diagnostic: kind=AST_USIZE_FROM_PTR reason=usize_from_ptr_requires_target_capability file=.../lib/std/runtime/runtime.uya line=60`
  - 后续重开条件：补齐或显式分桶 `usize_from_ptr` target capability 后，重跑
