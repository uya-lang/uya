...
- [f] `MIR-C99-FULL-SUPPORT-TEST-HARNESS-AND-NO-MAIN`: 修复 test 文件、无 main 文件和
  ...
  - 失败原因（2026-06-24）：新增 focused gate
    ...
    诊断 gate。
  - 当前状态（2026-06-27）：build-entry recovery 和 HelloWorld real route 已收敛；重跑
    `bash tests/verify_mir_c99_test_harness_and_no_main_real_cli.sh` 后，脚本已能越过
    bootstrap/helper 构建阶段，但仍失败于 no-main/test harness 输出策略。
  - 阻塞命令：`bash tests/verify_mir_c99_test_harness_and_no_main_real_cli.sh`
  - 关键错误：`error: output still contains fake MIR-C99 main wrapper: <tmp>/hello.c`
  - 额外证据：临时 wrapper 切换后，
  ...
  - 后续重开条件：保留 current-source `cmd/build` candidate 构建通过的前提下，修复真实
    MIR-C99 输出中的 fake `int main(void) { return uya_mir_fn_N(); }` wrapper；随后重跑
    `bash tests/verify_mir_c99_test_harness_and_no_main_real_cli.sh`，要求 test-only、
    helper-only、extern-only 和 declaration-only case 给出正确 harness 或稳定 diagnostic。
