...
- MIR-C99-built compiler 复跑 compiler regression、C99 output parity 和 full-language backend parity：仍需真实 MIR-C99-built compiler 复跑 regression/parity；当前 `mir_c99_unit_output` 只推进到最小 build smoke，不能证明完整 parity。
...
已部分重开进展（2026-06-15）：`tracked_cmd_build_seed` 已去除，`bash tests/verify_mir_c99_cmd_build_parity_frontier_gate.sh` 通过，证明 candidate 能接受最小 build smoke、产出并运行 exit 0 程序；但 log/summary 仍记录 `compiler_regression_status=not_yet_run`、`c99_output_parity_status=not_yet_run`、`full_language_backend_parity_status=not_yet_run`，所以本失败项不能移入 completed。
