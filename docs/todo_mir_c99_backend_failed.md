...
- [f] `MIR-C99-FULL-SUPPORT-GENERIC-COREBODY-LOWERING`: 将 full-language 主路径从
  ...
  - 当前状态（2026-06-28）：`MIR-C99-FULL-SUPPORT-GENERIC-COREBODY-CLI-ENTRY`、
    main-language varargs capability baseline 与 launcher ptr-diff frontier 已完成；
    父项仍未完成，因为 `src/main.uya --mir-c99` 当前 fail-closed 到
    `call_expr_requires_call_lowering`，尚未写出完整 `main.c`。
  ...
  - 后续重开条件：补齐或显式分桶 `call_expr` / runtime helper call lowering 后，重跑
...
- [f] `MIR-C99-FULL-SUPPORT-CLI-SUITE-DISTINCT-OUTPUTS`: 让 `src/main.uya` 输出进入
  ...
  - 当前状态（2026-06-28）：build-entry recovery、HelloWorld real route、varargs capability
    baseline 与 launcher ptr-diff frontier 已收敛；重跑
    `bash tests/verify_mir_c99_cli_distinct_outputs.sh` 仍失败，手动探针确认
    `src/main.uya` 已进入 `[MIR-C99]`，但 fail-closed 到
    `call_expr_requires_call_lowering`，尚未写出 distinct `main.c`。
  - 阻塞命令：
...
- [f] `MIR-C99-FULL-SUPPORT-CLI-SUITE-SRC-MAIN-DISTINCT-OUTPUTS`: 让 `src/main.uya`
  ...
  - 当前状态（2026-06-28）：fixed/current-source build entry、HelloWorld real route、
    varargs capability baseline 与 launcher ptr-diff frontier 已恢复；`src/main.uya`
    现在进入 `[MIR-C99]` 后 fail-closed 到
    `call_expr_requires_call_lowering`，不再卡在 current-source helper/bootstrap 构建、
    legacy C99 路由、extern varargs 声明 bucket 或未引用 `ptr_diff` helper，但仍没有生成
    distinct MIR-C99 `main.c`。
  - 阻塞命令：
    - `bash tests/verify_mir_c99_cli_distinct_outputs.sh`：失败；脚本未打印日志。
    - `UYA_ROOT="$PWD/lib/" ../uya/bin/uya build --mir-c99 src/main.uya -o <tmp>/main.c`：
      退出码 1，日志包含 `[MIR-C99]` 和 `call_expr` capability diagnostic，未生成输出 C。
  - 关键错误：
    - `mir_c99_capability_diagnostic: kind=AST_CALL_EXPR reason=call_expr_requires_call_lowering file=.../src/main.uya line=218`
  - 后续重开条件：补齐或显式分桶 `call_expr` / runtime helper call lowering 后，重跑
