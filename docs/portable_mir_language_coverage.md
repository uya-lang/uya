...
| MIR-C99 self-build | partial | `cmd/build` release baseline 现只认两类事实：没有静默 fallback / 静默成功，以及当前阻塞是否被归入通用 capability category。当前基线由 `bash tests/verify_mir_c99_self_build_convergence_audit.sh` 与 `bash tests/verify_mir_c99_cmd_build_host_binary_attempt_gate.sh` 固定记录 `self_build_convergence_status=real_compiler_candidate`、`host_compiler_binary_candidate_role=compiler_binary` 与 `blocked_category_count=4`；blocked categories 当前为 call ABI、runtime helper、emitter/output、link/absence。`native_hosted_handoff_frontier`、`native_build_type_named_equals`、`native_hosted_executable_writer_preflight` 等 frontier samples，以及 helper 名、statement count、`completed_body_detail`、`next_coverage` 仅保留为 diagnostic context，不再定义 active leaf、stage gate 或完成。helper-frontier 历史样本（含 `native_build_type_named_equals` / `generic_corebody_type_named_equals_body_lowering`）只用于保留 summary frontier 曾阻塞在 helper lowering 的历史事实；下一步应推动 MIR-C99-built compiler 进入真实 self-build、compiler regression 和 C99 output parity，而不是继续按 helper 样本枚举 frontier。 |
