...
| `AST_CATCH_EXPR` | done | partial | MIR-C99 full-language error union catch parity shard 覆盖 `maybe_argc(argc) catch { ... }` 的 success/error 分支；error-id binding parity shard 覆盖 `catch |err|` 绑定；`bash tests/verify_mir_c99_full_language_unit_output_first_bucket.sh` 现要求真实 fixed CLI 对 `catch { return ...; }` fail-closed，输出 `mir_c99_capability_diagnostic: kind=AST_CATCH_EXPR reason=catch_return_not_lowered`。 |
