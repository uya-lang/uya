...
| MIR-C99 self-build | partial | `cmd/build` real compiler candidate preflight 记录真实 compiler-source handoff 诊断：`native_hosted_handoff_frontier` / `pending_core_bodies`，归因到 `mir_instruction_coverage`；其中 `frontier_sample_1=native_hosted_handoff_frontier` 仅作为 diagnostic-only handoff 样本，用来保留 summary executable -> real compiler candidate 的历史上下文，不再定义 active leaf；`compile_stats_record_and_release_typed_program_stmt10_table_items`、`compile_stats_record_and_release_typed_program_stmt11_table_capacity`、`compile_stats_record_and_release_typed_program_stmt12_table_used_bytes`、`compile_stats_record_and_release_typed_program_stmt13_table_capacity_bytes`、`compile_stats_record_and_release_typed_program_stmt14_table_realloc_count`、`compile_stats_record_and_release_typed_program_stmt15_typed_program_release`、`compile_stats_record_and_release_typed_program_stmt16_typed_type_records_release`、`compile_stats_record_and_release_typed_program_stmt17_typed_program_released_bytes`、`compiler_should_profile_diagnostics_first_slice`、`compiler_should_profile_diagnostics_null_empty_branch`、`compiler_should_profile_diagnostics_false_like_branch`、`compiler_should_profile_diagnostics_tail_return`、`compiler_print_diagnostic_profile_guard`、`compiler_print_diagnostic_profile_count`、`compiler_print_diagnostic_profile_checker_branch`、`compiler_print_diagnostic_profile_tail_fprintf`、`build_driver_run_first_slice`、`build_driver_run_parse_prefix`、`build_driver_run_stack_init`、`build_driver_run_stack_guard`、`build_driver_run_stack_limit_call`、`build_driver_run_split_env`、`build_driver_run_output_path`、`build_driver_run_user_output_path`、`build_driver_run_explicit_output_path`、`build_driver_run_llvm_backend_c99_rewrite`、`build_driver_run_split_c_default`、`build_driver_run_output_path_for_compile`、`build_driver_run_output_path_selection`、`build_driver_run_split_c_arg`、`build_driver_run_split_c_arg_assign`、`build_driver_run_artifacts`、`build_driver_run_split_c_lock`、`build_driver_run_split_c_lock_defer`、`build_driver_run_split_c_lock_acquire`、`build_driver_run_compile_result`、`build_driver_run_result_error`、`build_driver_run_native_success`、`build_driver_run_is_output_c_file`、`build_driver_run_c_output_check`、`build_driver_run_link_output`、`build_driver_run_final_return`、`native_build_decl_is_extern_two_i32_param_fn`、`generic_corebody_guard_call_tail_return_lowering`、`generic_corebody_pointer_param_guard_tail_return_lowering`、`generic_corebody_two_pointer_param_guard_tail_return_lowering`、`generic_corebody_parse11_pointer_out_param_lowering`、`generic_corebody_empty_struct_return_lowering`、`generic_corebody_reachability_empty_struct_return_lowering`、`generic_corebody_local_table_empty_struct_return_lowering`、`generic_corebody_const_slice_sum_shape_empty_struct_return_lowering`、`generic_corebody_const_slice_len_index_shape_empty_struct_return_lowering`、`generic_corebody_pointer_deref_shape_empty_struct_return_lowering`、`generic_corebody_dynamic_catch_shape_empty_struct_return_lowering`、`generic_corebody_defer_local_assign_shape_empty_struct_return_lowering`、`generic_corebody_lexical_drop_shape_empty_struct_return_lowering`、`generic_corebody_interface_method_shape_empty_struct_return_lowering`、`generic_corebody_direct_method_shape_empty_struct_return_lowering`、`generic_corebody_struct_union_enum_shape_empty_struct_return_lowering`、`generic_corebody_atomic_i32_shape_empty_struct_return_lowering`、`generic_corebody_simd_vector_mask_shape_empty_struct_return_lowering`、`generic_corebody_local_table_init_body_lowering`、`generic_corebody_reachability_init_body_lowering`、`generic_corebody_type_is_i32_body_lowering` 和 `generic_corebody_type_is_usize_body_lowering` 已纳入 CoreBody -> PortableMIR lowering；通用 lowering 证据覆盖 if guard / 短路 OR、field load、array index、const local、resolved helper call 和 tail call return，并新增 parse11 call、pointer out-param 写回和嵌套 struct literal empty return；复用 shape-based materializer 覆盖 parse11 helper 与 empty struct return helper；不得新增按 helper 名称或固定 7-stmt body shape 命中的 materializer；当前 completed body detail 包含 `native_build_type_is_usize` body-complete，下一处 frontier 为 `native_build_type_named_equals`/`generic_corebody_type_named_equals_body_lowering`；2026-06-14 复盘后该 frontier 仅作为冻结样本，不再作为下一执行叶子；convergence audit 现固定输出 `status=real_compiler_candidate`、host binary candidate role `compiler_binary`、`pending_core_bodies=3511`、frontier samples（`native_hosted_handoff_frontier`、`native_build_type_named_equals`、`native_hosted_executable_writer_preflight`）和 blocked categories（call ABI、runtime helper、emitter/output、link/absence）；4.16 stage gate 固定只认三类收敛指标：候选状态从 summary executable 切换到 real compiler candidate、`blocked_category_count`/`blocked_category_*` 减少、以及 host C compiler 编译出的候选通过 `cmd/build --help` 或等价 compiler smoke。helper 名、frontier 样本名、statement count、`completed_body_detail` 和 `next_coverage` 只保留为诊断上下文，不再定义完成。当前真实 compiler candidate C 由 `mir_c99_unit_output` 提供，默认 generator 对 `src/cmd/build/main.uya` 写出带 `generated by MIR-C99 unit output writer` 标记和 `uya_mir_func_` 符号的候选 C，经 host C compiler 编译后可通过 `cmd/build --help` smoke；下一步让 MIR-C99-built compiler 进入真实 self-build、compiler regression 和 C99 output parity。 |
