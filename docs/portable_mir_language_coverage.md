...
| `AST_TYPE_ATOMIC` | done | partial | full-language backend hard parity 覆盖 `atomic i32` init/read/compound add native executable parity；完整 portable atomic helper / target capability 矩阵仍待后续扩展。 |
| `AST_TYPE_VECTOR` | done | partial | full-language backend hard parity 覆盖 `@vector(i32, 4)` splat、compare、select 和 reduce_add native executable parity；完整 SIMD vector capability 矩阵仍待后续扩展。 |
| `AST_TYPE_MASK` | done | partial | full-language backend hard parity 覆盖 `@mask(4)` compare/select native executable parity；完整 SIMD mask capability 矩阵仍待后续扩展。 |
...
| `CORE_EXPR_KIND_ATOMIC` | done | partial | full-language backend hard parity 覆盖 atomic compound-add native executable parity；完整 atomic opcode/helper 矩阵仍待后续扩展。 |
| `CORE_EXPR_KIND_VECTOR` | done | partial | full-language backend hard parity 覆盖 vector splat/compare/select/reduce_add native executable parity；完整 SIMD opcode/helper 矩阵仍待后续扩展。 |
| `CORE_EXPR_KIND_MASK` | done | partial | full-language backend hard parity 覆盖 mask compare/select native executable parity；完整 mask opcode/helper 矩阵仍待后续扩展。 |
