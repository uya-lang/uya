...
| `CORE_STMT_KIND_WHILE` | done | partial | PortableMIR CoreBody structured CFG shard 覆盖 condition `COND_BR`、body block 和 loop backedge；MIR-C99 full-language return/local/binary/branch/loop parity shard 覆盖 loop backedge；control-flow async full-language parity shard 覆盖 async while；break/continue 和复杂 cleanup edge 待补。 |
