...
### 2026-06-27 - Full Language Parity / Baseline Truth

父级路径：无父级 checkbox

- [x] `MIR-C99-FULL-SUPPORT-BASELINE-TRUTH`: 重新固定 full-language 口径下的真实基线，不把
  ...
  - 验收：新增或更新一个 fail-closed 基线门禁，显式区分 `--mir-c99`、hosted native、
    fixture generator 和 legacy C99；该门禁必须复现当前真实失败面，不允许静默成功。
  - 完成说明（2026-06-27）：固定 `../uya/bin/uya` 路由已恢复到 current launcher +
    current `cmd/build`；`lib/libc/stdio.uya` 的 `fopen` 改为使用本地 `_FOPEN_*`
    常量，避免旧 bootstrap C99 generator 输出裸 `O_RDONLY` / `O_RDWR` / `S_IRWXU`
    后无法重建 current `cmd/build`；`src/build_compiler_driver.uya` 的 extern-signature
    pointer type lowering 改为 opaque pointee，避免 `&FILE` 等自引用 pointee 在
    MIR-C99 extern signature metadata 中递归爆栈。
  - 当前真实基线：HelloWorld 真实进入 `[MIR-C99]` 并输出
    `generated by MIR-C99 unit output writer`；`src/main.uya` fail-closed 到
    `mir_c99_capability_diagnostic: kind=AST_FN_DECL reason=extern_varargs_requires_c_variadic_capability`；
    `tests/extern_function.uya` fail-closed 到
    `mir_c99_capability_diagnostic: kind=AST_SYSCALL reason=syscall_requires_target_capability`。
  - 验证：
    - `UYA_ROOT="$PWD" bin/cmd/build build src/cmd/build/main.uya -o /tmp/cmd-build-local3 --no-split-c --project-root src/`：通过，生成 current `cmd/build`。
    - `../uya/bin/uya build --mir-c99 src/main.uya -o /tmp/uya-src-main-mir-after.c`：失败但稳定 fail-closed 到 `extern_varargs_requires_c_variadic_capability`，不再段错误。
    - `../uya/bin/uya build --mir-c99 tests/extern_function.uya -o /tmp/uya-extern-function-mir-after.c`：失败但稳定 fail-closed 到 `syscall_requires_target_capability`。
    ...
    - `bash tests/verify_mir_c99_full_language_baseline_truth.sh`：通过，输出
      `baseline_mir_c99_helloworld=pass`、
      `baseline_mir_c99_src_main=fail_closed:extern_varargs_requires_c_variadic_capability`、
