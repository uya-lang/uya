...
- [x] 锁定同步 sink 观察 stopped direct child；当前无 job-control 模式将 stop 收敛为终端恢复、整组强制取消、reap 和 `error.Interrupted`。
  - 验证：确认 `docs/typed_pipeline_design.md` L684 已含阶段 0 已锁定段落，L846 checklist 已补 `> **阶段 0 已锁定**：` 标记；文本覆盖 terminal 恢复、group/PID 强制取消、reap 与 `error.Interrupted` 返回。
...
- [x] 锁定按 terminal identity 串行化的 interruptible foreground lease；所有返回路径都先恢复终端再释放 lease，避免并发 sink 的 `tcsetpgrp` 竞态。
  验证（2026-07-10）：
  - `grep -n "foreground lease\|tcsetpgrp\|terminal identity\|先恢复终端再释放 lease" docs/typed_pipeline_design.md` 命中 L649、L657、L682、L845。
...
## 类型化管道 TODO / 阶段 1：Lexer 与 Parser 骨架

- [x] 确保 `|>` 在单字符 `|` 和 `>` 之前识别。

代码改动：
- `src/lexer.uya` 已在 `c == 124`（`|`）分支中优先检查下一个字符是否为 `>`（62），是则返回 `TOKEN_PIPE_GT`；再检查 `||`；最后回退到 `TOKEN_PIPE`。`>` 单字符分支（`c == 62`）不会匹配首字符为 `|` 的输入，因此 `|>` 必然在单字符 `|` 和 `>` 之前识别。

验证命令：
```bash
cat > /tmp/test_pipe_gt.uya <<'SRC'
fn main() i32 {
    x |> y;
    return 0;
}
SRC
./bin/uya check /tmp/test_pipe_gt.uya
```

验证结果：
```
错误: 语法分析失败 (/tmp/test_pipe_gt.uya:2:7): 意外的 token '|>'
错误: 语法分析失败 (/tmp/test_pipe_gt.uya:2:7): 意外的 token '|>'
错误: 收集模块依赖失败: /tmp/test_pipe_gt.uya
```
- parser 报告“意外的 token '|>'”，证明 lexer 将 `|>` 识别为单一 token，而非 `|` 与 `>` 两个独立 token。

补充验证（确保 `|` 与 `||` 未被破坏）：
```bash
cat > /tmp/test_pipe_single.uya <<'SRC'
fn main() i32 {
    var a: i32 = 1;
    var b: i32 = 2;
    var c: i32 = a | b;
...
SRC
./bin/uya run /tmp/test_pipe_single.uya
```
