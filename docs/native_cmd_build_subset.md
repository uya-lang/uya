...
- `cmd/build` 当前失败卡点必须继续由 `tests/verify_native_cmd_build_no_silent_c99.sh` 固定；失败时必须保留
  ...
  `native_hosted_entry_child_frontier: first_pending_callee=build_compiler_driver_run parent_stmt=37 child_prefix=1 child_prefix_stmts=3 child_next_stmt=3 child_next_kind=var`、
...
nested frontier 是链接输出分支内部的 `link_result` 初始化；更大的真实门槛仍是把 `mir_body_functions=3`
