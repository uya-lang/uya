...
|----|-------------|------|----------|
| F01 | partial | 已有 nostdlib `_start`、syscall bridge 和最小 executable；build-only native writer 已从 Linux 初始栈读取 `argc` 并支持 `get_argc()` 最小 lowering；仍缺 `argv`、环境变量和完整 CLI runtime 接入 | native 程序入口传递 argv/env，并接入 build 参数解析 |
...
下一项继续按缺口补 native `cmd/build` 子集能力：当前只有 build CLI 的极小函数输出路径；`cmd/build` 自身已经进入 reachable lowering，并已支持无副作用局部声明占位、zero-arg / one-i32-constant-arg / two-i32-constant-arg direct call 局部初始化、`get_argc()` 读取、`&local` / `&array[0]` out-param 写回、两个 `&i32` out-param 写回和 `return local` / `return array[0]`。下一道真实门槛是支持 `parse_build_args(...)` 这种 11 参数调用、`get_argv` / 字符串比较、更多 out-param 写回、条件分支和后续 CLI 控制流。完成这些之前，不能声明已经生成 native `bin/cmd/build`。
