...
## `native_build_const_slice_len_index_shape_empty()` Body Complete Contract

`native_build_const_slice_sum_shape_empty()` body complete 后，当前真实 pending frontier 是：

```text
native_hosted_pending_body_frontier: function=native_build_const_slice_len_index_shape_empty decl=315 function_id=13 body_stmts=1 reason=pending_core_body
```

函数源码：

```text
fn native_build_const_slice_len_index_shape_empty() NativeBuildConstSliceLenIndexShape {
    return NativeBuildConstSliceLenIndexShape{
        array_decl: null,
        slice_decl: null,
        return_stmt: null,
        add_expr: null,
...
1. CoreIR 必须生成单条 `CORE_STMT_KIND_RETURN`，返回表达式保持
   `NativeBuildConstSliceLenIndexShape` struct literal surface。
2. struct literal 的字段必须保持源码顺序和语义：`array_decl = null`、`slice_decl = null`、
   `return_stmt = null`、`add_expr = null`、`len_expr = null`、`index_access = null`、
   `slice_start = 0`、`slice_len = 0`、`len_value = 0`、`indexed_value = 0`、`sum_value = 0`；
   不得省略任一 AST 指针字段，也不得把任一 i32 字段改成非零。
3. PortableMIR 必须保留 aggregate return surface，返回类型为
   `NativeBuildConstSliceLenIndexShape`，不得把该 helper 降成 noop 或 pending body。
4. 该切片迁入后 `native_build_const_slice_len_index_shape_empty()` 必须达到 body complete：
...
## `native_build_defer_local_assign_shape_empty()` Body Complete Contract

`native_build_dynamic_catch_shape_empty()` body complete 后，当前真实 pending frontier 是：

```text
native_hosted_pending_body_frontier: function=native_build_defer_local_assign_shape_empty decl=321 function_id=16 body_stmts=1 reason=pending_core_body
```
...
1. CoreIR 必须生成单条 `CORE_STMT_KIND_RETURN`，返回表达式保持
   `NativeBuildDeferLocalAssignShape` struct literal surface。
2. struct literal 的字段必须保持源码顺序和语义：`local_decl = null`、
   `defer_stmt = null`、`assign_stmt = null`、`return_stmt = null`、
   `initial_value = 0`、`deferred_value = 0`、`return_value = 0`、
   `return_is_local_ref = 0`；不得省略任一 AST 指针字段，也不得把任一 i32
   字段改成非零。
3. PortableMIR 必须保留 aggregate return surface，返回类型为
   `NativeBuildDeferLocalAssignShape`，不得把该 helper 降成 noop 或 pending body。
4. 该切片迁入后 `native_build_defer_local_assign_shape_empty()` 必须达到 body complete：
   `native_hosted_reachable_body_complete: function=native_build_defer_local_assign_shape_empty prefix_stmts=1 reason=body_complete`。
   下一步必须重新读取真实 self-build frontier。

## `native_build_lexical_drop_shape_empty()` Body Complete Contract

`native_build_defer_local_assign_shape_empty()` body complete 后，当前真实 pending frontier 是：

```text
native_hosted_pending_body_frontier: function=native_build_lexical_drop_shape_empty decl=322 function_id=17 body_stmts=1 reason=pending_core_body
```
...
1. CoreIR 必须生成单条 `CORE_STMT_KIND_RETURN`，返回表达式保持
   `NativeBuildLexicalDropShape` struct literal surface。
2. struct literal 的字段必须保持源码顺序和语义：`reset_stmt = null`、
   `drop_block = null`、`dropped_decl = null`、`return_stmt = null`、
   `reset_value = 0`、`field_value = 0`、`return_value = 0`；不得省略任一
   AST 指针字段，也不得把任一 i32 字段改成非零。
3. PortableMIR 必须保留 aggregate return surface，返回类型为
   `NativeBuildLexicalDropShape`，不得把该 helper 降成 noop 或 pending body。
4. 该切片迁入后 `native_build_lexical_drop_shape_empty()` 必须达到 body complete：
   `native_hosted_reachable_body_complete: function=native_build_lexical_drop_shape_empty prefix_stmts=1 reason=body_complete`。
   下一步必须重新读取真实 self-build frontier。

## `native_build_interface_method_shape_empty()` Body Complete Contract

`native_build_lexical_drop_shape_empty()` body complete 后，当前真实 pending frontier 是：

```text
native_hosted_pending_body_frontier: function=native_build_interface_method_shape_empty decl=323 function_id=18 body_stmts=1 reason=pending_core_body
```
...
1. CoreIR 必须生成单条 `CORE_STMT_KIND_RETURN`，返回表达式保持
   `NativeBuildInterfaceMethodShape` struct literal surface。
2. struct literal 的字段必须保持源码顺序和语义：`receiver_decl = null`、
   `direct_decl = null`、`interface_decl = null`、`return_stmt = null`、
   `direct_call_expr = null`、`interface_call_expr = null`、`field_value = 0`、
   `direct_value = 0`、`interface_value = 0`、`return_value = 0`；不得省略任一
   AST 指针字段，也不得把任一 i32 字段改成非零。
3. PortableMIR 必须保留 aggregate return surface，返回类型为
   `NativeBuildInterfaceMethodShape`，不得把该 helper 降成 noop 或 pending body。
4. 该切片迁入后 `native_build_interface_method_shape_empty()` 必须达到 body complete：
   `native_hosted_reachable_body_complete: function=native_build_interface_method_shape_empty prefix_stmts=1 reason=body_complete`。
   下一步必须重新读取真实 self-build frontier。

## `native_build_direct_method_shape_empty()` Body Complete Contract

`native_build_interface_method_shape_empty()` body complete 后，当前真实 pending frontier 是：

```text
native_hosted_pending_body_frontier: function=native_build_direct_method_shape_empty decl=324 function_id=19 body_stmts=1 reason=pending_core_body
```

函数源码：

```text
fn native_build_direct_method_shape_empty() NativeBuildDirectMethodShape {
    return NativeBuildDirectMethodShape{
        receiver_decl: null,
        return_stmt: null,
        method_call_expr: null,
        field_value: 0,
        return_value: 0,
    };
}
...
1. CoreIR 必须生成单条 `CORE_STMT_KIND_RETURN`，返回表达式保持
   `NativeBuildDirectMethodShape` struct literal surface。
2. struct literal 的字段必须保持源码顺序和语义：`receiver_decl = null`、
   `return_stmt = null`、`method_call_expr = null`、`field_value = 0`、
   `return_value = 0`；不得省略任一 AST 指针字段，也不得把任一 i32
   字段改成非零。
3. PortableMIR 必须保留 aggregate return surface，返回类型为
   `NativeBuildDirectMethodShape`，不得把该 helper 降成 noop 或 pending body。
4. 该切片迁入后 `native_build_direct_method_shape_empty()` 必须达到 body complete：
   `native_hosted_reachable_body_complete: function=native_build_direct_method_shape_empty prefix_stmts=1 reason=body_complete`。
   下一步必须重新读取真实 self-build frontier。

## `native_build_struct_union_enum_shape_empty()` Body Complete Contract

`native_build_direct_method_shape_empty()` body complete 后，当前真实 pending frontier 是：

```text
native_hosted_pending_body_frontier: function=native_build_struct_union_enum_shape_empty decl=325 function_id=20 body_stmts=1 reason=pending_core_body
```

函数源码：

```text
fn native_build_struct_union_enum_shape_empty() NativeBuildStructUnionEnumShape {
    return NativeBuildStructUnionEnumShape{
        struct_decl: null,
        union_decl: null,
        match_decl: null,
        return_stmt: null,
        struct_left_value: 0,
        struct_right_value: 0,
        union_value: 0,
        match_value: 0,
        return_value: 0,
    };
...
1. CoreIR 必须生成单条 `CORE_STMT_KIND_RETURN`，返回表达式保持
   `NativeBuildStructUnionEnumShape` struct literal surface。
2. struct literal 的字段必须保持源码顺序和语义：`struct_decl = null`、
   `union_decl = null`、`match_decl = null`、`return_stmt = null`、
   `struct_left_value = 0`、`struct_right_value = 0`、`union_value = 0`、
   `match_value = 0`、`return_value = 0`；不得省略任一 AST 指针字段，也不得把任一
   i32 字段改成非零。
3. PortableMIR 必须保留 aggregate return surface，返回类型为
   `NativeBuildStructUnionEnumShape`，不得把该 helper 降成 noop 或 pending body。
4. 该切片迁入后 `native_build_struct_union_enum_shape_empty()` 必须达到 body complete：
   `native_hosted_reachable_body_complete: function=native_build_struct_union_enum_shape_empty prefix_stmts=1 reason=body_complete`。
   下一步必须重新读取真实 self-build frontier。

## `native_build_atomic_i32_shape_empty()` Body Complete Contract

`native_build_struct_union_enum_shape_empty()` body complete 后，当前真实 pending frontier 是：

```text
native_hosted_pending_body_frontier: function=native_build_atomic_i32_shape_empty decl=326 function_id=21 body_stmts=1 reason=pending_core_body
```
...
1. CoreIR 必须生成单条 `CORE_STMT_KIND_RETURN`，返回表达式保持
   `NativeBuildAtomicI32Shape` struct literal surface。
2. struct literal 的字段必须保持源码顺序和语义：`atomic_decl = null`、
   `assign_stmt = null`、`read_decl = null`、`return_stmt = null`、
   `initial_value = 0`、`updated_value = 0`、`return_value = 0`；不得省略任一
   AST 指针字段，也不得把任一 i32 字段改成非零。
3. PortableMIR 必须保留 aggregate return surface，返回类型为
   `NativeBuildAtomicI32Shape`，不得把该 helper 降成 noop 或 pending body。
4. 该切片迁入后 `native_build_atomic_i32_shape_empty()` 必须达到 body complete：
   `native_hosted_reachable_body_complete: function=native_build_atomic_i32_shape_empty prefix_stmts=1 reason=body_complete`。
   下一步必须重新读取真实 self-build frontier。

## `native_build_simd_vector_mask_shape_empty()` Body Complete Contract

`native_build_atomic_i32_shape_empty()` body complete 后，当前真实 pending frontier 是：

```text
native_hosted_pending_body_frontier: function=native_build_simd_vector_mask_shape_empty decl=327 function_id=22 body_stmts=1 reason=pending_core_body
```
...
1. CoreIR 必须生成单条 `CORE_STMT_KIND_RETURN`，返回表达式保持
   `NativeBuildSimdVectorMaskShape` struct literal surface。
2. struct literal 的字段必须保持源码顺序和语义：`left_decl = null`、
   `right_decl = null`、`mask_decl = null`、`selected_decl = null`、
   `return_stmt = null`、`lanes = 0`、`left_value = 0`、`right_value = 0`、
   `mask_truth = 0`、`selected_value = 0`、`return_value = 0`；不得省略任一
   AST 指针字段，也不得把任一 i32 字段改成非零。
3. PortableMIR 必须保留 aggregate return surface，返回类型为
   `NativeBuildSimdVectorMaskShape`，不得把该 helper 降成 noop 或 pending body。
4. 该切片迁入后 `native_build_simd_vector_mask_shape_empty()` 必须达到 body complete：
...
```text
native_hosted_coreir_preflight: status=0 verifier_error=0 functions=3976 core_bodies=26 pending_bodies=3473
native_hosted_preflight: status=0 verifier_error=0 mir_extern_functions=481 mir_body_functions=25 mir_types=8 extern_symbols=478 c_import_objects=0 hosted_link_objects=0
native_hosted_reachable_body_complete: function=parse_build_args prefix_stmts=28 reason=body_complete
...
## `native_build_type_is_i32(...)` Body Complete Contract
...
1. CoreBody 必须覆盖完整 3 statement body surface：null/type/name guard、
   `str_equals(type_node.type_named_name, "i32" as *byte) != 0` branch 和 tail `return 0`。
...
4. 该切片迁入后 `native_build_type_is_i32(...)` 必须达到 body complete：
   `native_hosted_reachable_body_complete: function=native_build_type_is_i32 prefix_stmts=3 reason=body_complete`。
   下一步必须重新读取真实 self-build frontier。

2026-06-11 实测迁入后，hosted native self-build frontier 前进为：

```text
native_hosted_coreir_preflight: status=0 verifier_error=0 functions=3982 core_bodies=28 pending_bodies=3477
native_hosted_preflight: status=0 verifier_error=0 mir_extern_functions=481 mir_body_functions=27 mir_types=8 extern_symbols=478 c_import_objects=0 hosted_link_objects=0
native_hosted_reachable_body_complete: function=parse_build_args prefix_stmts=28 reason=body_complete
native_hosted_pending_body_frontier: function=native_build_type_is_usize decl=340 function_id=26 body_stmts=3 reason=pending_core_body
```

## `native_build_type_is_usize(...)` Body Complete Contract
...
```uya
fn native_build_type_is_usize(type_node: &ASTNode) i32 {
    if type_node == null || type_node.type != ASTNodeType.AST_TYPE_NAMED ||
       type_node.type_named_name == null {
        return 0;
    }
    if str_equals(type_node.type_named_name, "usize" as *byte) != 0 {
        return 1;
    }
    return 0;
}
...
1. CoreBody 必须覆盖完整 3 statement body surface：null/type/name guard、
   `str_equals(type_node.type_named_name, "usize" as *byte) != 0` branch 和 tail `return 0`。
2. guard 分支必须保持失败返回 `0`，string compare 成功分支必须返回 `1`，tail fallback
   必须返回 `0`。
3. PortableMIR 必须保留两个 branch 和 tail return surface，不得把该 helper 降成单一常量
   return 或 pending body。
4. 该切片迁入后 `native_build_type_is_usize(...)` 必须达到 body complete：
   `native_hosted_reachable_body_complete: function=native_build_type_is_usize prefix_stmts=3 reason=body_complete`。
   下一步必须重新读取真实 self-build frontier。

2026-06-11 实测迁入后，hosted native self-build frontier 前进为：

```text
native_hosted_coreir_preflight: status=0 verifier_error=0 functions=3985 core_bodies=29 pending_bodies=3479
native_hosted_preflight: status=0 verifier_error=0 mir_extern_functions=481 mir_body_functions=28 mir_types=8 extern_symbols=478 c_import_objects=0 hosted_link_objects=0
native_hosted_reachable_body_complete: function=parse_build_args prefix_stmts=28 reason=body_complete
native_hosted_pending_body_frontier: function=native_build_type_named_equals decl=344 function_id=27 body_stmts=3 reason=pending_core_body
```

## `native_build_type_named_equals(...)` Body Complete Contract
...
1. CoreBody 必须覆盖完整 3 statement body surface：`type_node == null || name == null`
   guard、named type/name guard、`str_equals(type_node.type_named_name as *byte, name as *byte) != 0`
   branch 和 tail `return 0`。
2. guard 分支必须保持失败返回 `0`，string compare 成功分支必须返回 `1`，tail fallback
   必须返回 `0`。
3. PortableMIR 必须保留两个 branch 和 tail return surface，不得把该 helper 降成单一常量
   return 或 pending body。
4. 该切片迁入后 `native_build_type_named_equals(...)` 必须达到 body complete：
   `native_hosted_reachable_body_complete: function=native_build_type_named_equals prefix_stmts=3 reason=body_complete`。
   下一步必须重新读取真实 self-build frontier。

2026-06-11 实测迁入后，hosted native self-build frontier 前进为：

```text
native_hosted_coreir_preflight: status=0 verifier_error=0 functions=3988 core_bodies=30 pending_bodies=3481
native_hosted_preflight: status=0 verifier_error=0 mir_extern_functions=481 mir_body_functions=29 mir_types=8 extern_symbols=478 c_import_objects=0 hosted_link_objects=0
native_hosted_reachable_body_complete: function=parse_build_args prefix_stmts=28 reason=body_complete
native_hosted_pending_body_frontier: function=native_build_decl_returns_param_directly decl=348 function_id=28 body_stmts=5 reason=pending_core_body
```

## `native_build_decl_returns_param_directly(...)` Body Complete Contract
...
当前源码 body surface 必须保持为 5 条顶层 statement：
...
1. CoreBody 必须覆盖完整 5 statement body surface：decl/param/body guard、
   `const stmt` 局部、identifier return guard、
   `str_equals(stmt.return_stmt_expr.identifier_name as *byte, param_name as *byte) != 0`
   branch 和 tail `return 0`。
2. 两个 guard 分支必须保持失败返回 `0`，string compare 成功分支必须返回 `1`，
   tail fallback 必须返回 `0`。
3. `const stmt` 必须保留为从 `decl.fn_decl_body.block_stmts[0]` 取出的 ASTNode 指针，
   不得用摘要 helper 或硬编码替代。
4. PortableMIR 必须保留两个 guard、局部声明、string compare branch 和 tail return
   surface，不得把该 helper 降成单一常量 return 或 pending body。
5. 该切片迁入后 `native_build_decl_returns_param_directly(...)` 必须达到 body complete：
   `native_hosted_reachable_body_complete: function=native_build_decl_returns_param_directly prefix_stmts=5 reason=body_complete`。
   下一步必须重新读取真实 self-build frontier。

2026-06-11 实测迁入后，hosted native self-build frontier 前进为：

```text
native_hosted_coreir_preflight: status=0 verifier_error=0 functions=3991 core_bodies=31 pending_bodies=3483
native_hosted_preflight: status=0 verifier_error=0 mir_extern_functions=481 mir_body_functions=30 mir_types=8 extern_symbols=478 c_import_objects=0 hosted_link_objects=0
native_hosted_reachable_body_complete: function=parse_build_args prefix_stmts=28 reason=body_complete
...
```text
native_hosted_coreir_preflight: status=0 verifier_error=0 functions=3994 core_bodies=32 pending_bodies=3485
native_hosted_preflight: status=0 verifier_error=0 mir_extern_functions=481 mir_body_functions=31 mir_types=8 extern_symbols=478 c_import_objects=0 hosted_link_objects=0
native_hosted_reachable_body_complete: function=parse_build_args prefix_stmts=28 reason=body_complete
...
```text
native_hosted_coreir_preflight: status=0 verifier_error=0 functions=3998 core_bodies=33 pending_bodies=3488
native_hosted_preflight: status=0 verifier_error=0 mir_extern_functions=481 mir_body_functions=32 mir_types=8 extern_symbols=478 c_import_objects=0 hosted_link_objects=0
native_hosted_reachable_body_complete: function=parse_build_args prefix_stmts=28 reason=body_complete
native_hosted_pending_body_frontier: function=native_build_type_is_byte decl=360 function_id=31 body_stmts=3 reason=pending_core_body
```

## `native_build_type_is_byte(...)` Body Complete Contract
...
1. CoreBody 必须覆盖完整 3 statement body surface：null/type/name guard、
   `str_equals(type_node.type_named_name, "byte" as *byte) != 0` branch 和 tail `return 0`。
2. guard 分支必须保持失败返回 `0`，string compare 成功分支必须返回 `1`，tail fallback
   必须返回 `0`。
3. PortableMIR 必须保留两个 branch 和 tail return surface，不得把该 helper 降成单一常量
   return 或 pending body。
4. 该切片迁入后 `native_build_type_is_byte(...)` 必须达到 body complete：
   `native_hosted_reachable_body_complete: function=native_build_type_is_byte prefix_stmts=3 reason=body_complete`。
   下一步必须重新读取真实 self-build frontier。

2026-06-11 实测迁入后，hosted native self-build frontier 前进为：

```text
native_hosted_coreir_preflight: status=0 verifier_error=0 functions=4001 core_bodies=34 pending_bodies=3490
native_hosted_preflight: status=0 verifier_error=0 mir_extern_functions=481 mir_body_functions=33 mir_types=8 extern_symbols=478 c_import_objects=0 hosted_link_objects=0
native_hosted_reachable_body_complete: function=parse_build_args prefix_stmts=28 reason=body_complete
native_hosted_pending_body_frontier: function=native_build_type_is_byte_ptr decl=364 function_id=32 body_stmts=2 reason=pending_core_body
```

## `native_build_type_is_byte_ptr(...)` Body Complete Contract
...
1. CoreBody 必须覆盖完整 2 statement body surface：pointer null/type guard 和
   `native_build_type_is_byte(type_node.type_pointer_pointed_type)` helper-call return。
2. guard 分支必须保持失败返回 `0`；tail return 必须保留 pointed-type 读取和 helper-call
   surface，不得压成常量或跳过被调 helper。
3. PortableMIR 必须保留 helper-to-helper call surface，不得把该 helper 降成单一常量
   return 或 pending body。
4. 该切片迁入后 `native_build_type_is_byte_ptr(...)` 必须达到 body complete：
   `native_hosted_reachable_body_complete: function=native_build_type_is_byte_ptr prefix_stmts=2 reason=body_complete`。
   下一步必须重新读取真实 self-build frontier。

2026-06-11 实测迁入后，hosted native self-build frontier 前进为：

```text
native_hosted_coreir_preflight: status=0 verifier_error=0 functions=4005 core_bodies=35 pending_bodies=3493
native_hosted_preflight: status=0 verifier_error=0 mir_extern_functions=481 mir_body_functions=34 mir_types=8 extern_symbols=478 c_import_objects=0 hosted_link_objects=0
native_hosted_reachable_body_complete: function=parse_build_args prefix_stmts=28 reason=body_complete
native_hosted_pending_body_frontier: function=native_build_type_is_i32_ptr decl=368 function_id=33 body_stmts=2 reason=pending_core_body
```

## `native_build_type_is_i32_ptr(...)` Body Complete Contract
...
1. CoreBody 必须覆盖完整 2 statement body surface：pointer null/type guard、
   FFI pointer / const pointer 排除，以及
   `native_build_type_is_i32(type_node.type_pointer_pointed_type)` helper-call return。
2. guard 分支必须保持失败返回 `0`；tail return 必须保留 pointed-type 读取和 helper-call
   surface，不得压成常量或跳过被调 helper。
3. PortableMIR 必须保留 helper-to-helper call surface，不得把该 helper 降成单一常量
   return 或 pending body。
4. 该切片迁入后 `native_build_type_is_i32_ptr(...)` 必须达到 body complete：
   `native_hosted_reachable_body_complete: function=native_build_type_is_i32_ptr prefix_stmts=2 reason=body_complete`。
   下一步必须重新读取真实 self-build frontier。

2026-06-11 实测迁入后，hosted native self-build frontier 前进为：

```text
native_hosted_coreir_preflight: status=0 verifier_error=0 functions=4009 core_bodies=36 pending_bodies=3496
native_hosted_preflight: status=0 verifier_error=0 mir_extern_functions=481 mir_body_functions=35 mir_types=8 extern_symbols=478 c_import_objects=0 hosted_link_objects=0
native_hosted_reachable_body_complete: function=parse_build_args prefix_stmts=28 reason=body_complete
native_hosted_pending_body_frontier: function=native_build_type_is_i32_like_ptr decl=372 function_id=34 body_stmts=2 reason=pending_core_body
```

## `native_build_type_is_i32_like_ptr(...)` Body Complete Contract
...
1. CoreBody 必须覆盖完整 2 statement body surface：pointer null/type guard、
   FFI pointer / const pointer 排除，以及
   `native_build_type_is_i32_like_scalar(type_node.type_pointer_pointed_type)` helper-call return。
2. guard 分支必须保持失败返回 `0`；tail return 必须保留 pointed-type 读取和 helper-call
   surface，不得压成常量或跳过被调 helper。
3. PortableMIR 必须保留 helper-to-helper call surface，不得把该 helper 降成单一常量
   return 或 pending body。
4. 该切片迁入后 `native_build_type_is_i32_like_ptr(...)` 必须达到 body complete：
   `native_hosted_reachable_body_complete: function=native_build_type_is_i32_like_ptr prefix_stmts=2 reason=body_complete`。
   下一步必须重新读取真实 self-build frontier。

2026-06-11 实测迁入后，hosted native self-build frontier 前进为：

```text
native_hosted_coreir_preflight: status=0 verifier_error=0 functions=4013 core_bodies=37 pending_bodies=3499
native_hosted_preflight: status=0 verifier_error=0 mir_extern_functions=481 mir_body_functions=36 mir_types=8 extern_symbols=478 c_import_objects=0 hosted_link_objects=0
native_hosted_reachable_body_complete: function=parse_build_args prefix_stmts=28 reason=body_complete
native_hosted_pending_body_frontier: function=native_build_type_is_i32_array decl=376 function_id=35 body_stmts=2 reason=pending_core_body
```

## `native_build_type_is_i32_array(...)` Body Complete Contract
...
1. CoreBody 必须覆盖完整 2 statement body surface：array null/type guard、
   `native_build_type_is_i32(type_node.type_array_element_type)` helper-call guard、
   array size expr null/number/positive checks 和 tail `return 1`。
2. guard 分支必须保持失败返回 `0`；tail return 必须保持成功返回 `1`。
3. PortableMIR 必须保留 guard 中的 element helper-call surface 和 tail return，不得把该 helper
   降成单一常量 return 或 pending body。
4. 该切片迁入后 `native_build_type_is_i32_array(...)` 必须达到 body complete：
   `native_hosted_reachable_body_complete: function=native_build_type_is_i32_array prefix_stmts=2 reason=body_complete`。
   下一步必须重新读取真实 self-build frontier。

2026-06-11 实测迁入后，hosted native self-build frontier 前进为：

```text
native_hosted_coreir_preflight: status=0 verifier_error=0 functions=4016 core_bodies=38 pending_bodies=3501
native_hosted_preflight: status=0 verifier_error=0 mir_extern_functions=481 mir_body_functions=37 mir_types=8 extern_symbols=478 c_import_objects=0 hosted_link_objects=0
native_hosted_reachable_body_complete: function=parse_build_args prefix_stmts=28 reason=body_complete
...
## `native_build_decl_is_noarg_i32_fn(...)` Body Complete Contract
...
1. CoreBody 必须覆盖完整 3 statement body surface：decl null/type/name guard、
   no-arg/non-extern/non-async/non-naked/non-varargs guard 和 tail
   `native_build_type_is_i32(decl.fn_decl_return_type)` helper-call return。
2. 两个 guard 分支必须保持失败返回 `0`；tail return 必须保留 return-type helper-call surface。
3. PortableMIR 必须保留 tail helper-call return，不得把该 helper 降成单一常量 return 或 pending body。
4. 该切片迁入后 `native_build_decl_is_noarg_i32_fn(...)` 必须达到 body complete：
   `native_hosted_reachable_body_complete: function=native_build_decl_is_noarg_i32_fn prefix_stmts=3 reason=body_complete`。
   下一步必须重新读取真实 self-build frontier。

2026-06-11 实测迁入后，hosted native self-build frontier 前进为：

```text
native_hosted_coreir_preflight: status=0 verifier_error=0 functions=4022 core_bodies=40 pending_bodies=3505
native_hosted_preflight: status=0 verifier_error=0 mir_extern_functions=481 mir_body_functions=39 mir_types=8 extern_symbols=478 c_import_objects=0 hosted_link_objects=0
native_hosted_reachable_body_complete: function=native_build_decl_is_noarg_i32_fn prefix_stmts=3 reason=body_complete
native_hosted_pending_body_frontier: function=native_build_decl_is_one_i32_param_fn decl=388 function_id=38 body_stmts=5 reason=pending_core_body
```

## `native_build_decl_is_one_i32_param_fn(...)` Body Complete Contract
...
1. CoreBody 必须覆盖完整 5 statement body surface：decl null/type/name guard、
   one-param/non-extern/non-async/non-naked/non-varargs/return-type/params guard、
   param local、param var-decl guard 和 tail `native_build_type_is_i32(param.var_decl_type)`
   helper-call return。
2. guard 分支必须保持失败返回 `0`；return-type 和 param-type helper-call surface
   必须保留 resolved call target。
3. PortableMIR 必须保留 tail helper-call return，不得把该 helper 降成单一常量 return 或 pending body。
4. 该切片迁入后 `native_build_decl_is_one_i32_param_fn(...)` 必须达到 body complete：
...
```text
native_hosted_coreir_preflight: status=0 verifier_error=0 functions=4028 core_bodies=42 pending_bodies=3509
native_hosted_preflight: status=0 verifier_error=0 mir_extern_functions=481 mir_body_functions=41 mir_types=8 extern_symbols=478 c_import_objects=0 hosted_link_objects=0
native_hosted_reachable_body_complete: function=native_build_decl_is_identity_generic_i32_fn prefix_stmts=9 reason=body_complete
native_hosted_pending_body_frontier: function=native_build_decl_is_two_i32_param_fn decl=396 function_id=40 body_stmts=7 reason=pending_core_body
```

## `native_build_decl_is_two_i32_param_fn(...)` Body Complete Contract
...
1. CoreBody 必须覆盖完整 7 statement body surface：decl guard、
   two-param/non-extern/non-async/non-naked/non-varargs/return-type/params guard、
   param0/param1 locals、param shape guard、param0 type helper-call guard 和 tail
   param1 type helper-call return。
2. guard 分支必须保持失败返回 `0`；三个 type helper-call surface 必须保留 resolved call target。
3. PortableMIR 必须保留 tail helper-call return，不得把该 helper 降成单一常量 return 或 pending body。
4. 该切片迁入后 `native_build_decl_is_two_i32_param_fn(...)` 必须达到 body complete：
   `native_hosted_reachable_body_complete: function=native_build_decl_is_two_i32_param_fn prefix_stmts=7 reason=body_complete`。
   下一步必须重新读取真实 self-build frontier。

2026-06-11 实测迁入后，hosted native self-build frontier 前进为：

```text
native_hosted_coreir_preflight: status=0 verifier_error=0 functions=4031 core_bodies=43 pending_bodies=3511
native_hosted_preflight: status=0 verifier_error=0 mir_extern_functions=481 mir_body_functions=42 mir_types=8 extern_symbols=478 c_import_objects=0 hosted_link_objects=0
native_hosted_reachable_body_complete: function=parse_build_args prefix_stmts=28 reason=body_complete
