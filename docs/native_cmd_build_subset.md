...
- hosted `build --native` 入口必须先进入 `PortableMIR` / `MirTargetBackendRequest` / `NativeHostedLinkPlan`
  handoff；当前已把 AST extern function 作为 verified `PortableMIR` extern function stub，并固定 Machine
  backend request 和 `NativeHostedLinkPlan` preflight，同时把 AST 中的 extern function symbol 与 `@c_import`
  object 计入 hosted link plan。真正的缺口是
