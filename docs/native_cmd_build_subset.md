...
下一项继续按缺口补 native `cmd/build` 子集能力：当前只有 build CLI 的极小函数输出路径；`cmd/build` 自身已经进入 reachable lowering，并已支持无副作用局部声明占位、zero-arg / one-i32-constant-arg / two-i32-constant-arg direct call 局部初始化、`&local` / `&array[0]` out-param 写回和 `return local` / `return array[0]`。下一道真实门槛是支持 `parse_build_args(...)` 这种更多参数调用、更多 out-param 写回、条件分支和后续 CLI 控制流。完成这些之前，不能声明已经生成 native `bin/cmd/build`。
