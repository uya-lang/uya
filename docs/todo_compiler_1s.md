...
1. 下一个未完成叶子是“为 line-directives 标量分支补独立合同脚本”；该叶子只写合同/测试/文档，
   固定 `--no-line-directives` / `--line-directives` 的 out-param store surface、branch frontier
   和 no-silent-C99 预期，不改生产实现。
2. line-directives 合同通过后，才进入“将 line-directives 标量分支迁入 PortableMIR”；之后按已列出的
   safety-proof、opt-level、`--nostdlib` 合同/实现叶子逐个推进。
