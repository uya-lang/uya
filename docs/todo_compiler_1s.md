      ...
      - 剩余任务差分（按 `native_hosted_reachable_callee_frontier` 顺序执行；每个切片只跑相关测试，不跑
        ...
        - frontier-driven helper 队列（每个 helper 到达后按同一模板执行；候选只能来自真实诊断）：
          ...
          - [x] 迁入 `native_build_reachability_empty()` 的 struct literal return，使该 helper
            ...
            reason=pending_core_body`。
          - [x] 为 `native_build_local_table_empty()` 补 CoreBody/PortableMIR body-complete
            合同；固定 `NativeBuildLocalTable{ names/call_targets/kinds/init_values/static_knowns/lengths:
            null, count: 0, capacity: 0 }` return surface，不改生产 lowering。
            - 2026-06-11：新增 `tests/verify_native_local_table_empty_contract.sh` 并接入
              `tests/verify_native_cmd_build_stage1.sh`；`docs/native_cmd_build_subset.md`
              新增 `native_build_local_table_empty()` Body Complete Contract，冻结当前 pending
              frontier，并要求迁入后达到 `body_complete`。
            - 实测 `bash tests/verify_native_local_table_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh` 和 `git diff --check` 通过；
              本叶子未改生产 lowering。
          - [x] 迁入 `native_build_local_table_empty()` 的 struct literal return，使该 helper
            达到 body complete；复测真实 frontier 后再选择下一个 helper。
            - 2026-06-11：在 `src/build_compiler_driver.uya` 接入该 helper 的 CoreBody/PortableMIR
              body-complete lowering；实测 `make -B cmd-build UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya`、
              `bash tests/verify_native_local_table_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_no_silent_c99.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh` 和 `git diff --check` 通过。
            - 真实 self-build frontier 前进为 `native_hosted_pending_body_frontier:
              function=native_build_const_slice_sum_shape_empty decl=311 function_id=12 body_stmts=1
              reason=pending_core_body`；preflight 计数同步为 `core_bodies=14`、
              `mir_body_functions=13`。
          - [x] 为 `native_build_const_slice_sum_shape_empty()` 补 CoreBody/PortableMIR
            body-complete 合同；固定 `NativeBuildConstSliceSumShape{ array_decl/slice_decl/
            return_stmt/add_expr/left_access/right_access: null, slice_start/slice_len/
            left_value/right_value/sum_value: 0 }` return surface，不改生产 lowering。
            - 2026-06-11：新增 `tests/verify_native_const_slice_sum_shape_empty_contract.sh`
              并接入 `tests/verify_native_cmd_build_stage1.sh`；`docs/native_cmd_build_subset.md`
              新增 `native_build_const_slice_sum_shape_empty()` Body Complete Contract，冻结当前
              pending frontier，并要求迁入后达到 `body_complete`。
            - 实测 `bash tests/verify_native_const_slice_sum_shape_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check` 和
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              通过；本叶子未改生产 lowering。
          - [x] 迁入 `native_build_const_slice_sum_shape_empty()` 的 struct literal return，
            使该 helper 达到 body complete；复测真实 frontier 后再选择下一个 helper。
            - 2026-06-11：在 `src/build_compiler_driver.uya` 接入该 helper 的 CoreBody/PortableMIR
              body-complete lowering；实测 `make -B cmd-build UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya`、
              `bash tests/verify_native_const_slice_sum_shape_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_no_silent_c99.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check` 和
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              通过。
            - 真实 self-build frontier 前进为 `native_hosted_pending_body_frontier:
              function=native_build_const_slice_len_index_shape_empty decl=315 function_id=13 body_stmts=1
              reason=pending_core_body`；preflight 计数同步为 `core_bodies=15`、
              `mir_body_functions=14`。
          - [x] 为 `native_build_const_slice_len_index_shape_empty()` 补 CoreBody/PortableMIR
            body-complete 合同；固定 `NativeBuildConstSliceLenIndexShape{ array_decl/slice_decl/
            return_stmt/add_expr/len_expr/index_access: null, slice_start/slice_len/len_value/
            indexed_value/sum_value: 0 }` return surface，不改生产 lowering。
            - 2026-06-11：新增
              `tests/verify_native_const_slice_len_index_shape_empty_contract.sh` 并接入
              `tests/verify_native_cmd_build_stage1.sh`；`docs/native_cmd_build_subset.md`
              新增 `native_build_const_slice_len_index_shape_empty()` Body Complete Contract，冻结当前
              pending frontier，并要求迁入后达到 `body_complete`。
            - 实测 `bash tests/verify_native_const_slice_len_index_shape_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check` 和
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              通过；本叶子未改生产 lowering。
          - [x] 迁入 `native_build_const_slice_len_index_shape_empty()` 的 struct literal return，
            使该 helper 达到 body complete；复测真实 frontier 后再选择下一个 helper。
            - 2026-06-11：在 `src/build_compiler_driver.uya` 接入该 helper 的 CoreBody/PortableMIR
              body-complete lowering；实测 `make -B cmd-build UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya`、
              `bash tests/verify_native_const_slice_len_index_shape_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_no_silent_c99.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check` 和
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              通过。
            - 真实 self-build frontier 前进为 `native_hosted_pending_body_frontier:
              function=native_build_pointer_deref_shape_empty decl=319 function_id=14 body_stmts=1
              reason=pending_core_body`；preflight 计数同步为 `core_bodies=16`、
              `mir_body_functions=15`。
          - [x] 为 `native_build_pointer_deref_shape_empty()` 补 CoreBody/PortableMIR
            body-complete 合同；固定 `NativeBuildPointerDerefShape{ value_decl/pointer_decl/
            return_stmt/address_expr/deref_expr: null, value: 0 }` return surface，不改生产 lowering。
            - 2026-06-11：新增 `tests/verify_native_pointer_deref_shape_empty_contract.sh`
              并接入 `tests/verify_native_cmd_build_stage1.sh`；`docs/native_cmd_build_subset.md`
              新增 `native_build_pointer_deref_shape_empty()` Body Complete Contract，冻结当前
              pending frontier，并要求迁入后达到 `body_complete`。
            - 实测 `bash tests/verify_native_pointer_deref_shape_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check` 和
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              通过；本叶子未改生产 lowering。
          ...
          - [x] 为 `native_build_dynamic_catch_shape_empty()` 补 CoreBody/PortableMIR
            body-complete 合同；固定 `NativeBuildDynamicCatchShape{ argc_decl/catch_decl/
            return_stmt/catch_arg_expr: null, compare_value/fallback_value/ok_value: 0 }`
            return surface，不改生产 lowering。
            - 2026-06-11：新增 `tests/verify_native_dynamic_catch_shape_empty_contract.sh`
              并接入 `tests/verify_native_cmd_build_stage1.sh`；`docs/native_cmd_build_subset.md`
              新增 `native_build_dynamic_catch_shape_empty()` Body Complete Contract，冻结当前
              pending frontier，并要求迁入后达到 `body_complete`。
            - 实测 `bash tests/verify_native_dynamic_catch_shape_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check` 和
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              通过；本叶子未改生产 lowering。
          - [x] 迁入 `native_build_dynamic_catch_shape_empty()` 的 struct literal return，使该 helper
            达到 body complete；复测真实 frontier 后再选择下一个 helper。
            - 2026-06-11：扩展 `src/build_compiler_driver.uya` 中的空 shape helper
              body-complete 识别，使 `native_build_dynamic_catch_shape_empty()` 复用单 return
              CoreBody/PortableMIR lowering；实测 `make -B cmd-build
              UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya`、
              `bash tests/verify_native_dynamic_catch_shape_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_no_silent_c99.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check` 和
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              通过。
            - 真实 self-build frontier 前进为 `native_hosted_pending_body_frontier:
              function=native_build_defer_local_assign_shape_empty decl=321 function_id=16 body_stmts=1
              reason=pending_core_body`；preflight 计数同步为 `core_bodies=18`、
              `mir_body_functions=17`。
          - [x] 为 `native_build_defer_local_assign_shape_empty()` 补 CoreBody/PortableMIR
            body-complete 合同；固定 `NativeBuildDeferLocalAssignShape{ local_decl/defer_stmt/
            assign_stmt/return_stmt: null, initial_value/deferred_value/return_value/
            return_is_local_ref: 0 }` return surface，不改生产 lowering。
            - 2026-06-11：新增
              `tests/verify_native_defer_local_assign_shape_empty_contract.sh` 并接入
              `tests/verify_native_cmd_build_stage1.sh`；`docs/native_cmd_build_subset.md`
              新增 `native_build_defer_local_assign_shape_empty()` Body Complete Contract，冻结当前
              pending frontier，并要求迁入后达到 `body_complete`。
            - 实测 `bash tests/verify_native_defer_local_assign_shape_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check` 和
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              通过；本叶子未改生产 lowering。
          - [x] 迁入 `native_build_defer_local_assign_shape_empty()` 的 struct literal return，
            使该 helper 达到 body complete；复测真实 frontier 后再选择下一个 helper。
            - 2026-06-11：扩展 `src/build_compiler_driver.uya` 中的空 shape helper
              body-complete 识别，使 `native_build_defer_local_assign_shape_empty()` 复用单 return
              CoreBody/PortableMIR lowering；实测 `make -B cmd-build
              UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya`、
              `bash tests/verify_native_defer_local_assign_shape_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_no_silent_c99.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check` 和
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              通过。
          ...
          - [x] 为 `native_build_lexical_drop_shape_empty()` 补 CoreBody/PortableMIR
            ...
            - 实测 `bash tests/verify_native_lexical_drop_shape_empty_contract.sh`、
              ...
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          - [x] 迁入 `native_build_lexical_drop_shape_empty()` 的 struct literal return，使该 helper
            达到 body complete；复测真实 frontier 后再选择下一个 helper。
            - 2026-06-11：扩展 `src/build_compiler_driver.uya` 中的空 shape helper
              body-complete 识别，使 `native_build_lexical_drop_shape_empty()` 复用单 return
              CoreBody/PortableMIR lowering；实测 `make -B cmd-build
              UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya`、
              `bash tests/verify_native_lexical_drop_shape_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_no_silent_c99.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check` 和
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              通过。
          ...
          - [x] 为 `native_build_interface_method_shape_empty()` 补 CoreBody/PortableMIR
            ...
            - 实测 `bash tests/verify_native_interface_method_shape_empty_contract.sh`、
              ...
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          - [x] 迁入 `native_build_interface_method_shape_empty()` 的 struct literal return，使该 helper
            达到 body complete；复测真实 frontier 后再选择下一个 helper。
            - 2026-06-11：扩展 `src/build_compiler_driver.uya` 中的空 shape helper
              body-complete 识别，使 `native_build_interface_method_shape_empty()` 复用单 return
              CoreBody/PortableMIR lowering；实测 `make -B cmd-build
              UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya`、
              `bash tests/verify_native_interface_method_shape_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_no_silent_c99.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check` 和
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              通过。
            - 真实 self-build frontier 前进为 `native_hosted_pending_body_frontier:
              function=native_build_direct_method_shape_empty decl=324 function_id=19 body_stmts=1
              reason=pending_core_body`；preflight 计数同步为 `core_bodies=21`、
              `mir_body_functions=20`。
          - [x] 为 `native_build_direct_method_shape_empty()` 补 CoreBody/PortableMIR
            body-complete 合同；固定当前 struct literal return surface，不改生产 lowering。
            - 2026-06-11：新增 `tests/verify_native_direct_method_shape_empty_contract.sh`
              并接入 `tests/verify_native_cmd_build_stage1.sh`；
              `docs/native_cmd_build_subset.md` 新增
              `native_build_direct_method_shape_empty()` Body Complete Contract，冻结当前
              pending frontier 与 struct literal return surface。
            - 实测 `bash tests/verify_native_direct_method_shape_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check`、
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          - [x] 迁入 `native_build_direct_method_shape_empty()` 的 struct literal return，使该 helper
            达到 body complete；复测真实 frontier 后再选择下一个 helper。
            - 2026-06-11：扩展 `src/build_compiler_driver.uya` 中的空 shape helper
              body-complete 识别，使 `native_build_direct_method_shape_empty()` 复用单 return
              CoreBody/PortableMIR lowering；实测 `make -B cmd-build
              UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya`、
              `bash tests/verify_native_direct_method_shape_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_no_silent_c99.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check` 和
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              通过。
            - 真实 self-build frontier 前进为 `native_hosted_pending_body_frontier:
              function=native_build_struct_union_enum_shape_empty decl=325 function_id=20 body_stmts=1
              reason=pending_core_body`；preflight 计数同步为 `core_bodies=22`、
              `mir_body_functions=21`。
          - [x] 为 `native_build_struct_union_enum_shape_empty()` 补 CoreBody/PortableMIR
            body-complete 合同；固定当前 struct literal return surface，不改生产 lowering。
            - 2026-06-11：新增 `tests/verify_native_struct_union_enum_shape_empty_contract.sh`
              并接入 `tests/verify_native_cmd_build_stage1.sh`；
              `docs/native_cmd_build_subset.md` 新增
              `native_build_struct_union_enum_shape_empty()` Body Complete Contract，冻结当前
              pending frontier 与 struct literal return surface。
            - 实测 `bash tests/verify_native_struct_union_enum_shape_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check`、
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          - [x] 迁入 `native_build_struct_union_enum_shape_empty()` 的 struct literal return，使该 helper
            达到 body complete；复测真实 frontier 后再选择下一个 helper。
            - 2026-06-11：扩展 `src/build_compiler_driver.uya` 中的空 shape helper
              body-complete 识别，使 `native_build_struct_union_enum_shape_empty()` 复用单 return
              CoreBody/PortableMIR lowering；实测 `make -B cmd-build
              UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya`、
              `bash tests/verify_native_struct_union_enum_shape_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_no_silent_c99.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check` 和
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              通过。
            - 真实 self-build frontier 前进为 `native_hosted_pending_body_frontier:
              function=native_build_atomic_i32_shape_empty decl=326 function_id=21 body_stmts=1
              reason=pending_core_body`；preflight 计数同步为 `core_bodies=23`、
              `mir_body_functions=22`。
          - [x] 为 `native_build_atomic_i32_shape_empty()` 补 CoreBody/PortableMIR
            body-complete 合同；固定当前 struct literal return surface，不改生产 lowering。
            - 2026-06-11：新增 `tests/verify_native_atomic_i32_shape_empty_contract.sh`
              并接入 `tests/verify_native_cmd_build_stage1.sh`；
              `docs/native_cmd_build_subset.md` 新增
              `native_build_atomic_i32_shape_empty()` Body Complete Contract，冻结当前
              pending frontier 与 struct literal return surface。
            - 实测 `bash tests/verify_native_atomic_i32_shape_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check`、
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          ...
          - [x] 为 `native_build_simd_vector_mask_shape_empty()` 补 CoreBody/PortableMIR
            body-complete 合同；固定当前 struct literal return surface，不改生产 lowering。
            - 2026-06-11：新增 `tests/verify_native_simd_vector_mask_shape_empty_contract.sh`
              并接入 `tests/verify_native_cmd_build_stage1.sh`；
              `docs/native_cmd_build_subset.md` 新增
              `native_build_simd_vector_mask_shape_empty()` Body Complete Contract，冻结当前
              pending frontier 与 struct literal return surface。
            - 实测 `bash tests/verify_native_simd_vector_mask_shape_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check`、
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          - [x] 迁入 `native_build_simd_vector_mask_shape_empty()` 的 struct literal return，使该 helper
            达到 body complete；复测真实 frontier 后再选择下一个 helper。
            - 2026-06-11：扩展 `src/build_compiler_driver.uya` 中的空 shape helper
              body-complete 识别，使 `native_build_simd_vector_mask_shape_empty()` 复用单 return
              CoreBody/PortableMIR lowering；实测 `make -B cmd-build
              UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya`、
              `bash tests/verify_native_simd_vector_mask_shape_empty_contract.sh`、
              `bash tests/verify_native_cmd_build_no_silent_c99.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check` 和
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              通过。
          ...
          - [x] 迁入 `native_build_reachability_init(...)` 的完整初始化 body，使该 helper
            ...
            - 实测 `make -B cmd-build UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya` 通过；
              ...
              reason=pending_core_body`。
          - [x] 为 `native_build_type_is_i32(...)` 补 CoreBody/PortableMIR body-complete
            合同；固定当前 3 statement body surface、null/type/name guard、`str_equals(..., "i32")`
            branch 和 tail `return 0`；不改生产 lowering。
            - 2026-06-11：新增 `tests/verify_native_type_is_i32_contract.sh`
              并接入 `tests/verify_native_cmd_build_stage1.sh`；
              `docs/native_cmd_build_subset.md` 新增
              `native_build_type_is_i32(...)` Body Complete Contract，冻结当前 3 statement
              body surface、guard、string compare branch 和 tail return。
            - 实测 `bash tests/verify_native_type_is_i32_contract.sh`、`git diff --check`、
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          - [x] 迁入 `native_build_type_is_i32(...)` 的完整 body，使该 helper 达到 body
            complete；复测真实 frontier 后再选择下一个 helper 或 callee。
            - 2026-06-11：新增 `native_build_hosted_decl_can_materialize_type_is_i32_body`
              与 `native_build_hosted_coreir_append_type_is_i32_body`，用 3 条 root statement
              固定 guard、string compare branch 和 tail return surface；hosted PortableMIR
              preflight 同步覆盖该 helper body function。
            - 同步 `lib/libc/syscall.uya` 的 Linux `sys_gettimeofday` 路径使用 syscall number
              literal，避免旧 build generator 发射裸 `SYS_gettimeofday` 导致 host C 链接失败。
            - 实测 `make -B cmd-build UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya` 通过；
              self-build native preflight 前进为 `core_bodies=28`、`mir_body_functions=27`，
              新 frontier 为 `native_hosted_pending_body_frontier:
              function=native_build_type_is_usize decl=340 function_id=26 body_stmts=3
              reason=pending_core_body`。
          - [x] 为 `native_build_type_is_usize(...)` 补 CoreBody/PortableMIR body-complete
            合同；固定当前 3 statement body surface、null/type/name guard、
            `str_equals(..., "usize")` branch 和 tail `return 0`；不改生产 lowering。
            - 2026-06-11：新增 `tests/verify_native_type_is_usize_contract.sh`
              并接入 `tests/verify_native_cmd_build_stage1.sh`；
              `docs/native_cmd_build_subset.md` 新增
              `native_build_type_is_usize(...)` Body Complete Contract，冻结当前 3 statement
              body surface、guard、string compare branch 和 tail return。
            - 实测 `bash tests/verify_native_type_is_usize_contract.sh`、`git diff --check`、
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          - [x] 迁入 `native_build_type_is_usize(...)` 的完整 body，使该 helper 达到 body
            complete；复测真实 frontier 后再选择下一个 helper 或 callee。
            - 2026-06-11：新增 `native_build_hosted_decl_can_materialize_type_is_usize_body`
              与 `native_build_hosted_coreir_append_type_is_usize_body`，用 3 条 root statement
              固定 guard、string compare branch 和 tail return surface；hosted PortableMIR
              preflight 同步覆盖该 helper body function。
            - 同步 `lib/libc/pthread.uya` 的 Linux `pthread_yield` 路径使用 syscall number
              literal，避免旧 build generator 发射裸 `SYS_sched_yield` 导致 host C 编译失败。
            - 实测 `make -B cmd-build UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya` 通过；
              self-build native preflight 前进为 `core_bodies=29`、`mir_body_functions=28`，
              新 frontier 为 `native_hosted_pending_body_frontier:
              function=native_build_type_named_equals decl=344 function_id=27 body_stmts=3
              reason=pending_core_body`。
          - [x] 为 `native_build_type_named_equals(...)` 补 CoreBody/PortableMIR
            body-complete 合同；固定当前 3 statement body surface、type/name null guard、
            named type guard、`str_equals(type_node.type_named_name, name)` branch 和 tail
            `return 0`；不改生产 lowering。
            - 2026-06-11：新增 `tests/verify_native_type_named_equals_contract.sh`
              并接入 `tests/verify_native_cmd_build_stage1.sh`；
              `docs/native_cmd_build_subset.md` 新增
              `native_build_type_named_equals(...)` Body Complete Contract，冻结当前 3
              statement body surface、type/name guard、named type guard、string compare
              branch 和 tail return。
            - 实测 `bash tests/verify_native_type_named_equals_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check`、
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          ...
          - [x] 为 `native_build_decl_returns_param_directly(...)` 补 CoreBody/PortableMIR
            body-complete 合同；固定当前 5 statement body surface、decl/param/body guard、
            single-return-body guard、identifier return guard、`str_equals(identifier_name,
            param_name)` branch 和 tail `return 0`；不改生产 lowering。
            - 2026-06-11：新增
              `tests/verify_native_decl_returns_param_directly_contract.sh` 并接入
              `tests/verify_native_cmd_build_stage1.sh`；`docs/native_cmd_build_subset.md`
              新增 `native_build_decl_returns_param_directly(...)` Body Complete Contract，
              冻结当前 5 statement body surface、decl/param/body guard、single-return-body
              guard、identifier return guard、string compare branch 和 tail return。
            - 实测 `bash tests/verify_native_decl_returns_param_directly_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check`、
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          - [x] 迁入 `native_build_decl_returns_param_directly(...)` 的完整 body，使该 helper
            达到 body complete；复测真实 frontier 后再选择下一个 helper 或 callee。
            - 2026-06-11：新增
              `native_build_hosted_decl_can_materialize_decl_returns_param_directly_body`
              与 `native_build_hosted_coreir_append_decl_returns_param_directly_body`，
              用 5 条 root statement 固定 decl/param/body guard、`const stmt` 局部、
              identifier return guard、string compare branch 和 tail return surface；
              hosted PortableMIR preflight 同步覆盖该 helper body function。
            - 同步 `lib/libc/syscall.uya` 的 Linux `sys_futex` 路径使用 target-gated
              syscall number literal，避免旧 build generator 发射裸 `SYS_futex` 导致
              host C 编译失败。
            - 实测 `make -B cmd-build UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya` 通过；
              self-build native preflight 前进为 `core_bodies=31`、`mir_body_functions=30`，
              新 frontier 为 `native_hosted_pending_body_frontier:
              function=native_build_type_is_backend_type decl=352 function_id=29 body_stmts=3
              reason=pending_core_body`。
          - [x] 为 `native_build_type_is_backend_type(...)` 补 CoreBody/PortableMIR
            body-complete 合同；固定当前 3 statement body surface、null/type/name guard、
            `str_equals(..., "BackendType")` branch 和 tail `return 0`；不改生产 lowering。
            - 2026-06-11：新增 `tests/verify_native_type_is_backend_type_contract.sh`
              并接入 `tests/verify_native_cmd_build_stage1.sh`；
              `docs/native_cmd_build_subset.md` 新增
              `native_build_type_is_backend_type(...)` Body Complete Contract，冻结当前 3
              statement body surface、guard、BackendType string compare branch 和 tail return。
            - 实测 `bash tests/verify_native_type_is_backend_type_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check`、
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          - [x] 迁入 `native_build_type_is_backend_type(...)` 的完整 body，使该 helper
            达到 body complete；复测真实 frontier 后再选择下一个 helper 或 callee。
            - 2026-06-11：新增
              `native_build_hosted_decl_can_materialize_type_is_backend_type_body`
              与 `native_build_hosted_coreir_append_type_is_backend_type_body`，用 3 条
              root statement 固定 guard、BackendType string compare branch 和 tail return
              surface；hosted PortableMIR preflight 同步覆盖该 helper body function。
            - 同步 `lib/libc/syscall.uya` 的 Linux `sys_getdents64` / non-Darwin
              `sys_getdirentries64` 路径使用 target-gated syscall number literal，避免旧
              build generator 发射裸 `SYS_getdents64` 导致 host C 编译失败。
            - 实测 `make -B cmd-build UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya` 通过；
              self-build native preflight 前进为 `core_bodies=32`、`mir_body_functions=31`，
              新 frontier 为 `native_hosted_pending_body_frontier:
              function=native_build_type_is_i32_like_scalar decl=356 function_id=30
              body_stmts=2 reason=pending_core_body`。
          - [x] 为 `native_build_type_is_i32_like_scalar(...)` 补 CoreBody/PortableMIR
            body-complete 合同；固定当前 2 statement body surface、
            `native_build_type_is_i32(type_node)` early branch 和 tail
            `return native_build_type_is_backend_type(type_node)`；不改生产 lowering。
            - 2026-06-11：新增
              `tests/verify_native_type_is_i32_like_scalar_contract.sh` 并接入
              `tests/verify_native_cmd_build_stage1.sh`；`docs/native_cmd_build_subset.md`
              新增 `native_build_type_is_i32_like_scalar(...)` Body Complete Contract，
              冻结当前 2 statement body surface、i32 early branch 和 BackendType tail
              return。
            - 实测 `bash tests/verify_native_type_is_i32_like_scalar_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check`、
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          - [x] 迁入 `native_build_type_is_i32_like_scalar(...)` 的完整 body，使该 helper
            达到 body complete；复测真实 frontier 后再选择下一个 helper 或 callee。
            - 2026-06-11：新增
              `native_build_hosted_decl_can_materialize_type_is_i32_like_scalar_body`
              与 `native_build_hosted_coreir_append_type_is_i32_like_scalar_body`，用 2 条
              root statement 固定 i32 early branch 和 BackendType tail call return；
              hosted PortableMIR preflight 同步覆盖该 helper body function，并用
              `MIR_INST_OP_CALL` 保留 helper-to-helper call surface。
            - 同步 `lib/libc/syscall.uya` 的 Linux `sys_unlink` / `sys_readlink` /
              `sys_rename` fallback 路径使用 target-gated syscall number literal，避免旧
              build generator 发射裸 `SYS_unlink`、`SYS_readlink`、`SYS_rename` 导致
              host C 编译失败。
            - 实测 `make -B cmd-build UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya` 通过；
              self-build native preflight 前进为 `core_bodies=33`、`mir_body_functions=32`，
              新 frontier 为 `native_hosted_pending_body_frontier:
              function=native_build_type_is_byte decl=360 function_id=31 body_stmts=3
              reason=pending_core_body`。
          - [x] 为 `native_build_type_is_byte(...)` 补 CoreBody/PortableMIR body-complete
            合同；固定当前 3 statement body surface、null/type/name guard、
            `str_equals(..., "byte")` branch 和 tail `return 0`；不改生产 lowering。
            - 2026-06-11：新增 `tests/verify_native_type_is_byte_contract.sh`
              并接入 `tests/verify_native_cmd_build_stage1.sh`；
              `docs/native_cmd_build_subset.md` 新增
              `native_build_type_is_byte(...)` Body Complete Contract，冻结当前 3 statement
              body surface、guard、byte string compare branch 和 tail return。
            - 实测 `bash tests/verify_native_type_is_byte_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check`、
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          - [x] 迁入 `native_build_type_is_byte(...)` 的完整 body，使该 helper 达到 body
            complete；复测真实 frontier 后再选择下一个 helper 或 callee。
            - 2026-06-11：在 `src/build_compiler_driver.uya` 中加入
              `native_build_hosted_decl_can_materialize_type_is_byte_body(...)`、
              `native_build_hosted_coreir_append_type_is_byte_body(...)` 和对应
              PortableMIR const-return lowering；`lib/libc/syscall.uya` 同步把
              `sys_mkdir` / `sys_rmdir` fallback 改为 target-gated syscall number
              literal，避免旧 build generator 发射裸 `SYS_mkdir` / `SYS_rmdir`。
            - 实测 `make -B cmd-build UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya` 通过；
              self-build native preflight 前进为 `core_bodies=34`、`mir_body_functions=33`，
              新 frontier 为 `native_hosted_pending_body_frontier:
              function=native_build_type_is_byte_ptr decl=364 function_id=32 body_stmts=2
              reason=pending_core_body`。
          - [x] 为 `native_build_type_is_byte_ptr(...)` 补 CoreBody/PortableMIR
            body-complete 合同；固定当前 2 statement body surface、pointer guard 和
            tail `native_build_type_is_byte(...)` helper-call return；不改生产 lowering。
            - 2026-06-11：新增 `tests/verify_native_type_is_byte_ptr_contract.sh`
              并接入 `tests/verify_native_cmd_build_stage1.sh`；
              `docs/native_cmd_build_subset.md` 新增
              `native_build_type_is_byte_ptr(...)` Body Complete Contract，冻结当前
              2 statement body surface、pointer guard 和 pointed-type helper-call
              return。
            - 实测 `bash tests/verify_native_type_is_byte_ptr_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check`、
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          - [x] 迁入 `native_build_type_is_byte_ptr(...)` 的完整 body，使该 helper 达到
            body complete；复测真实 frontier 后再选择下一个 helper 或 callee。
            - 2026-06-11：在 `src/build_compiler_driver.uya` 中加入
              `native_build_hosted_decl_can_materialize_type_is_byte_ptr_body(...)`、
              `native_build_hosted_coreir_append_type_is_byte_ptr_body(...)` 和对应
              PortableMIR helper-call return lowering；`lib/libc/syscall.uya` 同步把
              `sys_getcwd` / `sys_kill` fallback 改为 target-gated syscall number
              literal，避免旧 build generator 发射裸 `SYS_getcwd` / `SYS_kill`。
            - 实测 `make -B cmd-build UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya` 通过；
              self-build native preflight 前进为 `core_bodies=35`、`mir_body_functions=34`，
              新 frontier 为 `native_hosted_pending_body_frontier:
          ...
          - [x] 迁入 `native_build_type_is_i32_ptr(...)` 的完整 body，使该 helper 达到
            ...
            - 实测 `make -B cmd-build UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya` 通过；
              ...
              reason=pending_core_body`。
          - [x] 为 `native_build_type_is_i32_like_ptr(...)` 补 CoreBody/PortableMIR
            body-complete 合同；固定当前 2 statement body surface、non-FFI/non-const
            pointer guard 和 tail `native_build_type_is_i32_like_scalar(...)` helper-call
            return；不改生产 lowering。
            - 2026-06-11：新增 `tests/verify_native_type_is_i32_like_ptr_contract.sh`
              并接入 `tests/verify_native_cmd_build_stage1.sh`；
              `docs/native_cmd_build_subset.md` 新增
              `native_build_type_is_i32_like_ptr(...)` Body Complete Contract，冻结当前
              2 statement body surface、pointer guard、FFI/const pointer 排除和
              pointed-type helper-call return。
            - 实测 `bash tests/verify_native_type_is_i32_like_ptr_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check`、
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          - [x] 为 `native_build_type_is_i32_array(...)` 补 CoreBody/PortableMIR
            body-complete 合同；固定当前 2 statement body surface、array guard、
            element `native_build_type_is_i32(...)` helper-call、size number/positive
            checks 和 tail `return 1`；不改生产 lowering。
            - 2026-06-11：新增 `tests/verify_native_type_is_i32_array_contract.sh`
              并接入 `tests/verify_native_cmd_build_stage1.sh`；
              `docs/native_cmd_build_subset.md` 新增
              `native_build_type_is_i32_array(...)` Body Complete Contract，冻结当前
              2 statement body surface、array guard、element helper-call、size number
              / positive checks 和 tail return。
            - 实测 `bash tests/verify_native_type_is_i32_array_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check`、
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          - [x] 迁入 `native_build_type_is_i32_array(...)` 的完整 body，使该 helper
            达到 body complete；复测真实 frontier 后再选择下一个 helper 或 callee。
            - 2026-06-11：在 `src/build_compiler_driver.uya` 中加入
              `native_build_hosted_decl_can_materialize_type_is_i32_array_body(...)`、
              `native_build_hosted_coreir_append_type_is_i32_array_body(...)` 和对应
              PortableMIR const-return lowering。
            - 实测 `make -B cmd-build UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya` 通过；
              self-build native preflight 前进为 `core_bodies=38`、`mir_body_functions=37`，
              新 frontier 为 `native_hosted_pending_body_frontier:
          ...
          - [x] 为 `native_build_decl_is_noarg_i32_fn(...)` 补 CoreBody/PortableMIR
            body-complete 合同；固定当前 3 statement body surface、decl shape guard、
            no-arg/non-extern/non-async/non-naked/non-varargs guard 和 tail
            `native_build_type_is_i32(...)` helper-call return；不改生产 lowering。
            - 2026-06-11：新增
              `tests/verify_native_decl_is_noarg_i32_fn_contract.sh` 并接入
              `tests/verify_native_cmd_build_stage1.sh`；`docs/native_cmd_build_subset.md`
              新增 `native_build_decl_is_noarg_i32_fn(...)` Body Complete Contract，
              冻结当前 3 statement body surface、两个 guard 和 tail return-type
              helper-call。
            - 红灯：新增合同脚本后先运行
              `bash tests/verify_native_decl_is_noarg_i32_fn_contract.sh`，按预期失败于
              `todo 缺少 native_build_decl_is_noarg_i32_fn 合同任务`。
            - 实测 `bash tests/verify_native_decl_is_noarg_i32_fn_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check`、
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          - [x] 迁入 `native_build_decl_is_noarg_i32_fn(...)` 的完整 body，使该 helper
            达到 body complete；复测真实 frontier 后再选择下一个 helper 或 callee。
            - 2026-06-11：在 `src/build_compiler_driver.uya` 中加入
              `native_build_hosted_decl_can_materialize_decl_is_noarg_i32_fn_body(...)`、
              `native_build_hosted_coreir_append_decl_is_noarg_i32_fn_body(...)`，
              并让已有一参数 helper-call MIR emitter 覆盖该 helper 的 tail
              `native_build_type_is_i32(decl.fn_decl_return_type)` call；`lib/libc/syscall.uya`
              同步把 Linux `sys_mmap` fallback 改为 target-gated syscall number literal，
              避免旧 build generator 发射裸 `SYS_mmap`。
            - 实测 `make -B cmd-build UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya` 通过；
              self-build native preflight 前进为 `core_bodies=40`、`mir_body_functions=39`，
              新 frontier 为 `native_hosted_pending_body_frontier:
              function=native_build_decl_is_one_i32_param_fn decl=388 function_id=38
              body_stmts=5 reason=pending_core_body`。
          - [x] 为 `native_build_decl_is_one_i32_param_fn(...)` 补 CoreBody/PortableMIR
            body-complete 合同；固定当前 5 statement body surface、decl shape guard、
            one-param/non-extern/non-async/non-naked/non-varargs/return-type/params guard、
            param local/guard 和 tail `native_build_type_is_i32(...)` helper-call
            return；不改生产 lowering。
            - 2026-06-11：新增
              `tests/verify_native_decl_is_one_i32_param_fn_contract.sh` 并接入
              `tests/verify_native_cmd_build_stage1.sh`；`docs/native_cmd_build_subset.md`
              新增 `native_build_decl_is_one_i32_param_fn(...)` Body Complete Contract，
              冻结当前 5 statement body surface、param local/guard 和 tail param-type
              helper-call。
            - 红灯：新增合同脚本后先运行
              `bash tests/verify_native_decl_is_one_i32_param_fn_contract.sh`，按预期失败于
              `todo 缺少 native_build_decl_is_one_i32_param_fn 合同任务`。
            - 实测 `bash tests/verify_native_decl_is_one_i32_param_fn_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check`、
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          ...
          - [x] 为 `native_build_decl_is_identity_generic_i32_fn(...)` 补
            CoreBody/PortableMIR body-complete 合同；固定当前 9 statement body surface、
            type-param guard/local、param guard/local、两个
            `native_build_type_named_equals(...)` helper-call guard 和 tail
            `native_build_decl_returns_param_directly(...)` helper-call return；不改生产
            lowering。
            - 2026-06-11：新增
              `tests/verify_native_decl_is_identity_generic_i32_fn_contract.sh` 并接入
              `tests/verify_native_cmd_build_stage1.sh`；`docs/native_cmd_build_subset.md`
              新增 `native_build_decl_is_identity_generic_i32_fn(...)`
              Body Complete Contract，冻结当前 9 statement body surface、两个 local、
              三个 helper-call surface 和 tail identity return。
            - 红灯：新增合同脚本后先运行
              `bash tests/verify_native_decl_is_identity_generic_i32_fn_contract.sh`，
              按预期失败于
              `todo 缺少 native_build_decl_is_identity_generic_i32_fn 合同任务`。
            - 实测
              `bash tests/verify_native_decl_is_identity_generic_i32_fn_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check`、
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          - [x] 迁入 `native_build_decl_is_identity_generic_i32_fn(...)` 的完整 body，使该
            helper 达到 body complete；复测真实 frontier 后再选择下一个 helper 或 callee。
            - 2026-06-11：在 `src/build_compiler_driver.uya` 中加入
              `native_build_hosted_decl_can_materialize_decl_is_identity_generic_i32_fn_body(...)`、
              `native_build_hosted_coreir_append_decl_is_identity_generic_i32_fn_body(...)`，
              并让 identity generic MIR emitter 覆盖该 helper 的 tail
              `native_build_decl_returns_param_directly(decl, param.var_decl_name)` return。
            - 同步 `lib/libc/syscall.uya` 的 Linux `sys_open` / `sys_close` /
              `sys_stat` fallback 使用 target-gated syscall number literal，避免旧
              build generator 发射裸 `SYS_open` / `SYS_close` / `SYS_stat`。
            - 实测 `make -B cmd-build UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya` 通过；
              self-build native preflight 前进为 `core_bodies=42`、`mir_body_functions=41`，
              新 frontier 为 `native_hosted_pending_body_frontier:
          ...
          - [x] 为 `native_build_decl_is_two_i32_param_fn(...)` 补 CoreBody/PortableMIR
            ...
            - 实测 `bash tests/verify_native_decl_is_two_i32_param_fn_contract.sh`、
              ...
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          - [x] 迁入 `native_build_decl_is_two_i32_param_fn(...)` 的完整 body，使该
            helper 达到 body complete；复测真实 frontier 后再选择下一个 helper 或 callee。
            - 2026-06-11：在 `src/build_compiler_driver.uya` 中加入
              `native_build_hosted_decl_can_materialize_decl_is_two_i32_param_fn_body(...)`、
              `native_build_hosted_coreir_append_decl_is_two_i32_param_fn_body(...)`，
              并让 type-is-i32-like-scalar MIR emitter 覆盖该 helper 的 tail
              `native_build_type_is_i32(param1.var_decl_type)` return。
            - 同步 `lib/libc/syscall.uya` 的 Linux `sys_write` / `sys_read`
              fallback 使用 target-gated syscall number literal，避免旧 build generator
              发射裸 `SYS_write` / `SYS_read`。
            - 实测 `make -B cmd-build UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya` 通过；
              self-build native preflight 前进为 `core_bodies=43`、`mir_body_functions=42`，
              新 frontier 为 `native_hosted_pending_body_frontier:
              function=native_build_decl_is_extern_two_i32_param_fn decl=400 function_id=41
              body_stmts=7 reason=pending_core_body`。
          - [x] 为 `native_build_decl_is_extern_two_i32_param_fn(...)` 补
            CoreBody/PortableMIR body-complete 合同；固定当前 7 statement body
            surface、two-param extern guard、param0/param1 local、param shape guard、
            param0 type helper-call guard 和 tail param1 type helper-call return；不改生产
            lowering。
            - 2026-06-11：新增
              `tests/verify_native_decl_is_extern_two_i32_param_fn_contract.sh` 并接入
              `tests/verify_native_cmd_build_stage1.sh`；`docs/native_cmd_build_subset.md`
              新增 `native_build_decl_is_extern_two_i32_param_fn(...)` Body Complete
              Contract，冻结当前 7 statement body surface、extern-required guard、两个
              param local 和 tail param1 type helper-call。
            - 红灯：新增合同脚本后先运行
              `bash tests/verify_native_decl_is_extern_two_i32_param_fn_contract.sh`，按预期失败于
              `subset doc 缺少 native_build_decl_is_extern_two_i32_param_fn 合同`。
            - 实测 `bash tests/verify_native_decl_is_extern_two_i32_param_fn_contract.sh`、
              `bash tests/verify_native_cmd_build_stage1.sh`、`git diff --check`、
              `python3 ./.agents/skills/goal-task-runner/scripts/check_todo.py docs/todo_compiler_1s.md`
              均通过。
          - [ ] 迁入 `native_build_decl_is_extern_two_i32_param_fn(...)` 的完整 body，使该
            helper 达到 body complete；复测真实 frontier 后再选择下一个 helper 或 callee。
          - [x] 迁入 `native_build_type_is_i32_like_ptr(...)` 的完整 body，使该 helper
            达到 body complete；复测真实 frontier 后再选择下一个 helper 或 callee。
            - 2026-06-11：在 `src/build_compiler_driver.uya` 中加入
              `native_build_hosted_decl_can_materialize_type_is_i32_like_ptr_body(...)`、
              `native_build_hosted_coreir_append_type_is_i32_like_ptr_body(...)` 和对应
              PortableMIR helper-call return lowering；`lib/libc/syscall.uya` 同步把
              `sys_getpid` / `sys_fork` fallback 改为 target-gated syscall number
              literal，避免旧 build generator 发射裸 `SYS_getpid` / `SYS_fork`。
            - 实测 `make -B cmd-build UYA_CMD_BOOTSTRAP_COMPILER=./bin/uya` 通过；
              self-build native preflight 前进为 `core_bodies=37`、`mir_body_functions=36`，
              新 frontier 为 `native_hosted_pending_body_frontier:
              function=native_build_type_is_i32_array decl=376 function_id=35 body_stmts=2
              reason=pending_core_body`。
