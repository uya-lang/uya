...
- [ ] `MIR-C99-FULL-SUPPORT-CLI-SUITE`: 让真实 `--mir-c99` CLI 在主语言测试集上收敛。
  - 当前基线（2026-06-24）：将当前仓库现成 full `bin/cmd/build` 同步到 sibling
    ...
    cmd/build` 的真实 build CLI 入口已恢复，current-source `cmd/build` host C compile 不再卡
    裸 `O_RDONLY` / `SYS_*` 常量；下一挡剩余 blocker 是 HelloWorld `--mir-c99` 仍缺少
    `[MIR-C99]` / writer banner。
