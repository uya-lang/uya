...
- [ ] `MIR-C99-FULL-SUPPORT-CLI-SUITE`: 让真实 `--mir-c99` CLI 在主语言测试集上收敛。
  - 当前基线（2026-06-24）：固定 `../uya/bin/uya` 默认仍先委托 sibling
    `../uya/bin/cmd/build` 的 minimal candidate，直接卡在
    `error: MIR-C99 unit output candidate currently supports --help and minimal build smoke only`；
    临时同步当前仓库现成 full `bin/cmd/build` 后，HelloWorld `--mir-c99` 仍缺少
    `[MIR-C99]` / writer banner，current-source `cmd/build` candidate host C compile 继续卡
    裸 `O_RDONLY` / `SYS_*` 常量。
  - [ ] `MIR-C99-FULL-SUPPORT-CLI-SUITE-BUILD-ENTRY-RECOVERY`: 恢复 fixed/current-source
  ...
  - [ ] `MIR-C99-FULL-SUPPORT-CLI-SUITE-HELLOWORLD-REAL-ROUTE`: 让 fixed/current-source CLI
  ...
  - [ ] `MIR-C99-FULL-SUPPORT-CLI-SUITE-SRC-MAIN-DISTINCT-OUTPUTS`: 让 `src/main.uya`
