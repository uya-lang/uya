...
- [ ] `MIR-C99-FULL-SUPPORT-CLI-SUITE`: 让真实 `--mir-c99` CLI 在主语言测试集上收敛。
  ...
  - [ ] `MIR-C99-FULL-SUPPORT-CLI-SUITE-BUILD-ENTRY-RECOVERY`: 先恢复 fixed/current-source
    ...
    - 验收：
      - `make -B cmd-build UYA_CMD_BOOTSTRAP_COMPILER=../uya/bin/uya` 通过，或等价 fixed
        compiler bootstrap 链通过。
      - `../uya/bin/uya build --mir-c99 examples/HelloWorld.uya -o <tmp>.c` 日志出现
        `[MIR-C99]`，输出含 `generated by MIR-C99 unit output writer`。
