...
- [ ] MIR-C99-BACKEND-CALLS：实现 MIR call 到 C call。
  ...
  - [ ] parity shard：multi-arg call、extern object call、method dispatch、generic instance call。
    ...
    - [ ] extern object call 真实 MIR-C99 / 现有 C99 oracle parity。
...
- [ ] MIR-C99-BACKEND-ATOMICS：显式处理 `atomic T`，不把 C99 当作隐式原子语义提供者。
  ...
  - [ ] parity/reject shard：atomic i32 init/write/read；支持前可明确 reject，支持后必须与 oracle 行为一致。
    - 阻塞：当前无真实 MIR-C99 generator command/CLI，`bash tests/verify_mir_c99_oracle_parity_harness.sh` 仅报告 `generator commands are pending backend hookup`；atomic capability diagnostic 与 explicit reject plan 已验证，但尚不能执行 source-level MIR-C99 reject/parity shard。
