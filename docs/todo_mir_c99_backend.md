...
- 当前 parity frontier（2026-06-15）：candidate 已从 `--help` smoke 前进到 `return_literal_c99_output_parity`，可接受最小 `build <input.uya> -o <output>`，解析 literal return，产出可运行程序，并与现有 C99 oracle 的 stdout/stderr/exit code 对齐；log/summary 仍固定 `compiler_regression_status=not_yet_run`、`full_language_backend_parity_status=not_yet_run`，不得作为完整 parity 完成证据。
...
- [ ] MIR-C99-BACKEND-SELF-BUILD-CANDIDATE：生成真实 MIR-C99 compiler candidate。
  ...
  - [~] MIR-C99-built compiler 复跑 compiler regression、C99 output parity 和 full-language backend parity。
    - 当前进展：`bash tests/verify_mir_c99_cmd_build_parity_frontier_gate.sh` 已通过，证明 MIR-C99 `cmd/build` candidate 能接受最小 build smoke、解析 literal return，并与现有 C99 oracle 的 stdout/stderr/exit code 对齐；同一 gate 同时确认 compiler regression 和 full-language backend parity 仍为 `not_yet_run`，本叶尚未完成。
