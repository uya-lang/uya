...
> 尤其要注意以下几点：
> 1. 动态化并非全部完成：`TaskQueue<T>` 默认队列已从 legacy `64` 起步并在 `push()` 时自动增长，`LinuxEpoll` 也会在满载时自动扩容 slot / lookup / poll scratch；但 `ThreadPool` worker / pending / task slot 容量、`AsyncFramePool` bucket 默认上限等边界仍未完全清零。
> 2. 当前 `@async_fn` 的已通过回归不等于已经覆盖完整 Uya 函数体语法；现阶段应把 `tests/verify_async_full_language_matrix.sh` 视为高价值语法快照入口，而不是“完整语法已收口”的单独证明。
> 3. 本文中的“设计原则”包含目标态表述；评估当前真实状态时，应以源码、测试和权威 TODO 为准。
> 4. HTTP、DNS、TLS、`async_compute` 与 `Scheduler` 的分散回归只能证明各自阶段性主路径，尚不能当作“共享同一套稳定 async runtime 语义”已经统一验收；当前共享 runtime 发布闸门入口应以 `tests/async_shared_runtime_mix.sh` 和相关专项 smoke 为准。
...
**当前实现快照**：`lib/std/async.uya` 提供 **`struct Waker`**（`wake/reset/is_woken`、单次 `fd + interest` I/O 注册请求、`eventfd` 绑定/关闭、`cancel/is_cancelled`，以及 event-loop / cancel / 显式 wake 来源标记）、**`union Poll<T>`**（Ready/Pending）、**`interface Future<T>`**、**`struct Future<T>`**（含 `state: Poll<T>`、`fn poll(...) Poll<T>`）、**`struct Task<T> : Future<T>`**（含 `task_ready`、`poll`）。统一异步错误现已收敛到 `EventLoopSlotsFull` / `TaskQueueFull` / `SchedulerStopped` / `FutureNotReady` / `AsyncFramePoolFull` / `Cancelled` / `WriteZero`。针对常见标量 **`i32` / `u32` / `usize`** 的 **`Future<!T>`** 路径，已导出 **`poll_ready_ok_*` / `future_ready_ok_*` / `task_ready_ok_*`**，便于自定义 `Future` 与测试构造 `Ready(ok(...))`。`std.async.io` 当前已收敛到 **`Future<!usize>`** 主路径：`MemAsyncWriter` / `MemAsyncReader` 已支持 `write` / `write_all` / `flush` 与 `read` / `read_exact`，`AsyncFd` 已支持 `read` / `read_exact` / `write` / `write_all` / `flush`；helper 层也已补上 **`async_write_bytes`**、**`async_write_cstr`**、**`async_print_to`**、**`async_println_to`**（含 bytes 变体）。其中 `AsyncFd` 在 `poll()` 时确保 `O_NONBLOCK`，并将 `EAGAIN` / `EWOULDBLOCK` 映射为 `Poll.Pending`；此时 future 会把读/写关注记录到 `Waker`，由 `Scheduler` 通过 `EventLoop.register()/poll()/deregister()` 驱动下一轮唤醒。`Scheduler` 现已同时提供 `scheduler_run_*_with_event_loop`、`scheduler_run_pair_i32_with_event_loop`、泛型 **`TaskQueue<T>`** / `TaskQueue_i32` / `TaskQueue_u32` / `TaskQueue_usize` 与 **`scheduler_run_task_queue_with_event_loop<T>`** / typed wrappers；默认 `TaskQueue<T>` 以 legacy `64` 为起步容量并在 `push()` 时自动增长，`task_queue_new_with_capacity<T>()` 保留调用方显式上限；`Pending` 时会为每个 `Waker` 同步 `eventfd + io fd` 注册，支持 Linux 跨线程/跨执行上下文 wake。`Scheduler` 的 frame pool backing buffer 与 inline repoll 策略也已支持实例级配置以及 `UYA_SCHEDULER_FRAME_BUFFER_BYTES` / `UYA_SCHEDULER_INLINE_REPOLL_LIMIT` 环境变量。取消语义在已覆盖路径中走通：`Waker.cancel()` / `TaskQueue.cancel()` 会把取消位显式传给 future，调度器在取消或完成时统一清理 `eventfd`、I/O 注册与 slot 资源，并以 `error.Cancelled` 写回结果路径。`std.thread.async_compute<T>` 已在自身回归中复用这一链路：未启动/排队/one-shot 任务可立即取消，已运行共享槽任务会在结果回收时稳定返回 `error.Cancelled`；`ThreadPoolMetrics` 现已暴露 queue depth / running workers / saturation count，`std.async_scheduler.async_runtime_metrics_linux_epoll_with_thread_pool()` 可把这些 thread pool 指标与 scheduler frame/queue 指标、Linux epoll 注册指标，以及 timeout wait / timeout result / cancel result / wake 来源（inline、event-loop、cancel）计数汇总到统一快照。`LinuxEpoll` 当前会在 `register()` 命中 slot 满载时自动扩容 slot / lookup / poll scratch 存储，并暴露当前注册数、历史峰值和扩容次数指标。`poll()` 命中后按 fd 查找已注册 `Waker` 并调用带来源标记的唤醒。`lib/std/http/http1_async.uya` 的请求头构建已按 required capacity 分配堆 buffer，响应头 buffer 也会按需 grow；这一 readiness 路径同时被 HTTP/1.1 客户端的 nonblocking connect/read/write 复用。

**阶段性收口口径**：以上能力仍是阶段性覆盖。HTTP、DNS、TLS、`async_compute` 与 `Scheduler` 尚未通过同一共享 runtime 矩阵统一验收，`ThreadPool` / `AsyncFramePool` 等剩余固定容量策略、多 interest、跨平台后端、TLS async 化和组合取消语义仍是边界。语言/编译器侧现已支持把 `@async_fn` 放在结构体内部方法、外部方法块以及接口方法签名上，因此基于接口的 async 抽象可以直接通过 vtable 分派 `Future<!T>`；固定回归见 `tests/test_async_method_interface.uya`。编译器侧已补齐两处直接支撑队列的 codegen 能力：数组元素上的接口字段方法调用（如 `queue.slots[i].future.poll(...)`）可正确保留接口类型；结构体字段依赖收集也会跳过接口类型，避免误展开 `struct Future<T>` 模板。`test_async_await_parse.uya`、`test_task_std_async.uya`、`test_async_return_value.uya`、`test_async_nested.uya`、`test_std_async_waker.uya`、`test_async_io.uya`、`test_async_fd.uya`、`test_async_copy.uya`、`test_std_async_event.uya`、`test_std_async_scheduler.uya`、`test_async_task_queue_dynamic_growth.uya`、`test_async_multi_fd_concurrent.uya`、`test_async_event_dynamic_growth.uya`、`test_async_event_config.uya`、`test_std_thread.uya`、`test_async_compute_types.uya`、`test_async_compute_dynamic_resource_pressure.uya`、`test_async_thread_pool_dynamic_growth.uya`、`test_async_method_interface.uya` 已通过相关 `--c99` 回归；这些分散测试证明对应子能力，不单独证明完整量产。当前建议把 `tests/verify_async_full_language_matrix.sh` 视为语法快照入口，把 `tests/async_shared_runtime_mix.sh` 视为共享 runtime 发布闸门入口；二者都只是阶段闸门，不单独代表“生产完成”。

**@async_fn 中可直接 `return T`**：无 `@await` 时自动包装为 `Future<T>{ state: Poll<T>.Ready(expr) }`，poll 立即返回 Ready。Checker 对单态/泛型名做基名匹配（如 `Future<T>`、`Future_i32` 可解析为接口/结构体 `Future`），方法解析失败时回退到基名查找。**nested future 的当前状态**：`Future<Future<T>>` 这个值类型本身可以双层 poll，`tests/test_async_nested.uya` 已固定手工构造的 `Future<Future<i32>>` 与单层 `!Future<i32>` handoff 两条正向回归；无 await 的 `@async_fn` 若返回 `!Future<Future<T>>`，且 `return` 表达式里同步 `try` 另一个 `!Future<T>`，当前也已通过 `tests/test_async_nested_future_poll.uya` 与 `tests/verify_async_nested_future_boundary.sh` 固定为 C99 发射和宿主 C 编译正向回归。编译器在结构体含泛型 union 字段时会先输出该 union 的单态定义（如 `Poll_i32`、`uya_tagged_Poll_i32`），且通过 arena 持久化 tagged 名避免重定义。**无 await 且返回 `!Future<T>` 的 @async_fn**：状态机形态为 `Future<!Future<T>>`，其 `struct uya_interface_*` / `struct uya_vtable_*` 在 `src/codegen/c99/function.uya` 中按需生成（不经过 `mono_instances`），并用 `is_struct_defined` 避免重复定义。以下为**目标**目录结构，后续按阶段拆分实现。
...
- `Scheduler` 已有单任务、双任务与共享 `EventLoop` 的泛型 `TaskQueue<T>` 入口；默认队列会从 legacy `64` 起步并自动增长，`task_queue_new_with_capacity<T>()` 保留调用方显式上限
- `Scheduler` 的 frame pool backing buffer 与 inline repoll 策略已支持实例级配置，以及 `UYA_SCHEDULER_FRAME_BUFFER_BYTES` / `UYA_SCHEDULER_INLINE_REPOLL_LIMIT` 环境变量
- `LinuxEpoll.poll()` 已能在事件命中后唤醒对应 `Waker`
- HTTP/1.1 async 客户端的请求头 / 响应头 buffer 已改为按需分配或 grow，不再由 `4096` 级别的 scratch 常量直接决定请求是否可发起
...
- [x] **Linux 实现**（`lib/std/async_event.uya`，`struct LinuxEpoll : EventLoop`）：
  ...
  - **运行时指标**：测试/调试口径已暴露 `linux_epoll_registered_count()`、`linux_epoll_peak_registered_count()`、`linux_epoll_resize_count()`，以及统一 runtime 快照里的 timeout wait / timeout result / cancel result / wake 来源计数
...
- [~] **Scheduler**：
  ...
  - 基于事件循环实现
  - 零堆分配，栈分配状态机
  - 当前已覆盖单任务、双任务与共享 `EventLoop` 的泛型 `TaskQueue<T>` 主路径
  - 默认队列从 legacy `64` 起步并自动增长；`task_queue_new_with_capacity<T>()` 保留显式上限队列
  - frame pool backing buffer 与 inline repoll 策略已支持实例级配置，以及 `UYA_SCHEDULER_FRAME_BUFFER_BYTES` / `UYA_SCHEDULER_INLINE_REPOLL_LIMIT`
  - `Pending` 时会同步注册 `eventfd + io fd`，并在完成或取消后统一清理注册资源
  - 共享 runtime 生产收口仍缺 HTTP/DNS/TLS/`async_compute` 同队列 smoke，`ThreadPool` / `AsyncFramePool` 动态资源边界也尚未全部清零
  - 后续管理所有 `Task<T>` 的生命周期
...
- 语法快照基线：`tests/verify_async_full_language_matrix.sh`
- nested future / wrapper 边界：`tests/verify_async_nested_future_boundary.sh`
- shared runtime 发布闸门入口：`tests/async_shared_runtime_mix.sh`
- 资源动态化专项：`tests/test_async_task_queue_dynamic_growth.uya`、`tests/test_async_event_dynamic_growth.uya`、`tests/test_async_thread_pool_dynamic_growth.uya`、`tests/test_async_compute_dynamic_resource_pressure.uya`

这些入口的作用是把“阶段性能力证明”与“生产完成证明”分开：它们能证明当前设计文档提到的关键链路仍有真实验证，但在共享 runtime 矩阵、剩余固定容量资源和跨平台后端补齐前，仍不能单独升级为“async 生产化已完成”。
