...
- process-only MVP 只包含外部进程 stage，不包含自定义 Uya 流 stage；自定义 Uya stage 必须在 owned-data 规则确定，并且具备内存安全强制 task 终止或隔离 worker process 等满足取消/终止门槛的 execution domain 方案后，才能进入公共 API
...
- PATH 查找语义由 `std.process` 内部的平台 helper `process_resolve_path(program, env, cwd)` 执行，并复用 `std.path` 的平台原语；pipeline executor 只消费解析后的绝对 `exec_path`，不得在 executor 内再实现一套 PATH 搜索逻辑。具体接口与规则见 [`typed_pipeline_design.md`](typed_pipeline_design.md)。
