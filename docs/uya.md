...
- **联合体支持**：
  - 添加 `union` 关键字定义标签联合体（有运行时 `_tag` 字段）
  - 添加 `extern union` 声明外部 C 联合体（无 tag，C 兼容）
  - 编译期标签跟踪优化函数内访问
  - 强制模式匹配访问，处理所有变体
  - 支持联合体方法和接口实现
  - 详细错误信息指导正确使用
...
- **联合体**（第 4.5 章）：`union` 有 tag 支持 match；`extern union` 无 tag 与 C 兼容
...
| 类型 | 语法 | 内存布局 | 特点 |
|------|------|---------|------|
| 标签联合体 | `union U { ... }` | `struct { _tag; union u; }` | 有 tag，支持 match |
| 外部联合体 | `extern union U { ... }` | `union U { ... }` | 无 tag，C 兼容 |

- **安全访问**：标签联合体通过 match 访问；extern union 直接访问（程序员保证安全）
- **编译期标签跟踪**：函数内可通过编译期追踪直接访问标签联合体
- **C 互操作**：`extern union` 与 C union 内存布局完全相同
- **示例**：
```uya
union IntOrFloat {
    i: i32,
...
| 类型 | 语法 | 特点 | 用途 |
|------|------|------|------|
| **标签联合体** | `union U { ... }` | 有运行时 `_tag`，支持 match | Uya 内部类型安全联合体 |
| **外部联合体** | `extern union U { ... }` | 无 tag，与 C 兼容 | C FFI 互操作 |

**设计目标**：
- **类型安全**：标签联合体通过 `_tag` + match 确保类型安全
- **编译期优化**：函数内可通过编译期标签跟踪直接访问（跳过 match）
- **C 互操作**：`extern union` 与 C union 内存布局完全相同
- **显式控制**：强制模式匹配，无隐式类型转换
...
#### 4.5.6.1 extern union（外部 C 联合体）

`extern union` 用于声明外部 C 联合体，与 C 完全兼容：
...
**限制**：
- ❌ **不支持 match**：没有 `_tag` 字段，无法运行时判断变体
- ❌ **不能定义方法**：纯 C 布局，无方法
- ✅ **可直接访问变体**：程序员自行保证安全（像 C 一样）
- ✅ **与 C union 内存布局完全相同**

```uya
extern fn get_c_value() union CValue;  // C 函数返回 union
...
Uya 联合体使用 tagged 布局，传递给 C 需要转换：
...
```uya
// Uya 端
union IntOrFloat { i: i32, f: f64 }

// 传给 C 函数（需要 C 端理解 tagged 布局）
extern fn c_process(data: *void) void;
...
```c
// Uya: union IntOrFloat { i: i32, f: f64 }
// 生成的 C 代码：
union IntOrFloat {
    int32_t i;
    double f;
...
```c
// Uya: extern union CValue { i: i32, f: f64 }
// 生成的 C 代码：
union CValue {
    int32_t i;
    double f;
};
// 无 tagged 包装，与 C union 完全相同
```
...
4. **变体类型限制**（计划中）：变体类型不建议包含引用（`&T`）或切片（`&[T]`），可能导致生命周期问题
   - > **实现状态**：当前编译器未强制检查此限制，建议开发者自行避免
...
fn main() !i32 {
    ...
    const value2 = process_result(failure) catch |e| {
        printf("捕获错误: %v\n", e);
        return 1;
    };
...
**标签联合体（普通 union）**：
1. ✅ **程序员提供证明**：通过 match 显式处理所有变体
2. ✅ **编译器验证证明**：验证 match 完全性和类型一致性
3. ✅ **运行时安全**：通过 `_tag` 字段确保正确的变体访问
4. ✅ **编译期优化**：函数内标签跟踪，允许直接访问
5. ✅ **显式控制**：强制 match 或编译期证明的直接访问

**extern union（C 互操作）**：
1. ✅ **C 兼容性**：内存布局与 C union 完全相同
2. ✅ **零运行时开销**：无 `_tag` 字段
3. ⚠️ **程序员负责安全**：直接访问变体，无编译期保护

**一句话总结**：
- **普通 union** = Rust enum 风格的类型安全联合体（有 tag）
- **extern union** = C union 风格的 FFI 互操作联合体（无 tag）
...
> **实现状态（v0.48）**：原子类型 `atomic T` 已实现。完整的并发安全证明（Send/Sync trait 推导、跨线程验证）**待实现**。
...
> **实现状态（v0.48）**：语法解析已完成（`@async_fn`、`@await`），CPS 变换和状态机生成**待实现**（v0.7.0 计划）。当前可解析异步函数语法，但不会生成状态机代码。
...
- **联合体（union）**：一种复合类型，可以存储多种变体中的一种。Uya 有两种：`union U`（有运行时 `_tag`）和 `extern union U`（无 tag，C 兼容）。

- **变体（variant）**：联合体中的一种可能类型。每个变体有名称和类型，如 `IntOrFloat.i` 中的 `.i` 变体。

- **编译期标签跟踪**：Uya 编译器在函数内跟踪标签联合体当前活跃的变体标签，允许直接访问（无需 match）。

- **模式匹配（pattern matching）**：访问标签联合体的主要方式，通过 `match` 表达式处理所有可能的变体。
