...
- **词法 · shebang**：若源文件首两个字节为 **`#!`**，首行按 shebang 注释处理；该规则仅允许出现在文件开头。`uya run` 可直接运行带 shebang 的 `.ush` / `.uya` 源文件。**回归**：`tests/verify_run_shebang_ush.sh`。

### 0.49.51（2026-06-21）

- **async cleanup 限制显式化**：`@await` 不能出现在 `defer` / `errdefer` 清理块中；编译器在 checker 阶段给出稳定诊断，避免落到后端崩溃。
...
- 文件编码 UTF-8，Unix 换行 `\n`。
- 若文件首两个字节为 `#!`，第一行按 POSIX shebang 注释处理；该规则仅在文件开头生效，用于 `uya run path/to/script.ush` 等脚本入口。
...
- 必须使用 `try` 关键字处理错误传播
- 不能出现在 `defer` / `errdefer` 块中；清理逻辑必须保持同步
