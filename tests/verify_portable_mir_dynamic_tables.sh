...
fn append_portable_mir_dynamic_row(module: &PortableMirModule, id: i32) !void {
    ...
    var async_meta: MirAsyncFrameMeta = MirAsyncFrameMeta{
        async_frame_meta_id: id,
        function_id: id,
        frame_type_id: id,
        slot_kind: MIR_ASYNC_FRAME_SLOT_CAPTURED_LOCAL,
        slot_index: id,
        slot_type_id: id,
        captured_local_id: id,
        await_child_slot_index: id,
        result_slot_index: id,
        state_tag: id,
        poll_block_id: id,
        resume_block_id: id,
        debug_loc_id: id,
