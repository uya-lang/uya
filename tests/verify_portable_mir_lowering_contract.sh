...
test "PortableMIR lowering contract covers full language CoreBody features" {
    ...
    try assert_eq_i32(portable_mir_lowering_mask_has(portable_mir_lowering_feature_for_stmt_kind(CORE_STMT_KIND_BLOCK), MIR_LOWER_FEATURE_CONTROL_FLOW), 1);
    try assert_eq_i32(portable_mir_lowering_mask_has(portable_mir_lowering_feature_for_stmt_kind(CORE_STMT_KIND_BREAK), MIR_LOWER_FEATURE_CALL_RETURN_BRANCH), 1);
    try assert_eq_i32(portable_mir_lowering_mask_has(portable_mir_lowering_feature_for_stmt_kind(CORE_STMT_KIND_CONTINUE), MIR_LOWER_FEATURE_CALL_RETURN_BRANCH), 1);
...
(cd "$REPO_ROOT" && UYA_ROOT="$REPO_ROOT" ../uya/bin/uya test "$tmp_dir/main.uya" --no-split-c)
