...
require_pattern "$log_file" '^completed_coverage=generic_corebody_type_is_i32_body_lowering$' \
    "log records the migrated type_is_i32 body lowering"
require_pattern "$log_file" '^completed_body_detail=native_hosted_reachable_body_complete:function=native_build_type_is_i32,prefix_stmts=3,reason=body_complete$' \
    "log records the completed type_is_i32 helper body"
...
require_pattern "$summary_file" "^MIR_C99_COMPLETED_COVERAGE='generic_corebody_type_is_i32_body_lowering'$" \
    "summary sidecar records the migrated type_is_i32 body lowering"
require_pattern "$summary_file" "^MIR_C99_COMPLETED_BODY_DETAIL='native_hosted_reachable_body_complete:function=native_build_type_is_i32,prefix_stmts=3,reason=body_complete'$" \
    "summary sidecar records the completed type_is_i32 helper body"
require_pattern "$summary_file" "^MIR_C99_FRONTIER_DETAIL='native_hosted_pending_body_frontier:function=native_build_type_is_usize,decl=[0-9]+,function_id=[0-9]+,body_stmts=3,reason=pending_core_body'$" \
...
require_pattern "$summary_file" "^MIR_C99_NEXT_COVERAGE='generic_corebody_type_is_usize_body_lowering'$" \
...
require_pattern "$COVERAGE_DOC" 'MIR-C99 self-build.*generic_corebody_type_is_i32_body_lowering.*已纳入 CoreBody -> PortableMIR lowering' \
    "coverage matrix records the migrated type_is_i32 body lowering"
require_pattern "$COVERAGE_DOC" 'MIR-C99 self-build.*当前 completed body detail 包含 `native_build_type_is_i32` body-complete，下一处 frontier 为 `native_build_type_is_usize`/`generic_corebody_type_is_usize_body_lowering`' \
    "coverage matrix records the type_is_i32 completion and next type_is_usize frontier"
