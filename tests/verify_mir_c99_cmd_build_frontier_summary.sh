...
require_pattern "$log_file" '^completed_body_detail=native_hosted_reachable_body_complete:function=native_build_type_is_usize,prefix_stmts=3,reason=body_complete$' \
    "log records the completed type_is_usize helper body"
require_pattern "$log_file" '^frontier_detail=native_hosted_pending_body_frontier:function=native_build_type_named_equals,decl=[0-9]+,function_id=[0-9]+,body_stmts=3,reason=pending_core_body$' \
...
require_pattern "$log_file" '^next_coverage=generic_corebody_type_named_equals_body_lowering$' \
...
require_pattern "$summary_file" "^MIR_C99_COMPLETED_BODY_DETAIL='native_hosted_reachable_body_complete:function=native_build_type_is_usize,prefix_stmts=3,reason=body_complete'$" \
    "summary sidecar records the completed type_is_usize helper body"
require_pattern "$summary_file" "^MIR_C99_FRONTIER_DETAIL='native_hosted_pending_body_frontier:function=native_build_type_named_equals,decl=[0-9]+,function_id=[0-9]+,body_stmts=3,reason=pending_core_body'$" \
...
require_pattern "$summary_file" "^MIR_C99_NEXT_COVERAGE='generic_corebody_type_named_equals_body_lowering'$" \
...
require_pattern "$COVERAGE_DOC" 'MIR-C99 self-build.*当前 completed body detail 包含 `native_build_type_is_usize` body-complete，下一处 frontier 为 `native_build_type_named_equals`/`generic_corebody_type_named_equals_body_lowering`' \
    "coverage matrix records the type_is_usize completion and next type_named_equals frontier"
