...
require_pattern "$log_file" '^completed_coverage=generic_corebody_simd_vector_mask_shape_empty_struct_return_lowering$' \
    "log records the migrated simd vector mask empty struct return lowering"
require_pattern "$log_file" '^completed_body_detail=native_hosted_reachable_body_complete:function=native_build_simd_vector_mask_shape_empty,prefix_stmts=1,reason=body_complete$' \
    "log records the completed simd vector mask helper body"
require_pattern "$log_file" '^frontier_detail=native_hosted_pending_body_frontier:function=native_build_local_table_init,decl=433,function_id=59,body_stmts=15,reason=pending_core_body$' \
...
require_pattern "$log_file" '^next_coverage=generic_corebody_local_table_init_body_lowering$' \
...
require_pattern "$summary_file" "^MIR_C99_COMPLETED_COVERAGE='generic_corebody_simd_vector_mask_shape_empty_struct_return_lowering'$" \
    "summary sidecar records the migrated simd vector mask empty struct return lowering"
require_pattern "$summary_file" "^MIR_C99_COMPLETED_BODY_DETAIL='native_hosted_reachable_body_complete:function=native_build_simd_vector_mask_shape_empty,prefix_stmts=1,reason=body_complete'$" \
    "summary sidecar records the completed simd vector mask helper body"
require_pattern "$summary_file" "^MIR_C99_FRONTIER_DETAIL='native_hosted_pending_body_frontier:function=native_build_local_table_init,decl=433,function_id=59,body_stmts=15,reason=pending_core_body'$" \
...
require_pattern "$summary_file" "^MIR_C99_NEXT_COVERAGE='generic_corebody_local_table_init_body_lowering'$" \
...
require_pattern "$COVERAGE_DOC" 'MIR-C99 self-build.*generic_corebody_simd_vector_mask_shape_empty_struct_return_lowering.*已纳入 CoreBody -> PortableMIR lowering' \
    "coverage matrix records the migrated simd vector mask empty struct return lowering"
require_pattern "$COVERAGE_DOC" 'MIR-C99 self-build.*当前 completed body detail 包含 `native_build_simd_vector_mask_shape_empty` body-complete，下一处 frontier 为 `native_build_local_table_init`/`generic_corebody_local_table_init_body_lowering`' \
    "coverage matrix records the simd completion and next local-table frontier"
...
require_pattern "$COVERAGE_DOC" 'MIR-C99 self-build.*generic_corebody_parse11_pointer_out_param_lowering.*已纳入 CoreBody -> PortableMIR lowering' \
    "coverage matrix records the migrated two pointer-param helper slice"
require_pattern "$COVERAGE_DOC" 'MIR-C99 self-build.*generic_corebody_parse11_pointer_out_param_lowering' \
