...
require_pattern "$log_file" '^next_coverage=compile_stats_record_and_release_typed_program_stmt12_table_used_bytes$' \
...
require_pattern "$summary_file" "^MIR_C99_FRONTIER_DETAIL='native_hosted_reachable_body_frontier:function=compile_stats_record_and_release_typed_program,prefix_stmts=12,next_stmt=12,next_kind=AST_ASSIGN,reason=partial_core_body'$" \
...
require_pattern "$summary_file" "^MIR_C99_NEXT_COVERAGE='compile_stats_record_and_release_typed_program_stmt12_table_used_bytes'$" \
...
require_pattern "$COVERAGE_DOC" 'MIR-C99 self-build.*compile_stats_record_and_release_typed_program_stmt12_table_used_bytes' \
