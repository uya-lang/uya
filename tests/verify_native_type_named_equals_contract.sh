...
require_pattern "$SUBSET_DOC" 'type_node\.type != ASTNodeType\.AST_TYPE_NAMED' \
    "subset doc 缺少 named type guard"
require_pattern "$SUBSET_DOC" 'type_node\.type_named_name == null' \
    "subset doc 缺少 type name guard"
require_pattern "$SUBSET_DOC" 'str_equals\(type_node\.type_named_name as \*byte, name as \*byte\) != 0' \
    "subset doc 缺少 str_equals name branch"
require_pattern "$SUBSET_DOC" 'native_hosted_reachable_body_complete: function=native_build_type_named_equals prefix_stmts=3 reason=body_complete' \
    "subset doc 缺少 body-complete frontier"

require_pattern "$BUILD_DRIVER_SRC" 'fn native_build_type_named_equals\(type_node: &ASTNode, name: &byte\) i32' \
    "源码缺少 native_build_type_named_equals helper"
require_pattern "$BUILD_DRIVER_SRC" 'str_equals\(type_node\.type_named_name as \*byte, name as \*byte\) != 0' \
    "源码缺少 str_equals name branch"
require_pattern "$STAGE1_TEST" 'verify_native_type_named_equals_contract\.sh' \
    "stage1 未纳入 native_build_type_named_equals 合同"

echo "verify_native_type_named_equals_contract: ok"
