...
"$ROOT_DIR/bin/uya" microapp build --microapp-profile linux_x86_64_hardvm examples/microapp/microcontainer_hello_source.uya -o "$POBJ" >/tmp/verify_microapp_profile_cli_build.log 2>&1

"$ROOT_DIR/bin/uya" microapp inspect "$POBJ" >"$INSPECT_LOG" 2>&1
...
"$ROOT_DIR/bin/uya" microapp --help >"$HELP_LOG" 2>&1
