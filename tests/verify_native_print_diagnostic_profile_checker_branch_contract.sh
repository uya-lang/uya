...
require_pattern "$TODO_DOC" 'compiler_print_diagnostic_profile\(\.\.\.\).*checker 非空分支补' \
    "todo 缺少 compiler_print_diagnostic_profile checker branch 合同任务"
...
require_pattern "$STAGE1_TEST" 'verify_native_print_diagnostic_profile_checker_branch_contract\.sh' \
    "stage1 未纳入 compiler_print_diagnostic_profile checker branch 合同"
