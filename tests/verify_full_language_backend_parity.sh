...
run_case() {
    ...
    if [[ "$REQUIRE_NATIVE_PARITY" == "1" &&
        ( "$case_id" == "hello" || "$case_id" == "generic" ||
          ...
          "$case_id" == "slice_array" || "$case_id" == "stdlib_entry" ) ]]; then
...
if [[ "$REQUIRE_NATIVE_PARITY" == "1" ]]; then
    for required_case in hello generic method error_union_try try_catch defer errdefer struct_union_enum slice_array stdlib_entry; do
