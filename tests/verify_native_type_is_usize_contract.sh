...
# Native build-seed 边界：固定 native_build_type_is_usize(...)
...
require_pattern "$SUBSET_DOC" 'str_equals\(type_node\.type_named_name, "usize" as \*byte\) != 0' \
    "subset doc 缺少 usize str_equals branch"
require_pattern "$SUBSET_DOC" 'native_hosted_reachable_body_complete: function=native_build_type_is_usize prefix_stmts=3 reason=body_complete' \
    "subset doc 缺少 body-complete frontier"

require_pattern "$BUILD_DRIVER_SRC" 'fn native_build_type_is_usize\(type_node: &ASTNode\) i32' \
...
echo "verify_native_type_is_usize_contract: ok"
