...
if ! CFLAGS="${CFLAGS:-} -fno-builtin" "$ROOT_DIR/tests/run_programs_parallel.sh" --uya tests/test_kernel_update.uya >"$UPDATE_LOG" 2>&1; then
...
if ! CFLAGS="${CFLAGS:-} -fno-builtin" "$ROOT_DIR/tests/run_programs_parallel.sh" --uya tests/test_kernel_sim.uya >"$SIM_LOG" 2>&1; then
