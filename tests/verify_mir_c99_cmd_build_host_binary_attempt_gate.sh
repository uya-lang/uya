...
require_pattern "$log_file" '^completed_body_detail=native_hosted_reachable_body_complete:function=native_build_lexical_drop_shape_empty,prefix_stmts=1,reason=body_complete$' \
    "diagnostic log records the completed lexical-drop helper body"
require_pattern "$log_file" '^completed_coverage=generic_corebody_lexical_drop_shape_empty_struct_return_lowering$' \
    "diagnostic log records the migrated lexical-drop empty struct return lowering"
...
require_pattern "$log_file" '^next_coverage=generic_corebody_interface_method_shape_empty_struct_return_lowering$' \
    "diagnostic log records the next generic interface-method shape empty-struct return slice"
