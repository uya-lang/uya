...
require_pattern "$log_file" '^completed_body_detail=native_hosted_reachable_body_complete:function=native_build_defer_local_assign_shape_empty,prefix_stmts=1,reason=body_complete$' \
    "diagnostic log records the completed defer-local-assign helper body"
require_pattern "$log_file" '^completed_coverage=generic_corebody_defer_local_assign_shape_empty_struct_return_lowering$' \
    "diagnostic log records the migrated defer-local-assign empty struct return lowering"
...
require_pattern "$log_file" '^next_coverage=generic_corebody_lexical_drop_shape_empty_struct_return_lowering$' \
    "diagnostic log records the next generic lexical-drop shape empty-struct return slice"
