...
require_pattern "$log_file" '^completed_body_detail=native_hosted_reachable_body_complete:function=native_build_interface_method_shape_empty,prefix_stmts=1,reason=body_complete$' \
    "diagnostic log records the completed interface-method helper body"
require_pattern "$log_file" '^completed_coverage=generic_corebody_interface_method_shape_empty_struct_return_lowering$' \
    "diagnostic log records the migrated interface-method empty struct return lowering"
...
require_pattern "$log_file" '^next_coverage=generic_corebody_direct_method_shape_empty_struct_return_lowering$' \
    "diagnostic log records the next generic direct-method shape empty-struct return slice"
