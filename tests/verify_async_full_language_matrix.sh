...
run_baseline_matrix() {
    ...
    expect_check_fail "tests/error_async_await_in_defer.uya" "@await 不能出现在 defer/errdefer 块中；清理逻辑必须保持同步" "${args[@]}"
    expect_check_fail "tests/error_async_await_in_errdefer.uya" "@await 不能出现在 defer/errdefer 块中；清理逻辑必须保持同步" "${args[@]}"
