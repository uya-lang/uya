...
int32_t uya_main(void) {
const enum Status status1 = INIT;
const enum Status status2 = RUNNING;
const enum Status status3 = PAUSED;
const enum Status status4 = STOPPED;
const enum Status status5 = ERROR;
if ((status1 == INIT)) {
    if ((status2 == RUNNING)) {
        if ((status3 == PAUSED)) {
            if ((status4 == STOPPED)) {
