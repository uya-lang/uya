...
// 字符串常量
static const uint8_t str0[] = "FFI pointer cast test passed!";
...
extern int32_t test_ffi_ptr(struct Point * ptr);
...
struct Point * ptr = (&p);
const int32_t result = test_ffi_ptr((struct Point *)ptr);
const int32_t str_result = puts((uint8_t *)(uint8_t *)str0);
if ((str_result != 1)) {
    int32_t _uya_ret = 1;
    return _uya_ret;
}
