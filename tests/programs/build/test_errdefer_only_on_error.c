...
struct err_union_int32_t f(int32_t flag);
int32_t uya_main(void);

struct err_union_int32_t f(int32_t flag) {
    int32_t x = 0;
    if ((flag != 0)) {
        return (struct err_union_int32_t){ .error_id = 177642U, .value = 0 };
    }
    struct err_union_int32_t _uya_ret = (struct err_union_int32_t){ .error_id = 0, .value = 42 };
    /* defer */     x = (x + 1);
    return _uya_ret;
}
