...
struct err_union_int32_t uya_main(void) {
const int32_t x = ({ struct err_union_int32_t _uya_try_tmp = foo(); if (_uya_try_tmp.error_id != 0) return _uya_try_tmp; _uya_try_tmp.value; });
struct err_union_int32_t _uya_ret = (struct err_union_int32_t){ .error_id = 0, .value = x };
return _uya_ret;
}
