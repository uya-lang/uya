...
TMP_DIR="$(mktemp -d /tmp/uya-c99-fallback-seed.XXXXXX)"
...
if ! UYA_FAKE_CC_CALL_LOG="$CALL_LOG" make -C "$FIXTURE_REPO" from-c \
    ...
    echo "错误: fallback fixture make from-c 失败" >&2
...
cmp "$FIXTURE_REPO/backup/uya.c" "$FIXTURE_REPO/bin/uya.c"

if ! grep -q 'bin/uya.c' "$CALL_LOG"; then
    echo "错误: fallback from-c 未编译 bin/uya.c" >&2
    cat "$CALL_LOG" >&2
    exit 1
fi
if ! grep -q 'backup/cmd-build.c' "$CALL_LOG"; then
    echo "错误: fallback from-c 未编译通用 cmd-build seed" >&2
    cat "$CALL_LOG" >&2
    exit 1
fi

echo "verify_c99_fallback_seed_restore: ok"
