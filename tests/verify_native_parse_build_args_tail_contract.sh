...
require_pattern "$SUBSET_DOC" 'native_hosted_reachable_tail_branch_frontier: function=parse_build_args parent_stmt=26 covered_branch=tail-c-output-infer next_branch=tail-native-c-reject next_kind=AST_IF_STMT reason=partial_tail_branch' \
    "subset doc 缺少 tail .c 输出推断后的 native .c 拒绝 frontier"
...
require_pattern "$NO_SILENT_TEST" 'native_hosted_reachable_tail_branch_frontier: function=parse_build_args parent_stmt=26 covered_branch=tail-c-output-infer next_branch=tail-native-c-reject next_kind=AST_IF_STMT reason=partial_tail_branch' \
    "no-silent-C99 测试缺少 tail .c 输出推断后的 native .c 拒绝 frontier"
