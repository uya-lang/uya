#!/usr/bin/env bash
#
# Focused real-CLI gate for imported extern aggregate / error-union signatures.
...
tmp_dir="$(mktemp -d /tmp/uya-mir-c99-extern-aggregate-error-union.XXXXXX)"
...
run_case tests/test_async_return_value.uya
run_case tests/test_syscall_time.uya
run_case tests/test_https_google.uya

echo "OK: MIR-C99 imported extern aggregate / error-union signatures no longer stop at extern_signature_requires_i32_scalars"
