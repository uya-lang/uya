...
"$ROOT_DIR/bin/uya" microapp run examples/microapp/microcontainer_hello_source.uya >"$RUN_LOG" 2>&1
...
rm -f "$LOADER_UAPP"
"$ROOT_DIR/bin/uya" microapp build examples/microapp/microcontainer_hello_source.uya -o "$LOADER_UAPP" >"$BUILD_LOG" 2>&1
