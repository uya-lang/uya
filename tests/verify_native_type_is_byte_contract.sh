...
# Native build-seed 边界：固定 native_build_type_is_byte(...)
...
require_pattern "$TODO_DOC" 'native_build_type_is_byte\(\.\.\.\)' \
    "todo 缺少 native_build_type_is_byte 合同任务"
require_pattern "$SUBSET_DOC" '^## `native_build_type_is_byte\(\.\.\.\)` Body Complete Contract' \
    "subset doc 缺少 native_build_type_is_byte 合同"
require_pattern "$SUBSET_DOC" 'native_hosted_pending_body_frontier: function=native_build_type_is_byte .*body_stmts=3 reason=pending_core_body' \
    "subset doc 缺少当前 native_build_type_is_byte pending frontier"
...
require_pattern "$SUBSET_DOC" 'str_equals\(type_node\.type_named_name, "byte" as \*byte\) != 0' \
...
require_pattern "$BUILD_DRIVER_SRC" 'str_equals\(type_node\.type_named_name, "byte" as \*byte\) != 0' \
    "源码缺少 byte str_equals branch"
require_pattern "$STAGE1_TEST" 'verify_native_type_is_byte_contract\.sh' \
    "stage1 未纳入 native_build_type_is_byte 合同"

echo "verify_native_type_is_byte_contract: ok"
