...
# semantic_vector_release(&checker.typed_type_records) 调用的 CoreBody/PortableMIR 切片合同。
...
require_pattern "$TODO_DOC" 'semantic_vector_release\(&checker\.typed_type_records\)' \
    "todo 缺少 compile_stats typed_type_records release 调用任务"
require_pattern "$SUBSET_DOC" '^## `compile_stats_record_and_release_typed_program\(\.\.\.\)` Typed Type Records Release Slice Contract' \
    "subset doc 缺少 compile_stats typed_type_records release 合同"
require_pattern "$SUBSET_DOC" 'semantic_vector_release\(&checker\.typed_type_records\);' \
    "subset doc 缺少 typed_type_records release 调用 surface"
require_pattern "$SUBSET_DOC" 'prefix_stmts=16 next_stmt=16 next_kind=AST_CALL_EXPR reason=partial_core_body' \
    "subset doc 缺少 typed_type_records release 调用前 frontier"
require_pattern "$SUBSET_DOC" 'prefix_stmts=17 next_stmt=17 next_kind=AST_ASSIGN reason=partial_core_body' \
    "subset doc 缺少 typed_type_records release 调用后的下一 frontier"
...
require_pattern "$NO_SILENT_TEST" 'compile_stats_record_and_release_typed_program' \
...
require_pattern "$STAGE1_TEST" 'verify_native_compile_stats_typed_type_records_release_contract\.sh' \
    "stage1 未纳入 compile_stats typed_type_records release 合同"

echo "verify_native_compile_stats_typed_type_records_release_contract: ok"
