...
require_pattern() {
    ...
    if ! grep -Eq "$pattern" "$file"; then
        echo "错误: SemanticDb functions_by_name 缺少证据: $description" >&2
        return 1
    fi
}
...
require_pattern "$DB_FILE" "^export[[:space:]]+struct[[:space:]]+SemanticFunctionOverloadRange" "FunctionOverloadRange 结构"
require_pattern "$DB_FILE" "function_ranges:[[:space:]]+SemanticVector" "function range 为动态 vector"
require_pattern "$DB_FILE" "function_range_decl_ids:[[:space:]]+SemanticVector" "函数 DeclId range 数据为动态 vector"
require_pattern "$DB_FILE" "functions_by_name:[[:space:]]+SemanticHash" "functions_by_name 为动态 hash"
require_pattern "$BUILD_FILE" "semantic_db_rebuild_functions_by_name" "构建 functions_by_name 索引"

tmp_dir="$(mktemp -d /tmp/uya-semantic-db-function-ranges.XXXXXX)"
...
test "semantic db functions_by_name keeps only function overloads" {
    ...
    var same_name_alias: ASTNode = semantic_test_node(ASTNodeType.AST_TYPE_ALIAS, "family_alias.uya", "family");
    var helper_fn: ASTNode = semantic_test_node(ASTNodeType.AST_FN_DECL, "helper.uya", "helper");

    var decls: [&ASTNode: 4] = [];
    decls[0] = &body_fn;
    decls[1] = &same_name_alias;
    decls[2] = &stub_fn;
    decls[3] = &helper_fn;
    ...
    try assert_eq_i32(semantic_db_function_overload_range_count(&db), 2);

    const family_name_id: i32 = semantic_db_find_interned_name(&db, "family");
    const helper_name_id: i32 = semantic_db_find_interned_name(&db, "helper");
    try expect(family_name_id >= 0);
    try expect(helper_name_id >= 0);

    var family_range: SemanticFunctionOverloadRange = SemanticFunctionOverloadRange{
        name_id: -1,
        function_start: -1,
        function_count: 0,
    };
    try assert_eq_i32(semantic_db_find_function_overload_range(&db, family_name_id, &family_range), 1);
    try assert_eq_i32(family_range.name_id, family_name_id);
    try assert_eq_i32(family_range.function_count, 2);
    try assert_eq_i32(semantic_db_function_range_decl_id(&db, &family_range, 0), 0);
    try assert_eq_i32(semantic_db_function_range_decl_id(&db, &family_range, 1), 2);

    var helper_range: SemanticFunctionOverloadRange = SemanticFunctionOverloadRange{
        name_id: -1,
        function_start: -1,
        function_count: 0,
    };
    try assert_eq_i32(semantic_db_find_function_overload_range(&db, helper_name_id, &helper_range), 1);
    try assert_eq_i32(helper_range.function_count, 1);
    try assert_eq_i32(semantic_db_function_range_decl_id(&db, &helper_range, 0), 3);
...
echo "✓ SemanticDb functions_by_name range lookup passed"
