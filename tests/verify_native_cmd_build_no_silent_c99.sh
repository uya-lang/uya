...
run_cmd_build_self_preflight_check() {
    ...
    grep -Eq 'native_hosted_coreir_preflight: status=0 verifier_error=0 functions=[1-9][0-9]* core_bodies=37 pending_bodies=[1-9][0-9]*' "$stderr"
    grep -Eq 'native_hosted_preflight: status=0 verifier_error=0 mir_extern_functions=[1-9][0-9]* mir_body_functions=36 mir_types=[1-9][0-9]* extern_symbols=[1-9][0-9]* c_import_objects=0 hosted_link_objects=0' "$stderr"
    ...
    grep -Eq 'native_hosted_pending_body_frontier: function=native_build_type_is_i32_array .*reason=pending_core_body' "$stderr"
