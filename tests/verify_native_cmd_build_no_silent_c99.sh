...
run_cmd_build_self_preflight_check() {
    ...
    grep -Eq 'native_hosted_coreir_preflight: status=0 verifier_error=0 functions=[1-9][0-9]* core_bodies=42 pending_bodies=[1-9][0-9]*' "$stderr"
    grep -Eq 'native_hosted_preflight: status=0 verifier_error=0 mir_extern_functions=[1-9][0-9]* mir_body_functions=41 mir_types=[1-9][0-9]* extern_symbols=[1-9][0-9]* c_import_objects=0 hosted_link_objects=0' "$stderr"
    ...
    grep -Eq 'native_hosted_pending_body_frontier: function=native_build_decl_is_two_i32_param_fn .*reason=pending_core_body' "$stderr"
    if grep -Eq 'native_hosted_pending_body_frontier: function=native_build_decl_is_identity_generic_i32_fn .*reason=pending_core_body' "$stderr"; then
        echo "错误: $label self-build 不应在 native_build_decl_is_identity_generic_i32_fn 迁入后继续报告 pending" >&2
        cat "$stderr" >&2
        exit 1
    fi
    if grep -Eq 'native_hosted_pending_body_frontier: function=native_build_decl_is_one_i32_param_fn .*reason=pending_core_body' "$stderr"; then
        echo "错误: $label self-build 不应在 native_build_decl_is_one_i32_param_fn 迁入后继续报告 pending" >&2
        cat "$stderr" >&2
        exit 1
    fi
    if grep -Eq 'native_hosted_pending_body_frontier: function=native_build_decl_is_noarg_i32_fn .*reason=pending_core_body' "$stderr"; then
        echo "错误: $label self-build 不应在 native_build_decl_is_noarg_i32_fn 迁入后继续报告 pending" >&2
        cat "$stderr" >&2
        exit 1
    fi
    if grep -Eq 'native_hosted_pending_body_frontier: function=native_build_type_is_byte_path_max_array .*reason=pending_core_body' "$stderr"; then
        echo "错误: $label self-build 不应在 native_build_type_is_byte_path_max_array 迁入后继续报告 pending" >&2
        cat "$stderr" >&2
        exit 1
    fi
    if grep -Eq 'native_hosted_pending_body_frontier: function=native_build_type_is_i32_array .*reason=pending_core_body' "$stderr"; then
        echo "错误: $label self-build 不应在 native_build_type_is_i32_array 迁入后继续报告 pending" >&2
        cat "$stderr" >&2
        exit 1
    fi
    if grep -Eq 'native_hosted_pending_body_frontier: function=native_build_type_is_i32_like_ptr .*reason=pending_core_body' "$stderr"; then
        echo "错误: $label self-build 不应在 native_build_type_is_i32_like_ptr 迁入后继续报告 pending" >&2
        cat "$stderr" >&2
        exit 1
    fi
    if grep -Eq 'native_hosted_pending_body_frontier: function=native_build_type_is_i32_ptr .*reason=pending_core_body' "$stderr"; then
        echo "错误: $label self-build 不应在 native_build_type_is_i32_ptr 迁入后继续报告 pending" >&2
        cat "$stderr" >&2
        exit 1
    fi
    if grep -Eq 'native_hosted_pending_body_frontier: function=native_build_type_is_byte_ptr .*reason=pending_core_body' "$stderr"; then
        echo "错误: $label self-build 不应在 native_build_type_is_byte_ptr 迁入后继续报告 pending" >&2
        cat "$stderr" >&2
        exit 1
    fi
    if grep -Eq 'native_hosted_pending_body_frontier: function=native_build_type_is_byte .*reason=pending_core_body' "$stderr"; then
        echo "错误: $label self-build 不应在 native_build_type_is_byte 迁入后继续报告 pending" >&2
        cat "$stderr" >&2
        exit 1
    fi
    ...
    if grep -Eq 'native_hosted_pending_body_frontier: function=native_build_decl_returns_param_directly .*reason=pending_core_body' "$stderr"; then
        echo "错误: $label self-build 不应在 native_build_decl_returns_param_directly 迁入后继续报告 pending" >&2
        cat "$stderr" >&2
        exit 1
    fi
    if grep -Eq 'native_hosted_pending_body_frontier: function=native_build_type_named_equals .*reason=pending_core_body' "$stderr"; then
        echo "错误: $label self-build 不应在 native_build_type_named_equals 迁入后继续报告 pending" >&2
        cat "$stderr" >&2
        exit 1
    fi
    if grep -Eq 'native_hosted_pending_body_frontier: function=native_build_type_is_usize .*reason=pending_core_body' "$stderr"; then
        echo "错误: $label self-build 不应在 native_build_type_is_usize 迁入后继续报告 pending" >&2
        cat "$stderr" >&2
        exit 1
    fi
