#!/usr/bin/env bash
#
# MIR-C99 CFG function mapping verifier.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
CFG_FILE="$REPO_ROOT/src/codegen/mir_c99/cfg.uya"
DRIVER_FILE="$REPO_ROOT/src/codegen/mir_c99/driver.uya"

require_pattern() {
    local file="$1"
    local pattern="$2"
    local description="$3"
    if ! grep -Eq "$pattern" "$file"; then
        echo "error: MIR-C99 CFG function plan missing evidence: $description" >&2
        exit 1
    fi
}

for file in "$CFG_FILE" "$DRIVER_FILE"; do
    if [[ ! -f "$file" ]]; then
        echo "error: missing $file" >&2
        exit 1
    fi
done

require_pattern "$CFG_FILE" 'export struct MirC99FunctionPlanEntry' "function plan entry"
require_pattern "$CFG_FILE" 'export struct MirC99BlockPlanEntry' "block plan entry"
require_pattern "$CFG_FILE" 'export struct MirC99TerminatorPlanEntry' "terminator plan entry"
require_pattern "$CFG_FILE" 'function_id:[[:space:]]*MirFunctionId' "MIR function id captured"
require_pattern "$CFG_FILE" 'signature_type_id:[[:space:]]*MirTypeId' "signature type id captured"
require_pattern "$CFG_FILE" 'block_start:[[:space:]]*i32' "block range captured"
require_pattern "$CFG_FILE" 'entry_block_id:[[:space:]]*MirBlockId' "entry block captured"
require_pattern "$CFG_FILE" 'c_label_source_id:[[:space:]]*i32' "C label source id captured"
require_pattern "$CFG_FILE" 'terminator_id:[[:space:]]*MirTerminatorId' "block terminator captured"
require_pattern "$CFG_FILE" 'branch_target_block_id:[[:space:]]*MirBlockId' "BR target block captured"
require_pattern "$CFG_FILE" 'condition_value_id:[[:space:]]*MirValueId' "COND_BR condition value captured"
require_pattern "$CFG_FILE" 'true_target_block_id:[[:space:]]*MirBlockId' "COND_BR true target captured"
require_pattern "$CFG_FILE" 'false_target_block_id:[[:space:]]*MirBlockId' "COND_BR false target captured"
require_pattern "$CFG_FILE" 'return_value_id:[[:space:]]*MirValueId' "RETURN value captured"
require_pattern "$CFG_FILE" 'MIR_TERMINATOR_KIND_BR' "BR terminator handled"
require_pattern "$CFG_FILE" 'MIR_TERMINATOR_KIND_COND_BR' "COND_BR terminator handled"
require_pattern "$CFG_FILE" 'MIR_TERMINATOR_KIND_RETURN' "RETURN terminator handled"
require_pattern "$CFG_FILE" 'successor_count == 1' "BR requires one successor"
require_pattern "$CFG_FILE" 'successor_count == 2' "COND_BR requires two successors"
require_pattern "$CFG_FILE" 'goto bb' "BR emits low-level C goto"
require_pattern "$CFG_FILE" 'if \(tmp' "COND_BR emits low-level C if/goto"
require_pattern "$CFG_FILE" 'return tmp' "RETURN emits low-level C value return"
require_pattern "$CFG_FILE" 'return;' "RETURN emits low-level C void return"
require_pattern "$CFG_FILE" 'while i < module\.blocks\.count' "all MIR blocks scanned"
require_pattern "$CFG_FILE" 'while i < module\.terminators\.count' "all MIR terminators scanned"
require_pattern "$CFG_FILE" 'mir_c99_plan_append_ref\(output_plan,[[:space:]]*MIR_C99_REF_KIND_FUNCTION' "program function ref registered"
require_pattern "$CFG_FILE" 'mir_c99_unit_append_ref\(unit,[[:space:]]*MIR_C99_REF_KIND_FUNCTION' "unit function ref registered"
require_pattern "$CFG_FILE" 'while i < module\.functions\.count' "all MIR functions scanned"
require_pattern "$DRIVER_FILE" 'use codegen\.mir_c99\.cfg' "driver imports CFG plan"
require_pattern "$DRIVER_FILE" 'cfg_plan:[[:space:]]*&MirC99CfgPlan' "driver accepts CFG plan"
require_pattern "$DRIVER_FILE" 'mir_c99_cfg_plan_build_functions' "driver builds function mapping"
require_pattern "$DRIVER_FILE" 'mir_c99_cfg_plan_build_blocks' "driver builds block label mapping"
require_pattern "$DRIVER_FILE" 'mir_c99_cfg_plan_build_terminators' "driver builds terminator mapping"
require_pattern "$DRIVER_FILE" 'result\.function_count = cfg_plan\.function_count' "driver reports function count"
require_pattern "$DRIVER_FILE" 'result\.block_count = cfg_plan\.block_count' "driver reports block count"
require_pattern "$DRIVER_FILE" 'result\.terminator_count = cfg_plan\.terminator_count' "driver reports terminator count"

tmp="$(mktemp /tmp/mir_c99_cfg_function_plan.XXXXXX.uya)"
trap 'rm -f "$tmp"' EXIT
sed '/^use codegen\.mir_c99\./d' \
    "$REPO_ROOT/src/codegen/mir_c99/plan.uya" \
    "$REPO_ROOT/src/codegen/mir_c99/names.uya" \
    "$REPO_ROOT/src/codegen/mir_c99/types.uya" \
    "$CFG_FILE" \
    "$REPO_ROOT/src/codegen/mir_c99/unit_output.uya" \
    "$REPO_ROOT/src/codegen/mir_c99/emitter.uya" \
    "$REPO_ROOT/src/codegen/mir_c99/values.uya" \
    "$REPO_ROOT/src/codegen/mir_c99/place_memory.uya" \
    "$DRIVER_FILE" >"$tmp"
"$REPO_ROOT/bin/uya" check "$tmp" >/dev/null

echo "OK: MIR-C99 function-to-C-function plan verified"
