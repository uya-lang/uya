...
- **内置函数**（以 `@` 开头）：`@size_of`（类型大小）、`@align_of`（类型对齐）、`@len`（数组长度）、`@max`/`@min`（整数极值）
...
**维护策略**：C 版编译器（`src/`）将维护至 Uya 版编译器（`../src/`）完整实现。在此之前，C 版为唯一 bootstrap 与自举验证参考；新特性在 C 与 Uya 两套实现中同步维护，以保证 `../src/compile.sh --c99 -b` 自举对比有效。
...
```bash
# 生成 C99 源代码
./build/compiler-c program.uya -o program.c

# 多文件编译
./build/compiler-c file1.uya file2.uya file3.uya -o output.c
...
# 或者使用 -exec 选项自动编译并链接
./build/compiler-c program.uya -o program.c -exec
```
