...
- [ ] 创建 `src/codegen/generator_test.go`
  - [ ] 测试 CodeGenerator 创建
  - [ ] 测试简单 IR 到 C 代码生成

#### TDD Green: 实现 CodeGenerator 核心

- [ ] 创建 `src/codegen/generator.go`（<1500行）
  - [ ] 定义 CodeGenerator 结构体
  - [ ] 实现 NewCodeGenerator 函数
  - [ ] 实现 Generate 函数（主入口，<150行）
  - [ ] 实现代码输出管理
  - [ ] 验证文件<1500行，所有函数<150行
...
- [ ] 创建 `src/codegen/inst.go`（<1500行）
  - [ ] 实现 generateInstruction 函数（主入口，<100行）
  - [ ] 实现各种 IR 指令到 C 代码的转换函数（每个<150行）
  - [ ] 验证文件<1500行，所有函数<150行

#### TDD Green: 实现类型生成

- [ ] 创建 `src/codegen/type.go`（<800行）
  - [ ] 实现 IR 类型到 C 类型的转换函数
  - [ ] 验证文件<800行，所有函数<150行

#### TDD Green: 实现值生成

- [ ] 创建 `src/codegen/value.go`（<800行）
  - [ ] 实现值到 C 代码的转换函数
  - [ ] 验证文件<800行，所有函数<150行

#### TDD Green: 实现错误处理生成

- [ ] 创建 `src/codegen/error.go`（<600行）
  - [ ] 实现错误联合类型代码生成
  - [ ] 实现错误处理代码生成
  - [ ] 验证文件<600行，所有函数<150行
...
- [ ] 创建 `cmd/uyac/main_test.go`
  - [ ] 测试编译流程
  - [ ] 测试错误处理

#### TDD Green: 实现主程序

- [ ] 创建 `cmd/uyac/main.go`（<800行）
  - [ ] 实现 main 函数（<100行）
  - [ ] 实现命令行参数解析
  - [ ] 实现错误处理和退出码
  - [ ] 验证文件<800行，所有函数<150行
...
- [ ] 创建 `cmd/uyac/compile.go`（<500行）
  - [ ] 实现 compileFile 函数（<150行）
  - [ ] 整合所有模块的调用流程
  - [ ] 实现资源清理
  - [ ] 验证文件<500行，所有函数<150行
