...
bench-compile-stats:
	@MAKE="$(MAKE)" CC="$(CC)" CC_DRIVER="$(CC_DRIVER)" CC_TARGET_FLAGS="$(CC_TARGET_FLAGS)" HOST_OS="$(HOST_OS)" HOST_ARCH="$(HOST_ARCH)" TARGET_OS="$(TARGET_OS)" TARGET_ARCH="$(TARGET_ARCH)" TARGET_TRIPLE="$(TARGET_TRIPLE)" TOOLCHAIN="$(TOOLCHAIN)" ZIG="$(ZIG)" CFLAGS="$(CFLAGS)" LDFLAGS="$(LDFLAGS)" bash scripts/bench_compile_stats.sh $(ARGS)
...
bench-compiler-1s-check:
	@./tests/verify_bench_compiler_1s.sh
...
help:
	...
	@echo "  make bench-compiler-1s-check - 验证 1 秒冷构建 benchmark 脚本"
